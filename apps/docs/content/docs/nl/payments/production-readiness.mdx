...
[`@solana/kit`](https://github.com/anza-xyz/kit) biedt helperfuncties die de
resourcelimieten van een transactie schatten en deze in één stap op het bericht
instellen (met de methode `simulatetransaction` onder de motorkap). Voor versie
1-transacties schatten deze helpers ook de datalimiet voor geladen accounts.
...
<Callout type="info">
  ...
  toe bij het verzenden (`.sendTransaction()`). De handmatige werkwijze
  hierboven is bedoeld voor wanneer u transacties rechtstreeks samenstelt met
  `@solana/kit`.
</Callout>

In productieomgevingen, wanneer u hetzelfde type transactie meerdere keren
herhaalt, overweeg dan de compute-schatting voor dat transactietype te cachen om
de overhead van het telkens opnieuw schatten van compute units te vermijden.

### Jito Bundles

Jito-bundles zijn een hulpmiddel voor het beheren van atomaire uitvoering van
...
Jito-netwerk te stimuleren uw transacties in een block op te nemen.
...
Transacties kunnen om vele redenen mislukken. Anders dan traditionele
betaal-API's die direct succes of mislukking teruggeven, vereisen
...
- **Blockhash-vervaldatum**: Transacties zijn geldig voor ~150 blokken (~60-90
  seconden)
- **Idempotentie**: Dezelfde ondertekende transactie levert altijd dezelfde
  handtekening op — opnieuw indienen is veilig
- **Exponentiële terugval**: Vermijd het overbelasten van het netwerk met snelle
  herhaalde pogingen
...
bevestigingspolling en blokhoogte-tracking automatisch. Het gooit
...
| ----------- | ----------------------------- | -------------------------- | ----------------------- |
| `processed` | In een blok, nog niet gestemd | Autorisatie in behandeling | Realtime UI-updates     |
| `confirmed` | Supermeerderheid gestemd      | Verrekende middelen        | **Meeste betalingen**   |
| `finalized` | Verankerd, onomkeerbaar       | Afgewikkelde middelen      | Hoge waarde, compliance |

**Wanneer elk te gebruiken:**

- **UI-updates**: Toon `processed` voor directe feedback ("Betaling ingediend")
- **Gebruikersaccount tegoed bijschrijven**: Wacht op `confirmed` (veilig voor
  de meeste transacties)
...
Zie voor meer informatie over het controleren van de transactiestatus
...
foutcodes in plaats van stringovereenkomst:
...
| `INSUFFICIENT_FUNDS_FOR_FEE` | Onvoldoende SOL         | Betalende partij financieren of gebruik [fee abstraction](/docs/payments/send-payments/payment-processing/fee-abstraction) |
...
Gebruikers verwachten te betalen in stablecoins, niet dat ze SOL moeten
aanschaffen voor netwerkkosten. Transacties zonder gas lossen dit
op—vergelijkbaar met hoe Venmo-gebruikers niet nadenken over ACH-kosten. Zie
...
- **Scheid hot- en cold-wallets.** Hot wallet voor operaties, cold voor de
  treasury.
- **Maak back-ups van alle productiesleutels.** Sla versleutelde back-ups op op
  meerdere beveiligde locaties. Het verliezen van een sleutel betekent permanent
  verlies van toegang.
- **Gebruik verschillende sleutels voor devnet en mainnet.** Je devnet-sleutels
  ...
  om te zorgen dat de juiste sleutels voor elk netwerk worden geladen.

### Ondertekeninfrastructuur

Voor backend-ondertekening in productie gebruik je
[Keychain](/docs/tools/keychain)—een uniforme ondertekenbibliotheek die meerdere
sleutelbeheer-backends abstraheert via één interface: Memory, Vault, Privy,
Turnkey, AWS KMS, Fireblocks, GCP KMS, CDP, Para, Dfns, Crossmint, Openfort en
Utila. Hiermee kun je lokaal ontwikkelen met in-memory sleutels en vervolgens
overschakelen naar productie-grade backends zonder de applicatiecode te
wijzigen.
