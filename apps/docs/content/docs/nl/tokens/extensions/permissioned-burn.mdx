...
## Hoe maak je een Mint en Burn met toestemming

Om een mint met permissioned burn te maken en tokens te verbranden:

1. Bereken de mint account-grootte en de rent die nodig zijn voor de mint en de
...
4. Verbrand met _rs`PermissionedBurnInstruction::BurnChecked`_, ondertekend door
   zowel de token account-eigenaar als de burn authority.
5. Schakel optioneel permissioned burning uit met _rs`SetAuthority`_ via
   _rs`AuthorityType::PermissionedBurn`_ en een `None`-authority, waardoor de
   standaard burn-instructies weer worden ingeschakeld. De volledige
...
Bereken de rent op basis van de grootte die nodig is voor de mint plus de
_rs`PermissionedBurnConfig`_-extensie.
...
Initialiseer de _rs`PermissionedBurnConfig`_-extensie op de mint met de burn
authority.
...
<Callout type="info" title="Volgorde van instructies">
  _rs`PermissionedBurnInstruction::Initialize`_ moet komen voor
...
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Verbrandingsinstructie met een decimalen-controle die dezelfde accounts gebruikt als _rs`PermissionedBurnInstruction::Burn`_.                       | [Bron](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Autoriteits-discriminator gebruikt met _rs`SetAuthority`_ om de verbrandingsautoriteit op een mint te roteren of uit te schakelen.                  | [Bron](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
...
| _rs`process_burn`_                             | Processorlogica die standaard verbrandingen afwijst wanneer de verbrandingsautoriteit is ingesteld en de handtekening van de autoriteit verifieert. | [Bron](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
...
Het gepubliceerde legacy `@solana/spl-token` pakket bevat nog geen ondersteuning
voor permissioned burn, dus er is geen legacy voorbeeld opgenomen.
