...
Om tokens op te nemen van het vertrouwelijk beschikbaar saldo naar het publieke
saldo:
...
1. Maak
   ...
   **Gelijkheidsbewijs
   ...
   Verifieert dat de resterende beschikbare saldociphertext na de opname
   overeenkomt met de bijbehorende
   [Pedersen-verbintenis](https://en.wikipedia.org/wiki/Commitment_scheme),
   waardoor wordt gegarandeerd dat het nieuwe beschikbare saldo van het account
   ...
   **Bereikbewijs
   ...
   binnen een opgegeven bereik valt.

2. Voor elk bewijs:
   - Roep het ZK ElGamal-bewijsprogramma aan om de bewijsgegevens te verifiëren.
   - Sla de bewijsspecifieke metadata op in een bewijs-"contextstatus"-account
...
4. Sluit de bewijsaccounts om de SOL die werd gebruikt om ze aan te maken terug
   te vorderen.

Het volgende diagram toont de stappen voor het opnemen van tokens van
vertrouwelijk beschikbaar saldo naar publiek saldo:
...
- Roep het Zk ElGamal-bewijsprogramma aan om de bewijzen te verifiëren en de
  "contextstatusaccounts" te initialiseren
- Roep de
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  instructie aan met de twee bewijsaccounts.
...
Het volgende voorbeeld laat zien hoe tokens kunnen worden opgenomen van
vertrouwelijk beschikbaar saldo naar publiek saldo.

Om het voorbeeld uit te voeren, start een lokale validator met het Token
Extensions Program gekloneerd van mainnet met behulp van de volgende opdracht. U
