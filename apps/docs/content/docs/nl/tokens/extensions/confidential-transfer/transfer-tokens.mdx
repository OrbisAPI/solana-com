...
andere, moeten zowel de zender als de ontvanger token accounts hebben die
geconfigureerd zijn met de _rs`ConfidentialTransferAccount`_ toestand en
goedgekeurd zijn voor vertrouwelijke overdrachten. Het token account van de
zender moet ook een beschikbaar vertrouwelijk saldo hebben om van over te
dragen.
...
1. Maak
   ...
   **Gelijkheidsbewijs
   ...
   Verifieert dat de nieuwe beschikbare saldotekst na de overdracht overeenkomt
   met de bijbehorende
   [Pedersen-verbintenis](https://en.wikipedia.org/wiki/Commitment_scheme),
   waarmee wordt gewaarborgd dat het nieuwe beschikbare saldo van het
   bronaccount correct wordt berekend als
   `new_balance = current_balance - transfer_amount`.

   **Cijfertekstgeldigheidsbewijs
   ...
   Verifieert dat de cijferteksten van het overdrachtsbedrag correct zijn
   gegenereerd voor alle drie de partijen (bron, bestemming en auditor), waarmee
   wordt gewaarborgd dat het overdrachtsbedrag correct versleuteld is onder de
...
3. Roep de
   ...
   instructie aan, met opgave van de bewijscontextstaataccounts.

4. Sluit de bewijscontextstaataccounts om de SOL te recupereren die is gebruikt
   om ze aan te maken.

Het volgende diagram toont de stappen die betrokken zijn bij het overdragen van
tokens van het token account van een zender naar het token account van een
ontvanger.
...
- Genereer een gelijkheidsbewijs, cijfertekstgeldigheidsbewijs en bereikbewijs
  aan de clientzijde
- Roep het Zk ElGamal-bewijsprogramma aan om de bewijzen te verifiëren en de
  "contextstaataccounts" te initialiseren
- Roep de
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  instructie aan met de drie bewijsaccounts.
...
Het volgende voorbeeld laat zien hoe je tokens vertrouwelijk kunt overdragen
tussen token accounts.

Om het voorbeeld uit te voeren, start je een lokale validator met het Token
Extensions Program dat gekloond is van mainnet met het volgende commando. Je
moet de Solana CLI geïnstalleerd hebben om de lokale validator te starten.
