...
## Hoe het openstaande saldo op het beschikbare saldo toepassen
...
1. **Vertrouwelijk openstaand saldo**: Initieel worden tokens "gestort" van het
   publieke saldo naar een "openstaand" vertrouwelijk saldo.
2. **Vertrouwelijk beschikbaar saldo**: Het openstaande saldo wordt vervolgens
   "toegepast" op het beschikbare saldo, waardoor de tokens beschikbaar worden
   voor vertrouwelijke overdrachten.

Dit gedeelte legt de tweede fase uit: het toepassen van het openstaande saldo op
het beschikbare saldo.

Wanneer tokens worden "gestort" vanuit het publieke saldo of wanneer tokens
vertrouwelijk worden overgedragen van de ene token account naar de andere,
worden de tokens aanvankelijk toegevoegd aan het vertrouwelijke openstaande
saldo. Voordat tokens kunnen worden gebruikt voor vertrouwelijke overdrachten,
moet het openstaande saldo worden "toegepast" op het beschikbare saldo.
...
het openstaande saldo op het beschikbare saldo:
...
Om een openstaand saldo om te zetten naar een beschikbaar saldo, roep de
...
Het volgende voorbeeld laat zien hoe het vertrouwelijke openstaande saldo op het
vertrouwelijke beschikbare saldo kan worden toegepast.

Om het voorbeeld uit te voeren, start een lokale validator met het Token
Extensions Program gekloond van mainnet met behulp van de volgende opdracht. De
Solana CLI moet geïnstalleerd zijn om de lokale validator te starten.
...
<Callout type="info">
  Op het moment van schrijven zijn Vertrouwelijke Overdrachten niet ingeschakeld
  op de standaard lokale validator. U moet het mainnet Token Extensions Program
