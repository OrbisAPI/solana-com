...
### Gebruik met een Kit Client (keychainSigner plugin)
...
`payer` en `identity`, zodat u de handmatige `pipe`-installatie zoals hierboven
beschreven kunt overslaan:
...
De plugin accepteert dezelfde backend-getagde configuratie als
`createKeychainSigner`, en alleen de backend waarnaar wordt doorgestuurd wordt
...
en backends te combineren op één enkele client:
...
Heeft u al een `SolanaSigner`? Elke keychain signer is een geldige Kit
`TransactionSigner`, zodat u een bestaande kunt installeren met
