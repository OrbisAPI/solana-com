...
<Callout type="info">
  Menjalankan headless — dalam rute API, worker, atau skrip tanpa dompet
  browser? Tandatangani dengan layanan manajemen kunci backend, bukan keypair
  mentah. Lihat [Keychain](/docs/tools/keychain) dan [Penandatanganan dalam
...
- **State machine headless**: Jalankan klien di dalam rute API, skrip, atau
  worker untuk menggunakan kembali logika orkestrasi dompet + RPC yang sama yang
  menggerakkan UI Anda.
- **Dashboard realtime**: Gabungkan watcher (saldo, akun, tanda tangan) dengan
  library UI pilihan Anda; klien menangani distribusi websocket dan invalidasi
  cache.
- **Store kustom**: Masukkan store Zustand milik Anda sendiri untuk menghidrasi
  dari IndexedDB/localStorage, mencerminkan state ke sesi server, atau
  berkoordinasi antar tab browser.
- **Jembatan ke React hooks**: Berikan instance klien yang telah dikonfigurasi
...
- **Kemampuan pengujian**: Antarmuka klien tunggal dapat di-mock dalam pengujian
  unit, sehingga mudah untuk mensimulasikan respons RPC atau sesi dompet tanpa
  dompet browser.
