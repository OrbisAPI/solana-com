...
Test validator lokal juga menyertakan build Token-2022 versi lama, sehingga
...
5. Secara opsional nonaktifkan permissioned burning dengan _rs`SetAuthority`_
   menggunakan _rs`AuthorityType::PermissionedBurn`_ dan otoritas `None`, yang
   mengaktifkan kembali instruksi burn standar. Contoh kode lengkap di bawah ini
   menampilkan langkah ini.
...
## !!steps Bakar dengan otoritas bakar

Bakar token dengan _rs`PermissionedBurnInstruction::BurnChecked`_ yang
ditandatangani oleh pemilik token account dan otoritas bakar.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Ekstensi mint yang menyimpan otoritas yang diperlukan untuk menandatangani setiap pembakaran bersama pada mint.                      | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Instruksi yang menginisialisasi konfigurasi pembakaran bersyarat sebelum _rs`InitializeMint`_.                                       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Instruksi bakar yang memerlukan tanda tangan dari otoritas bakar dan pemilik atau delegasi token account.                            | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Instruksi bakar dengan pemeriksaan desimal yang menggunakan akun yang sama dengan _rs`PermissionedBurnInstruction::Burn`_.           | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Diskriminator otoritas yang digunakan bersama _rs`SetAuthority`_ untuk merotasi atau menonaktifkan otoritas bakar pada mint.         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Logika prosesor yang menginisialisasi _rs`PermissionedBurnConfig`_ pada mint yang belum diinisialisasi dan menyimpan otoritas bakar. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Logika prosesor yang menolak pembakaran standar ketika otoritas bakar telah diatur dan memverifikasi tanda tangan otoritas.          | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logika prosesor yang memvalidasi dan merotasi otoritas bakar ketika _rs`AuthorityType::PermissionedBurn`_ digunakan.                 | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung.
Paket `@solana/spl-token` legacy yang diterbitkan belum menyertakan dukungan
burn dengan izin, sehingga tidak ada contoh legacy yang disertakan.
