...
## Cara mentransfer token secara rahasia dari satu token account ke token account lain

Untuk mentransfer token secara rahasia dari satu token account ke token account
lain, baik pengirim maupun penerima harus memiliki token account yang
...
1. Buat
   ...
   **Bukti Kesetaraan
   ...
   Memverifikasi bahwa ciphertext saldo tersedia baru setelah transfer sesuai
   dengan [komitmen Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme)
   yang berkaitan, memastikan saldo tersedia baru akun sumber dihitung dengan
   ...
   **Bukti Rentang
   ...
   Memverifikasi bahwa saldo tersedia baru dan jumlah transfer (dibagi menjadi
   bit rendah/tinggi) semuanya tidak negatif dan berada dalam rentang yang
   ditentukan.
...
- Hasilkan bukti kesetaraan, bukti validitas ciphertext, dan bukti rentang di
  sisi klien
...
- Tutup ketiga akun bukti untuk memulihkan rent.

Crate `spl_token_client` menyediakan metode-metode berikut:

- Metode `confidential_transfer_create_context_state_account` yang membuat
  sebuah proof account.
- Metode `confidential_transfer_transfer` yang memanggil instruksi `Transfer`.
- Metode `confidential_transfer_close_context_state_account` yang menutup sebuah
  proof account.

### Contoh Kode

Contoh berikut mendemonstrasikan cara mentransfer token secara rahasia antara
token account.
...
menginstal Solana CLI terlebih dahulu untuk memulai validator lokal.
...
<Callout type="info">
  ...
  validator lokal default. Anda harus mengkloning Token Extension Program
  mainnet untuk menjalankan kode contoh ini.
</Callout>
