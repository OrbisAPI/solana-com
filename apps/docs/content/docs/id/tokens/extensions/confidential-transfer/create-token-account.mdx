...
menambahkan status tambahan ke token account. Bagian ini menjelaskan cara
...
### Status Token Account Confidential Transfer
...
- **approved**: Status persetujuan akun untuk transfer konfidensial. Jika
  ...
  `true`, semua token account disetujui secara otomatis untuk transfer
  konfidensial.

- **elgamal_pubkey**: Kunci publik ElGamal yang digunakan untuk mengenkripsi
  saldo dan jumlah transfer.

- **pending_balance_lo**: 16 bit bawah terenkripsi dari saldo tertunda. Saldo
  dibagi menjadi bagian tinggi dan rendah untuk dekripsi yang efisien.

- **pending_balance_hi**: 48 bit atas terenkripsi dari saldo tertunda. Saldo
  dibagi menjadi bagian tinggi dan rendah untuk dekripsi yang efisien.

- **available_balance**: Saldo terenkripsi yang tersedia untuk transfer.

- **decryptable_available_balance**: Saldo yang tersedia dienkripsi dengan kunci
...
- **maximum_pending_balance_credit_counter**: Batas jumlah kredit tertunda
  sebelum mengharuskan instruksi _rs`ApplyPendingBalance`_ untuk mengonversi
...
untuk menstransfer token. Pemilik token account tidak akan dapat mentransfer
token karena saldo terenkripsi akan berubah antara saat transaksi diajukan dan
saat diproses, mengakibatkan transaksi gagal.
...
Pencocokkan antara penghitung yang diharapkan/aktual menunjukkan bahwa
`decryptable_available_balance` sesuai dengan `available_balance`.

Saat mengambil status token account untuk membaca
`decryptable_available_balance`, perbedaan nilai penghitung yang
diharapkan/aktual mengharuskan klien untuk mencari instruksi deposit/transfer
terbaru yang sesuai dengan selisih penghitung untuk menghitung saldo yang benar.
...
2. Ambil transaksi terbaru termasuk instruksi deposit dan transfer hingga
   selisih penghitung (aktual - yang diharapkan):
...
3. **Konfigurasi Confidential Transfer**: Panggil instruksi
   ...
   dari Token Extension Program untuk menginisialisasi status
...
Instruksi _rs`PubkeyValidityProofData`_ membuat bukti yang memverifikasi bahwa
kunci ElGamal adalah valid. Untuk detail implementasi, lihat:
...
Kode berikut menunjukkan cara membuat Associated Token Account dengan extensi
Confidential Transfer,
...
<Callout type="info">
  ...
  validator lokal default. Anda harus mengkloning Token Extensions Program
