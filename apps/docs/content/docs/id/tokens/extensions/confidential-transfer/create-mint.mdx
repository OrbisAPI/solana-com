...
_rs`ConfidentialTransferMint`_ memiliki tiga field konfigurasi:

- **authority**: Akun yang memiliki izin untuk mengubah pengaturan confidential
  transfer pada mint dan menyetujui token account baru jika auto-approval
  dinonaktifkan.

- **auto_approve_new_accounts**: Ketika disetel ke true, pengguna dapat membuat
  token account dengan confidential transfer yang diaktifkan secara default.
  Ketika false, authority harus menyetujui setiap token account baru sebelum
  dapat digunakan untuk confidential transfer.

- **auditor_elgamal_pubkey**: Auditor opsional yang dapat mendekripsi jumlah
  transfer dalam transaksi konfidensial, menyediakan mekanisme kepatuhan sambil
...
mengirimkan transaksi dengan ketiga instruksi tersebut dalam satu pemanggilan
fungsi, seperti yang ditunjukkan pada contoh di bawah ini.
...
<Callout type="info">
  ...
  mainnet untuk menjalankan kode contoh.
</Callout>
