...
untuk memperkirakan unit komputasi yang diperlukan untuk sebuah transaksi,
termasuk perkiraan unit komputasi yang akan digunakan.
...
<Callout type="info">
  Jika Anda membangun dan mengirim transaksi dengan klien plugin kit, Anda
  biasanya tidak memerlukan langkah ini — klien menambahkan instruksi anggaran
  komputasi untuk Anda saat mengirim (`.sendTransaction()`). Alur manual di atas
  adalah untuk saat Anda merakit transaksi secara langsung dengan `@solana/kit`.
</Callout>

Dalam produksi, jika Anda mengulangi jenis transaksi yang sama beberapa kali,
Anda sebaiknya mempertimbangkan untuk menyimpan cache perkiraan komputasi untuk
jenis transaksi tersebut guna menghindari overhead estimasi unit komputasi
setiap kali.

### Jito Bundles

Jito bundle adalah alat untuk mengelola eksekusi atomik dari beberapa transaksi.
Hal ini dicapai dengan mengirim beberapa transaksi ke jaringan Jito beserta tip.
Tip dapat digunakan untuk memberi insentif kepada jaringan Jito agar menyertakan
transaksi Anda dalam sebuah blok.
...
- **Idempotency**: Transaksi yang ditandatangani yang sama selalu menghasilkan
  tanda tangan yang sama — pengiriman ulang aman dilakukan
...
- [Helius: Cara Melancarkan Transaksi di Solana](https://www.helius.dev/blog/how-to-land-transactions-on-solana)
...
| ----------- | ------------------------------ | ----------------------- | ----------------------------- |
| `processed` | Dalam blok, belum divotasi     | Otorisasi tertunda      | Pembaruan UI real-time        |
| `confirmed` | Supermayoritas telah memvotasi | Dana telah dikliringkan | **Sebagian besar pembayaran** |
| `finalized` | Berakar, tidak dapat diubah    | Dana telah diselesaikan | Bernilai tinggi, kepatuhan    |
...
- **Kredit akun pengguna**: Tunggu `confirmed` (aman untuk sebagian besar
  transaksi)
- **Pengiriman barang fisik**: Tunggu `finalized`
...
Solana Kit menyediakan kesalahan bertipe melalui `isSolanaError()`. Gunakan kode
kesalahan spesifik alih-alih pencocokan string:
...
| Kode Kesalahan               | Penyebab                  | Pemulihan                                                                                                            |
| ---------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------- |
...
| `INSUFFICIENT_FUNDS_FOR_FEE` | SOL tidak mencukupi       | Danai pembayar biaya atau gunakan [abstraksi biaya](/docs/payments/send-payments/payment-processing/fee-abstraction) |
| `INSUFFICIENT_FUNDS`         | Token tidak mencukupi     | Pengguna membutuhkan saldo lebih banyak                                                                              |
| `ACCOUNT_NOT_FOUND`          | token account tidak ada   | Buat ATA dalam transaksi                                                                                             |

## Transaksi Tanpa Gas

Pengguna mengharapkan untuk membayar dengan stablecoin, bukan harus mendapatkan
SOL untuk biaya jaringan. Transaksi tanpa gas menyelesaikan masalah ini—mirip
seperti pengguna Venmo yang tidak perlu memikirkan biaya ACH. Lihat
...
- **Jangan pernah mengekspos kunci privat di kode frontend.** Gunakan
  penandatanganan backend, dompet hardware, dompet multitanda tangan, atau
...
- **Gunakan kunci berbeda untuk devnet dan mainnet.** Kunci devnet Anda
  sebaiknya tidak digunakan untuk mainnet. Gunakan konfigurasi berbasis
  lingkungan untuk memastikan kunci yang tepat dimuat untuk setiap jaringan.
...
mengabstraksikan berbagai backend manajemen kunci melalui satu antarmuka:
Memory, Vault, Privy, Turnkey, AWS KMS, Fireblocks, GCP KMS, CDP, Para, Dfns,
Crossmint, Openfort, dan Utila. Ini memungkinkan Anda mengembangkan secara lokal
dengan kunci dalam memori, lalu beralih ke backend tingkat produksi tanpa
