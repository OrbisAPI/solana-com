...
keputusan desain produksi. Lihat
[Penandatanganan di Produksi](/docs/core/transactions/signing-in-production).

Tanda tangan pertama dalam array dimiliki oleh **fee payer**, yaitu akun yang
membayar
[biaya dasar dan biaya prioritas](/docs/core/fees/fee-structure#base-fee)
transaksi. Tanda tangan pertama ini juga berfungsi sebagai **ID transaksi**,
yang digunakan untuk mencari transaksi di jaringan. ID transaksi umumnya disebut
sebagai **tanda tangan transaksi**.
...
- [`recent_blockhash`](mention:recent-blockhash): Sebuah
  [blockhash](#recent-blockhash) yang berfungsi sebagai stempel waktu untuk
  transaksi
...
adalah array public key yang dikodekan secara ringkas. Setiap entri
mengidentifikasi sebuah akun yang digunakan oleh setidaknya satu instruksi
transaksi. Array ini harus menyertakan setiap akun dan harus mengikuti urutan
ketat berikut:
...
<Callout>
  ...
  memungkinkan Anda mendapatkan blockhash saat ini dan tinggi blok terakhir saat
...
adalah array berenkode kompak dari struct
...
array `account_keys`, bukan berdasarkan kunci publik lengkap. Ini berisi:
...
memungkinkan System Program untuk memotong lamport. Akun pengirim maupun
penerima harus dapat ditulis (writable), agar saldo lamport keduanya dapat
berubah. Untuk menjalankan instruksi ini, dompet pengirim mengirimkan transaksi
yang berisi tanda tangannya dan pesan yang memuat instruksi transfer SOL.
...
akan menolak transfer yang akunnya tidak cocok dengan mint yang diharapkan.
Transfer **SOL** native tidak memiliki perlindungan semacam itu, sehingga
pengirim harus memverifikasi penerima sebelum menandatangani. Lihat
...
tidak didanai); tolak mint, token account, program, dan PDA yang tidak Anda
kendalikan.
