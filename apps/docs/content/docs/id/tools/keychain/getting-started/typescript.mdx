...
dan `identity` sehingga Anda dapat melewati pengaturan `pipe` manual seperti
...
Plugin ini menerima konfigurasi bertag backend yang sama seperti
`createKeychainSigner`, dan hanya backend yang dikirimkan yang akan dibundel
(paket backend dimuat melalui `import()` dinamis).

Gunakan `keychainPayer` atau `keychainIdentity` untuk mengatur hanya satu peran,
dan gabungkan backend pada satu klien:
...
## Membuat Penanda Tangan Kustom

Implementasikan antarmuka `SolanaSigner` untuk membuat penanda tangan kustom:
...
Lihat [panduan Menambahkan Penanda Tangan](/docs/tools/keychain/adding-signers)
