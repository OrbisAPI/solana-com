...
5. Opcionalmente, desative o burn com permissão usando _rs`SetAuthority`_ com
   ...
   as instruções padrão de burn. Os exemplos de código completos abaixo mostram
   esta etapa.
...
| _rs`PermissionedBurnInstruction::Burn`_        | Instrução de queima que exige assinaturas da autoridade de queima e do proprietário ou delegado do token account.                   | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Instrução de queima com verificação de decimais que utiliza as mesmas contas que _rs`PermissionedBurnInstruction::Burn`_.           | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
