...
Antes de os tokens poderem ser transferidos de forma confidencial, o saldo
público de tokens deve ser convertido em saldo confidencial. Esta conversão
...
Esta secção explica a primeira etapa: depositar o saldo público de tokens no
saldo pendente confidencial.

O diagrama seguinte mostra os passos envolvidos no depósito de tokens do saldo
...
Para converter um saldo público em saldo pendente confidencial, invoque a
instrução
...
O exemplo seguinte demonstra como depositar o saldo público de tokens no saldo
pendente confidencial.

Para executar o exemplo, inicie um validator local com o Token Extensions
Program clonado da mainnet usando o seguinte comando. É necessário ter o Solana
CLI instalado para iniciar o validator local.
...
<Callout type="info">
  No momento em que este texto foi escrito, as Transferências Confidenciais não
  estavam habilitadas no validator local padrão. É necessário clonar o Token
