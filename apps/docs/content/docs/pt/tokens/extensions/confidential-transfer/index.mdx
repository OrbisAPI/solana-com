...
5. **Aplicar** o saldo pendente do remetente ao **saldo disponível
   confidencial**.
6. **Transferir** tokens confidencialmente da token account do remetente para a
...
tokens correspondentes ao diagrama anterior. As etapas estão separadas em
...
CLI instalado para usar o validator local.
...
<Callout type="info">
  ...
  no validator local padrão. É necessário clonar o Token Extensions Program da
...
A lista completa de instruções da extensão de Transferência Confidencial
[instruções](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
são as seguintes:
