...
[Kit client](https://github.com/anza-xyz/kit), conectando-o como `payer` e
`identity`, dispensando a configuração manual de `pipe` mostrada acima:
...
O plugin aceita a mesma configuração com tag de backend que
...
Use `keychainPayer` ou `keychainIdentity` para definir apenas uma função e
combinar backends em um único client:
...
válido, portanto você pode instalar um existente com
...
## Criando Signatários Personalizados

Implemente a interface `SolanaSigner` para criar signatários personalizados:
...
Consulte o [guia de Adição de Signatários](/docs/tools/keychain/adding-signers)
