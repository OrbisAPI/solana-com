...
Tokenlar gizli olarak transfer edilmeden önce, genel token bakiyesinin gizli
...
Bu bölüm, birinci aşamayı açıklamaktadır: genel token bakiyesinin gizli bekleme
bakiyesine yatırılması.
...
talimatını çağırın. Deposit talimatı başına izin verilen
...
talimatıyla bir işlem oluşturup gönderen `confidential_transfer_deposit`
metodunu sağlar.
...
<Callout type="info">
  ...
  etkin değildir. Örnek kodu çalıştırmak için mainnet Token Extensions Program'ı
  klonlamanız gerekmektedir.
</Callout>
