...
olması gerekir. Gönderenin token account'ının aynı zamanda transfer edilebilecek
mevcut bir gizli bakiyesi olmalıdır.
...
1. İstemci tarafında
   ...
   **Eşitlik Kanıtı
   ...
   Transfer sonrasındaki yeni mevcut bakiye şifreli metninin karşılık gelen
   ...
   **Şifreli Metin Geçerlilik Kanıtı
   ...
   Transfer miktarı şifreli metinlerinin üç taraf (kaynak, hedef ve denetçi)
   için de uygun şekilde oluşturulduğunu doğrular; transfer miktarının her
   tarafın genel anahtarı altında doğru şekilde şifrelendiğini güvence altına
   alır.
   ...
   **Aralık Kanıtı
   ...
   Yeni mevcut bakiyenin ve transfer miktarının (düşük/yüksek bitlere bölünmüş)
   tamamının negatif olmadığını ve belirli bir aralık içinde olduğunu doğrular.

2. Her kanıt için:
   - Kanıt verilerini doğrulamak amacıyla ZK ElGamal kanıt programını çağırın.
...
4. Bunları oluşturmak için kullanılan SOL'u geri kazanmak amacıyla kanıt bağlam
   durumu hesaplarını kapatın.

Aşağıdaki diyagram, token'ların gönderenin token account'ından alıcının token
account'ına aktarılmasında yer alan adımları göstermektedir.
...
- Bir proof account'u kapatan
  `confidential_transfer_close_context_state_account` metodu.

### Örnek Kod

Aşağıdaki örnek, token accounts arasında gizli token transferinin nasıl
...
için Solana CLI'nin kurulu olması gerekmektedir.
...
<Callout type="info">
  ...
  etkin değildir. Örnek kodu çalıştırmak için mainnet Token Extensions
  Program'ını klonlamanız gerekmektedir.
</Callout>
