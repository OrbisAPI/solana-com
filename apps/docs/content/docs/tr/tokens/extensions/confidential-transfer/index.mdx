...
Gizli transferler, transfer miktarını açıklamadan token account'lar arasında
...
2. Gönderici ve alıcı için gizli transfer uzantılı token account'lar oluşturun.
...
5. Göndericinin bekleyen bakiyesini **gizli kullanılabilir bakiyeye**
   **uygulayın**.
6. token'ları gönderici token account'tan alıcı token account'a gizli olarak
   **transfer edin**.
...
<Cards>
  ...
  <Card
    title="İhraççı Kılavuzu"
    href="/docs/tokens/extensions/confidential-transfer/issuer-guide"
  >
    Gizli transfer token'ı nasıl ihraç edilir ve işletilir (onay politikası,
    denetçiler, ücretler, mint ve burn)
  </Card>
</Cards>

Aşağıdaki diyagram, gizli token transferlerinin temel akışına ilişkin ayrıntılı
bir sıralamayı göstermektedir:
...
adımlarını göstermektedir. Adımlar, netlik sağlamak amacıyla yardımcı
fonksiyonlara ayrılmıştır.

Örneği çalıştırmak için, aşağıdaki komutu kullanarak mainnet'ten klonlanan Token
Extensions Program ile birlikte yerel bir validator başlatın. Yerel validator'ı
kullanabilmek için Solana CLI'nin yüklü olması gerekmektedir.
...
<Callout type="info">
  Bu yazının hazırlandığı sırada, Gizli Transferler varsayılan yerel validator
  üzerinde etkinleştirilmemişti. Örnek kodu çalıştırabilmek için mainnet Token
...
| Talimat                             | Açıklama                                                                                                                                                            |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | mint account'u gizli transferler için yapılandırır. Bu talimat, _rs`TokenInstruction::InitializeMint`_ talimatıyla aynı işleme dahil edilmelidir.                   |
| _rs`UpdateMint`_                    | Bir mint için gizli transfer ayarlarını günceller.                                                                                                                  |
| _rs`ConfigureAccount`_              | Bir token account'u gizli transferler için yapılandırır.                                                                                                            |
...
| _rs`EmptyAccount`_                  | Bir token account'ı kapatabilmek için bekleyen ve mevcut gizli bakiyeleri sıfırlar.                                                                                 |
...
| _rs`Transfer`_                      | Token'ları, token account'lar arasında gizli şekilde transfer eder.                                                                                                 |
...
| _rs`TransferWithFee`_               | Token'ları, token account'lar arasında ücret alınarak gizli şekilde transfer eder.                                                                                  |
| _rs`ConfigureAccountWithRegistry`_  | Token account'larını gizli transferler için yapılandırmanın alternatif bir yöntemi; _rs`VerifyPubkeyValidity`_ kanıtı yerine _rs`ElGamalRegistry`_ hesabı kullanır. |
