...
Yerel bir test validator'ı aynı zamanda eski bir Token-2022 derlemesi de içerir;
bu nedenle bu sayfadaki örnekleri çalıştırmak için devnet'ten v11.0.0 programını
test validator'ınıza yükleyin:
...
## İzinli Mint ve Burn Nasıl Oluşturulur

İzinli burn ile mint oluşturmak ve token'ları yakmak için:
...
2. _rs`CreateAccount`_ ile mint account oluşturun, _rs`PermissionedBurnConfig`_
   uzantısını başlatın ve _rs`InitializeMint`_ ile mint'i başlatın.
3. Bir token account oluşturun ve token'ları mint edin.
4. token account sahibi ve burn yetkisi tarafından imzalanan
   _rs`PermissionedBurnInstruction::BurnChecked`_ ile yakın.
5. İsteğe bağlı olarak, _rs`AuthorityType::PermissionedBurn`_ ve `None` yetkisi
   kullanarak _rs`SetAuthority`_ ile izinli yakma özelliğini devre dışı bırakın;
   bu işlem standart burn talimatlarını yeniden etkinleştirir. Aşağıdaki tam kod
   örnekleri bu adımı göstermektedir.

<ScrollyCoding>

## !!steps Hesap boyutunu hesapla

Temel mint için mint account boyutunu ve _rs`PermissionedBurnConfig`_ uzantısını
hesaplayın. Bu boyut, _rs`CreateAccount`_ içinde kullanılır.
...
Mint için gereken boyut ve _rs`PermissionedBurnConfig`_ uzantısı kullanılarak
...
Hesaplanan alan ve lamport miktarıyla mint account oluşturun.
...
Burn yetkisiyle mint üzerindeki _rs`PermissionedBurnConfig`_ uzantısını
başlatın.
...
Ödeyici için bir associated token account oluşturun ve buna token'ları mint
edin.
...
sahibi hem de yakma yetkisi tarafından imzalanarak yakın.
...
| _rs`PermissionedBurnInstruction::BurnChecked`_ | _rs`PermissionedBurnInstruction::Burn`_ ile aynı hesapları kullanan ondalık doğrulamalı yakma talimatı.                                     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Bir mint üzerindeki yakma yetkisini döndürmek veya devre dışı bırakmak için _rs`SetAuthority`_ ile birlikte kullanılan yetki ayrıştırıcısı. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
...
Yayınlanan eski `@solana/spl-token` paketi henüz izinli yakma desteği
içermediğinden, eski bir örnek dahil edilmemiştir.
