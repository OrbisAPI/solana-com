...
<Callout type="info">
  Tarayıcı cüzdanı olmayan bir API rotası, worker veya script içinde başsız
  (headless) çalışıyor musunuz? Ham bir keypair yerine arka uç anahtar yönetim
...
- **Başsız durum makineleri**: İstemciyi API rotaları, script'ler veya
  worker'lar içinde çalıştırarak kullanıcı arayüzünüzü destekleyen aynı cüzdan +
  RPC orkestrasyon mantığını yeniden kullanın.
- **Gerçek zamanlı gösterge panelleri**: İzleyicileri (bakiyeler, hesaplar,
  imzalar) tercih ettiğiniz kullanıcı arayüzü kütüphanesiyle birleştirin;
  istemci websocket yayılımını ve önbellek geçersizleştirmeyi yönetir.
- **Özel store'lar**: Kendi Zustand store'unuzu enjekte ederek
  IndexedDB/localStorage'dan hidrasyonu sağlayın, durumu sunucu oturumlarına
  yansıtın veya tarayıcı sekmeleri arasında koordinasyon kurun.
