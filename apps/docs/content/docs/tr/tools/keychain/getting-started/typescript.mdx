...
### Kit İstemcisi ile Kullanım (keychainSigner eklentisi)
...
Eklenti, `createKeychainSigner` ile aynı backend etiketli yapılandırmayı kabul
eder ve yalnızca yönlendirilen backend paketlenir (backend paketleri dinamik
`import()` aracılığıyla yüklenir).

Yalnızca tek bir rol ayarlamak ve tek bir istemcide farklı backend'leri bir
arada kullanmak için `keychainPayer` veya `keychainIdentity` kullanın:
...
Zaten bir `SolanaSigner`'niz var mı? Her keychain imzalayıcı geçerli bir Kit
`TransactionSigner`'dır, dolayısıyla mevcut bir imzalayıcıyı
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
ile yükleyebilirsiniz (`pnpm add @solana/kit-plugin-signer`):
