...
suojaavissa transaktioissa. Ainoastaan siirron määrät ja token-saldot ovat
...
- [Protokollan yleiskatsaus](https://www.solana-program.com/docs/confidential-balances/overview) -
  Tiedot taustalla olevasta kryptografisesta protokollasta
...
välillä paljastamatta siirron määrää.
...
jotka vastaavat edellistä kaaviota. Vaiheet on eroteltu selkeyden vuoksi
apufunktioihin.

Suorittaaksesi esimerkin, käynnistä paikallinen validator Token Extension
Program kloonattuna mainnetistä seuraavalla komennolla. Sinulla on oltava Solana
CLI asennettuna paikallisen validatorin käyttämiseksi.
...
<Callout type="info">
  ...
  validatorissa. Sinun täytyy kloonata mainnet Token Extension Program
...
| _rs`ApproveAccount`_                | Hyväksyy token accountin luottamuksellisia siirtoja varten, jos mint edellyttää hyväksyntää uusille token accounteille.                                                  |
| _rs`EmptyAccount`_                  | Tyhjentää odottavan ja käytettävissä olevan luottamuksellisen saldon token accountin sulkemisen mahdollistamiseksi.                                                      |
...
| _rs`DisableNonConfidentialCredits`_ | Estää tavalliset siirrot, jolloin tili vastaanottaa vain luottamuksellisia siirtoja.                                                                                     |
| _rs`TransferWithFee`_               | Siirtää tokeneita token accountien välillä luottamuksellisesti maksun kanssa.                                                                                            |
| _rs`ConfigureAccountWithRegistry`_  | Vaihtoehtoinen tapa määrittää token accountit luottamuksellisia siirtoja varten käyttämällä _rs`ElGamalRegistry`_ -tiliä _rs`VerifyPubkeyValidity`_ -todistuksen sijaan. |
