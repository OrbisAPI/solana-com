...
2. Kutakin todistetta varten:
   - Käynnistä ZK ElGamal -todistusohjelma todisteen datan vahvistamiseksi.
...
3. Käynnistä
   ...
   -ohje antamalla kaksi todistustiliä.

4. Sulje todistustilit palauttaaksesi niiden luomiseen käytetyn SOL:n.

Seuraava kaavio näyttää luottamuksellisesta käytettävissä olevasta saldosta
julkiseen saldoon nostamiseen liittyvät vaiheet:
...
- Luo yhtäläisyystodiste ja aluetodiste asiakaspuolella
- Käynnistä Zk ElGamal -todistusohjelma todisteiden vahvistamiseksi ja
  "kontekstitila"-tilien alustamiseksi
- Käynnistä
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  -ohje antamalla kaksi todistustiliä.
- Sulje kaksi todistustiliä palauttaaksesi rent.

`spl_token_client` -paketti tarjoaa seuraavat menetelmät:

- `confidential_transfer_create_context_state_account` -menetelmä, joka luo
  todistustilin.
- `confidential_transfer_withdraw` -menetelmä, joka käynnistää `Withdraw`
  -ohjeen.
- `confidential_transfer_close_context_state_account` -menetelmä, joka sulkee
  todistustilin.
...
Program kloonattuna mainnetistä käyttämällä seuraavaa komentoa. Sinulla täytyy
olla Solana CLI asennettuna käynnistääksesi paikallisen validatorin.
