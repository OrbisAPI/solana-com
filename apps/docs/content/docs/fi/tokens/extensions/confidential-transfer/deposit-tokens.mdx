...
yhtä talletusohjausta kohti on rajoitettu arvoon 2^48.

`spl_token_client` -paketti tarjoaa `confidential_transfer_deposit` -metodin,
joka rakentaa ja lähettää transaktion `Deposit` -ohjeella, kuten alla olevassa
esimerkissä on osoitettu.
...
Suorittaaksesi esimerkin, käynnistä paikallinen validator, johon on kloonattu
Token Extensions Program mainnettistä käyttämällä seuraavaa komentoa. Sinulla
täytyy olla Solana CLI asennettuna käynnistääksesi paikallisen validatorin.
...
<Callout type="info">
  ...
  validatorissa. Sinun on kloonattava mainnet Token Extensions Program
