...
<Callout type="info">
  Käynnistetäänkö headless-tilassa — API-reitissä, workerissa tai skriptissä
  ilman selainlompakkoa? Käytä allekirjoitukseen taustapalvelun avainten
  hallintapalvelua raa'an keypair:n sijaan. Katso
  [Keychain](/docs/tools/keychain) ja [Allekirjoittaminen
  tuotannossa](/docs/core/transactions/signing-in-production).
</Callout>

## Yleisiä malleja Solana-kehittäjille

- **Headless-tilakoneet**: Aja asiakasta API-reiteissä, skripteissä tai
  workereissa käyttääksesi samaa lompakko- ja RPC-orkestrointi­logiikkaa, joka
  toimii käyttöliittymäsi pohjana.
- **Reaaliaikaiset koontinäytöt**: Yhdistä tarkkailijat (saldot, tilit,
  allekirjoitukset) haluamaasi käyttöliittymäkirjastoon; asiakas hoitaa
  websocket-fanijakelun ja välimuistin mitätöinnin.
- **Mukautetut storet**: Injektoi oma Zustand-storesi hydratoidaksesi
  IndexedDB:stä/localStoragesta, peilataksesi tilaa palvelinsessioihin tai
  koordinoidaksesi selaimen välilehtien välillä.
- **Silta React-hookeille**: Välitä konfiguroitu asiakas­instanssi
  `@solana/react-hooks`:lle, kun haluat ergonomiset hookit saman ajoympäristön
  päälle.
- **Testattavuus**: Yksittäistä asiakasrajapintaa voidaan simuloida
  yksikkötesteissä, mikä helpottaa RPC-vastausten tai lompakko­sessioiden
  simulointia ilman selainlompakkoa.
