...
### Kit-asiakkaan käyttäminen (keychainSigner-laajennus)

`@solana/keychain-kit-plugin` asentaa keychain-allekirjoittajan suoraan
[Kit-asiakkaaseen](https://github.com/anza-xyz/kit) ja kytkee sen `payer`- ja
`identity`-rooleihin, jolloin yllä esitetty manuaalinen `pipe`-määritys jää
tarpeettomaksi:
...
Laajennus hyväksyy saman taustapalvelukohtaisen konfiguraation kuin
`createKeychainSigner`, ja vain se taustapalvelu, jolle pyyntö ohjataan,
sisällytetään pakettiin (taustapalvelupaketit ladataan dynaamisella
`import()`-kutsulla).

Käytä `keychainPayer`- tai `keychainIdentity`-kutsua asettaaksesi vain yhden
roolin ja yhdistelläksesi taustapalveluita yhdellä asiakkaalla:
...
Onko sinulla jo `SolanaSigner`? Jokainen keychain-allekirjoittaja on kelvollinen
Kit-`TransactionSigner`, joten voit asentaa olemassa olevan allekirjoittajan
...
lisätäksesi muita avainten hallintapalveluita.
