...
يتضمن validator المحلي للاختبار أيضًا إصدارًا قديمًا من Token-2022، لذا لتشغيل
الأمثلة الواردة في هذه الصفحة، قم بتحميل برنامج الإصدار v11.0.0 من devnet إلى
validator الاختبار الخاص بك:
...
## كيفية إنشاء Mint وحرق بصلاحيات

لإنشاء mint مع حرق مقيّد بصلاحيات وحرق الرموز:

1. احسب حجم mint account والـ rent المطلوب لـ mint وامتداد
   _rs`PermissionedBurnConfig`_.
2. أنشئ mint account باستخدام _rs`CreateAccount`_، وقم بتهيئة
   _rs`PermissionedBurnConfig`_، ثم هيّئ mint باستخدام _rs`InitializeMint`_.
3. أنشئ token account واسكّ الرموز.
4. قم بالحرق باستخدام _rs`PermissionedBurnInstruction::BurnChecked`_ موقّعًا من
   كلٍّ من مالك token account وسلطة الحرق.
5. يمكنك اختياريًا تعطيل الحرق المقيّد بالصلاحيات باستخدام _rs`SetAuthority`_
   عبر _rs`AuthorityType::PermissionedBurn`_ وسلطة `None`، مما يُعيد تفعيل
   تعليمات الحرق القياسية. تُوضح أمثلة الكود الكاملة أدناه هذه الخطوة.

<ScrollyCoding>

## !!steps حساب حجم الحساب

احسب حجم mint account للـ mint الأساسي بالإضافة إلى امتداد
...
احسب الـ rent باستخدام الحجم المطلوب لـ mint بالإضافة إلى امتداد
_rs`PermissionedBurnConfig`_.
...
أنشئ mint account بالمساحة والـ lamports المحسوبة.
...
قم بتهيئة امتداد _rs`PermissionedBurnConfig`_ على الـ mint مع سلطة الحرق.
...
قم بتهيئة الـ mint باستخدام _rs`InitializeMint`_ في نفس المعاملة.
...
| العنصر                                         | الوصف                                                                                                            | المصدر                                                                                                                                         |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | امتداد الـ Mint الذي يخزن الصلاحية المطلوبة للتوقيع المشترك على كل عملية حرق للـ mint.                           | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | تعليمة تُهيئ إعدادات الحرق المقيَّد قبل _rs`InitializeMint`_.                                                    | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | تعليمة حرق تستلزم توقيعات من صلاحية الحرق ومالك token account أو المفوَّض.                                       | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | تعليمة حرق مع التحقق من الكسور العشرية تستخدم نفس الحسابات المستخدمة في _rs`PermissionedBurnInstruction::Burn`_. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
...
| _rs`process_initialize`_                       | منطق المعالج الذي يُهيئ _rs`PermissionedBurnConfig`_ على mint غير مُهيَّأ ويخزن صلاحية الحرق.                    | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | منطق المعالج الذي يرفض عمليات الحرق العادية عند تعيين صلاحية الحرق ويتحقق من توقيع الصلاحية.                     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | منطق المعالج الذي يتحقق من صلاحية الحرق ويدوِّرها عند استخدام _rs`AuthorityType::PermissionedBurn`_.             | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### TypeScript

يستخدم المثال `Kit` أدناه التعليمات المُنشأة مباشرةً. لا تتضمن حزمة
`@solana/spl-token` القديمة المنشورة دعمَ الحرق المقيّد بالصلاحيات بعد، لذا لا
يوجد مثال قديم مُدرج.
