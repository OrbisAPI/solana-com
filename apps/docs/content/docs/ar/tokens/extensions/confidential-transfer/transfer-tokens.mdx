...
## كيفية نقل الرموز المميزة بشكل سري من token account إلى آخر

لنقل الرموز المميزة بشكل سري من token account إلى آخر، يجب على كلٍّ من المُرسِل
والمُستقبِل امتلاك token accounts مُهيَّأة بحالة
_rs`ConfidentialTransferAccount`_ ومُعتمَدة لعمليات النقل السري. كما يجب أن
يمتلك token account الخاص بالمُرسِل رصيداً سرياً متاحاً للنقل.

لنقل الرموز المميزة بشكل سري:
...
1. أنشئ
   ...
   **إثبات المساواة
   ...
   يتحقق من أن نص التشفير الخاص بالرصيد المتاح الجديد بعد عملية النقل يطابق
   [التزام Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل
   له، مما يضمن احتساب الرصيد المتاح الجديد للحساب المصدر بشكل صحيح على هيئة
   ...
   **إثبات صحة نص التشفير
   ...
   يتحقق من أن نصوص تشفير مبلغ النقل قد تم توليدها بشكل صحيح لجميع الأطراف
   الثلاثة (المصدر، والوجهة، والمدقق)، مما يضمن تشفير مبلغ النقل بشكل صحيح
   باستخدام المفتاح العام لكل طرف.
   ...
   **إثبات النطاق
   ...
   يتحقق من أن الرصيد المتاح الجديد ومبلغ النقل (المقسَّم إلى بتات منخفضة/عالية)
   كلاهما غير سالب وضمن نطاق محدد.

2. لكل إثبات:
   - استدعِ برنامج إثبات ZK ElGamal للتحقق من بيانات الإثبات.
   - خزِّن البيانات الوصفية الخاصة بالإثبات في حساب "حالة السياق" لاستخدامها في
     التعليمات الأخرى.
...
يوضح المخطط التالي الخطوات المتبعة في نقل الرموز المميزة من token account الخاص
بالمُرسِل إلى token account الخاص بالمُستقبِل.
...
لنقل الرموز المميزة بشكل سري من token account إلى آخر، يجب عليك:

- توليد إثبات مساواة، وإثبات صحة نص تشفير، وإثبات نطاق على جانب العميل
- استدعاء برنامج Zk ElGamal للتحقق من الإثباتات وتهيئة حسابات "حالة السياق"
- استدعاء تعليمة
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  مع توفير حسابات الإثباتات الثلاثة.
- إغلاق حسابات الإثباتات الثلاثة لاسترداد rent.

يوفر الـ `spl_token_client` الطرق التالية:
...
يوضح المثال التالي كيفية نقل الرموز بشكل سري بين token account.

لتشغيل المثال، ابدأ تشغيل validator محلي مع Token Extensions Program المستنسخ من
...
<Callout type="info">
  في وقت كتابة هذا المحتوى، لم يتم تفعيل ميزة التحويلات السرية في الـ validator
  المحلي الافتراضي. يجب عليك استنساخ Token Extensions Program من الشبكة الرئيسية
  لتشغيل الكود المثال.
</Callout>
