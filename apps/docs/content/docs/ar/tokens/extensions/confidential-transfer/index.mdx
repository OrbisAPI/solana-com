...
التحويل. وهذا مفيد للمعاملات التي تحافظ على الخصوصية. يظل مبلغ التحويل ورصيد
الرموز المميزة فقط طيَّ الكتمان، في حين تبقى عناوين token accounts متاحة للعموم.
...
تسير عملية التحويلات السرية للرموز المميزة وفق الخطوات التالية:

1. إنشاء mint account مع امتداد التحويل السري.
2. إنشاء token accounts مع امتداد التحويل السري للمُرسِل والمستلم.
3. سك الرموز المميزة في حساب المُرسِل.
4. **إيداع** الرصيد العام للمُرسِل في **الرصيد المعلَّق السري**.
5. **تطبيق** الرصيد المعلَّق للمُرسِل على **الرصيد المتاح السري**.
6. **تحويل** الرموز المميزة سريًا من token account المُرسِل إلى token account
   المستلم.
7. **تطبيق** الرصيد المعلَّق للمستلم على **الرصيد المتاح السري**.
8. **سحب** الرصيد المتاح السري للمستلم إلى **الرصيد العام**.

لمزيد من التفاصيل حول خطوات سير عملية التحويل السري، راجع الصفحات المقابلة:
...
<Cards>
  ...
  >
    كيفية إيداع الرموز المميزة في الرصيد المعلَّق السري
  </Card>
  <Card
    title="تطبيق الرصيد المعلَّق"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    كيفية تطبيق الرصيد المعلَّق على الرصيد السري المتاح
  </Card>
  <Card
  ...
  >
    كيفية تحويل الرموز المميزة سريًا بين token accounts
  </Card>
  <Card
    title="دليل التكامل"
    href="/docs/tokens/extensions/confidential-transfer/integration-guide"
  >
    كيف يمكن للمحافظ الرقمية والمستكشفات والبورصات دعم رموز التحويل السري
    المميزة
  </Card>
  <Card
    title="دليل المُصدِر"
  ...
  >
    كيفية إصدار وإدارة رمز التحويل السري المميز (سياسة الموافقة، والمدققون،
    والرسوم، والسك والحرق)
  </Card>
</Cards>

يوضح الرسم البياني أدناه تسلسلاً تفصيلياً للتدفق الأساسي لعمليات نقل الرموز
السرية:
...
يوضح مثال Rust أدناه خطوات عمليات نقل الرموز السرية المقابلة للرسم البياني
السابق. تم تقسيم الخطوات إلى دوال مساعدة لتوضيح المحتوى.

لتشغيل المثال، ابدأ تشغيل validator محلي مع Token Extensions Program المنسوخ من
الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك سولانا CLI مثبتاً
لاستخدام validator المحلي.
...
<Callout type="info">
  في وقت كتابة هذا المحتوى، لم يتم تفعيل ميزة النقل السري في validator المحلي
  الافتراضي. يجب عليك نسخ Token Extensions Program من الشبكة الرئيسية لتشغيل
  الكود المثال.
</Callout>
...
| _rs`EmptyAccount`_                  | يُفرغ الأرصدة السرية المعلقة والمتاحة للسماح بإغلاق token account.                                                                   |
| _rs`Deposit`_                       | يحوّل الرصيد العلني للرموز إلى رصيد سري معلق.                                                                                        |
| _rs`Withdraw`_                      | يحوّل الرصيد السري المتاح إلى رصيد علني.                                                                                             |
| _rs`Transfer`_                      | ينقل الرموز بين token accounts بشكل سري.                                                                                             |
| _rs`ApplyPendingBalance`_           | يحوّل الرصيد المعلق إلى رصيد متاح بعد عمليات الإيداع أو النقل.                                                                       |
| _rs`EnableConfidentialCredits`_     | يسمح لـ token account باستقبال عمليات نقل الرموز السرية.                                                                             |
| _rs`DisableConfidentialCredits`_    | يحجب عمليات النقل السري الواردة مع السماح بالنقل العلني.                                                                             |
| _rs`EnableNonConfidentialCredits`_  | يسمح لـ token account باستقبال عمليات نقل الرموز العلنية.                                                                            |
| _rs`DisableNonConfidentialCredits`_ | يحجب عمليات النقل العادية لجعل الحساب يستقبل النقل السري فقط.                                                                        |
