...
## كيفية سحب الرموز المميزة من الرصيد المتاح السري

لسحب الرموز المميزة من الرصيد المتاح السري إلى الرصيد العام:

1. أنشئ
   [دليلَين](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   على جانب العميل:
   ...
   **دليل المساواة
   ...
   يتحقق من أن نص الرصيد المتاح المتبقي المشفر بعد عملية السحب يتطابق مع
   [التزام Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل
   له، مما يضمن أن الرصيد المتاح الجديد للحساب محسوب بشكل صحيح على أنه
   ...
   **دليل النطاق
   ...
   يتحقق من أن الرصيد المتاح المتبقي بعد السحب غير سلبي وضمن نطاق محدد.

2. لكل دليل:
   - استدعِ برنامج دليل ZK ElGamal للتحقق من بيانات الدليل.
   - احفظ البيانات الوصفية الخاصة بالدليل في حساب "حالة السياق" للدليل لاستخدامه
     في التعليمات الأخرى.
...
3. استدعِ تعليمة
   ...
   مع تزويدها بحسابَي الدليلين.

4. أغلق حسابات الدليل لاسترداد SOL المستخدمة في إنشائها.

يوضح المخطط التالي الخطوات المتبعة في سحب الرموز المميزة من الرصيد المتاح السري
إلى الرصيد العام:
...
لسحب الرموز المميزة من الرصيد المتاح السري إلى الرصيد العام، يجب عليك:

- إنشاء دليل مساواة ودليل نطاق على جانب العميل
- استدعاء برنامج دليل Zk ElGamal للتحقق من الأدلة وتهيئة حسابات "حالة السياق"
- استدعاء تعليمة
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  مع تزويدها بحسابَي الدليلين.
- إغلاق حسابَي الدليلين لاسترداد rent.

تُوفر حزمة `spl_token_client` الأساليب التالية:

- الأسلوب `confidential_transfer_create_context_state_account` الذي ينشئ حساب
  دليل.
- الأسلوب `confidential_transfer_withdraw` الذي يستدعي تعليمة `Withdraw`.
- الأسلوب `confidential_transfer_close_context_state_account` الذي يغلق حساب
  دليل.

### مثال على الكود

يوضح المثال التالي كيفية سحب الرموز المميزة من الرصيد المتاح السري إلى الرصيد
العام.

لتشغيل المثال، ابدأ تشغيل validator محلي مع Token Extensions Program مستنسخًا من
الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك سولانا CLI مثبتًا لبدء
...
<Callout type="info">
  في وقت كتابة هذا المقال، لم يتم تفعيل التحويلات السرية على validator المحلي
  الافتراضي. يجب عليك استنساخ Token Extensions Program الخاصة بالشبكة الرئيسية
  لتشغيل كود المثال.
</Callout>
