...
### الاستخدام مع Kit Client (إضافة keychainSigner)

تقوم `@solana/keychain-kit-plugin` بتثبيت موقّع سلسلة المفاتيح مباشرةً على
[Kit client](https://github.com/anza-xyz/kit)، وربطه كـ `payer` و `identity`،
مما يُغنيك عن إعداد `pipe` اليدوي الموضح أعلاه:
...
تقبل الإضافة نفس الإعداد المُصنَّف بالواجهة الخلفية كـ `createKeychainSigner`،
ويتم تجميع الواجهة الخلفية التي تُوجَّه إليها فقط (تُحمَّل حزم الواجهة الخلفية
عبر `import()` الديناميكي).

استخدم `keychainPayer` أو `keychainIdentity` لتعيين دور واحد فقط، ودمج الواجهات
الخلفية على عميل واحد:
...
bدلاً من ذلك (`pnpm add @solana/kit-plugin-signer`):
...
تمتد واجهة `SolanaSigner` لتشمل أنواع `@solana/signers` لتحقيق توافق كامل:
...
قم بتطبيق واجهة `SolanaSigner` لإنشاء موقّعين مخصصين:
