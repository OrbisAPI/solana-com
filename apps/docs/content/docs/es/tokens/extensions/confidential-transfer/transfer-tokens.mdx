...
el remitente como el destinatario deben tener token accounts configurados con el
estado _rs`ConfidentialTransferAccount`_ y aprobados para transferencias
confidenciales. El token account del remitente también debe tener un saldo
confidencial disponible desde el cual realizar la transferencia.
...
1. Crear
   ...
   **Prueba de Validez del Texto Cifrado
   ...
   correctamente para las tres partes involucradas (origen, destino y auditor),
   garantizando que el monto de la transferencia esté correctamente cifrado con
   la clave pública de cada parte.
...
2. Para cada prueba:
   - Invocar el programa de pruebas ZK ElGamal para verificar los datos de la
     prueba.
...
3. Invocar la instrucción
   ...
   proporcionando las cuentas de estado de contexto de las pruebas.

4. Cerrar las cuentas de estado de contexto de las pruebas para recuperar el SOL
...
- Generar una prueba de igualdad, una prueba de validez del texto cifrado y una
  prueba de rango del lado del cliente
...
- El método `confidential_transfer_create_context_state_account` que crea una
  cuenta de prueba.
- El método `confidential_transfer_transfer` que invoca la instrucción
  `Transfer`.
- El método `confidential_transfer_close_context_state_account` que cierra una
  cuenta de prueba.

### Código de ejemplo

El siguiente ejemplo demuestra cómo transferir tokens de forma confidencial
entre token accounts.
...
Solana instalada para iniciar el validator local.
