...
2. Para cada prueba:
   ...
   - Almacena los metadatos específicos de la prueba en una cuenta de "estado de
     contexto" de prueba para utilizarla en otras instrucciones.
...
El siguiente diagrama muestra los pasos para retirar tokens del saldo disponible
...
- El método `confidential_transfer_create_context_state_account` crea una cuenta
  de prueba.
- El método `confidential_transfer_withdraw` invoca la instrucción `Withdraw`.
- El método `confidential_transfer_close_context_state_account` cierra una
  cuenta de prueba.

### Código de Ejemplo

El siguiente ejemplo muestra cómo retirar tokens del saldo disponible
confidencial al saldo público.
