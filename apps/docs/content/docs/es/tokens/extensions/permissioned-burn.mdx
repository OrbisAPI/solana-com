...
Para crear un mint con quema autorizada y quemar tokens:
...
5. Opcionalmente, deshabilita la quema autorizada con _rs`SetAuthority`_ usando
   _rs`AuthorityType::PermissionedBurn`_ y una autoridad `None`, lo que reactiva
   las instrucciones de quema estándar. Los ejemplos de código completos a
   continuación muestran este paso.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Extensión de mint que almacena la autoridad requerida para cofirmar cada quema del mint.                                             | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
...
El ejemplo de `Kit` que se muestra a continuación utiliza las instrucciones
generadas directamente. El paquete `@solana/spl-token` heredado publicado aún no
incluye compatibilidad con quema con permisos, por lo que no se incluye ningún
ejemplo heredado.
