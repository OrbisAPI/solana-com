...
directamente en un [cliente Kit](https://github.com/anza-xyz/kit),
configurándolo como `payer` y `identity` para que omitas la configuración manual
de `pipe` mostrada anteriormente:
...
Usa `keychainPayer` o `keychainIdentity` para definir un único rol y combinar
...
¿Ya tienes un `SolanaSigner`? Cada firmante de cadena de claves es un Kit
`TransactionSigner` válido, por lo que puedes instalar uno existente con
