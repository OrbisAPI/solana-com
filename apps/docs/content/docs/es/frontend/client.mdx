...
- **Máquinas de estado headless**: Ejecuta el cliente dentro de rutas de API,
  ...
  billetera + RPC que impulsa tu UI.
- **Dashboards en tiempo real**: Combina observadores (saldos, cuentas, firmas)
  con tu biblioteca de UI preferida; el cliente gestiona la distribución de
  websockets y la invalidación de caché.
...
- **Testabilidad**: La interfaz de cliente único puede ser simulada en pruebas
  unitarias, lo que facilita simular respuestas RPC o sesiones de billetera sin
  una billetera de navegador presente.
