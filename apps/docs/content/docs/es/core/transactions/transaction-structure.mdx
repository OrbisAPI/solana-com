...
La primera firma del array pertenece al **pagador de comisiones**, la cuenta que
paga la
...
- Debe ser una cuenta de propiedad del System Program o una cuenta nonce
  (validada por
...
- [`recent_blockhash`](mention:recent-blockhash): Un
  [blockhash](#recent-blockhash) que actúa como marca de tiempo de la
  transacción
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Número
  de cuentas firmantes que son de solo lectura.
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  Número de cuentas no firmantes que son de solo lectura.
...
transacción. El array debe incluir cada cuenta y debe seguir este orden
estricto:

1. Firmante + Con permiso de escritura
2. Firmante + Solo lectura
3. No firmante + Con permiso de escritura
4. No firmante + Solo lectura

<Callout>
  Este orden estricto permite que el array `account_keys` se combine con los
  tres contadores del [`header`](#header) del mensaje para determinar los
  permisos de cada cuenta sin necesidad de almacenar indicadores de metadatos
  por cuenta. Los contadores del encabezado dividen el array en los cuatro
  grupos de permisos listados anteriormente.
</Callout>
...
llevan como prefijo una codificación de longitud compact-u16. Este formato usa 1
byte para valores del 0 al 127 y 2-3 bytes para valores mayores.
...
que el System Program deduzca lamports. Tanto la cuenta del remitente como la
del destinatario deben ser modificables para que su saldo en lamports pueda
cambiar. Para ejecutar esta instrucción, la billetera del remitente envía la
transacción con su firma y el mensaje que contiene la instrucción de
...
puede retirar el SOL. Los lamports solo pueden ser retirados por el programa
...
Las transferencias de **tokens** SPL tienen cierta protección inherente: el
...
pero observe que cada instrucción contiene la misma información requerida.
...
Debido a que una transferencia de SOL se realiza correctamente en cualquier
cuenta, verifique el destinatario antes de firmar. Obtenga la cuenta y envíe
únicamente a una billetera de System Program (o una dirección
...
<Callout>
  Este fragmento verifica destinatarios de SOL nativo. Para la clasificación
