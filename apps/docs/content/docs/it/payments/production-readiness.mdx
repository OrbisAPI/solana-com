...
utilizzato per stimare le unità di calcolo richieste da una transazione, inclusa
una stima delle unità di calcolo che verranno utilizzate.
[`@solana/kit`](https://github.com/anza-xyz/kit) fornisce funzioni di supporto
che stimano i limiti di risorse di una transazione e li impostano sul messaggio
...
<Callout type="info">
  Se crei e invii transazioni con un client plugin kit, in genere non hai
  bisogno di questo passaggio — il client aggiunge automaticamente le istruzioni
  del budget di calcolo al momento dell'invio (`.sendTransaction()`). Il flusso
  manuale descritto sopra è per quando assembli le transazioni direttamente con
  `@solana/kit`.
</Callout>
...
transazione, così da evitare il sovraccarico derivante dalla stima delle unità
di calcolo ogni volta.

### Bundle Jito

I bundle Jito sono uno strumento per gestire l'esecuzione atomica di più
transazioni. Ciò si ottiene inviando più transazioni alla rete Jito con una
...
Le transazioni possono fallire per molti motivi. A differenza delle tradizionali
API di pagamento che restituiscono immediatamente un esito positivo o negativo,
le transazioni blockchain richiedono il monitoraggio della conferma.

**Concetti chiave:**

- **Scadenza del blockhash**: Le transazioni sono valide per circa 150 blocchi
  (~60-90 secondi)
- **Idempotenza**: La stessa transazione firmata produce sempre la stessa firma
  — la reinvio è sicuro
- **Backoff esponenziale**: Evita di sovraccaricare la rete con tentativi
  ripetuti in rapida successione
...
- [Helius: Come Completare Transazioni su Solana](https://www.helius.dev/blog/how-to-land-transactions-on-solana)
...
| `confirmed` | Supermajority votata            | Fondi compensati          | **La maggior parte dei pagamenti** |
...
- **Aggiornamenti UI**: Mostra `processed` per un feedback immediato ("Pagamento
  inviato")
- **Accredito account utente**: Attendi `confirmed` (sicuro per la maggior parte
  delle transazioni)
- **Spedizione beni fisici**: Attendi `finalized`
- **Prelievi di grandi importi**: Attendi `finalized`
...
di errore specifici anziché la corrispondenza di stringhe:
...
| `INSUFFICIENT_FUNDS_FOR_FEE` | SOL insufficiente      | Finanzia il pagante delle commissioni o usa [l'astrazione delle commissioni](/docs/payments/send-payments/payment-processing/fee-abstraction) |
| `INSUFFICIENT_FUNDS`         | Token insufficienti    | L'utente necessita di un saldo maggiore                                                                                                       |
| `ACCOUNT_NOT_FOUND`          | token account mancante | Creare ATA nella transazione                                                                                                                  |
...
modo simile a come gli utenti di Venmo non si preoccupano delle commissioni ACH.
Consulta
...
- **Non esporre mai le chiavi private nel codice frontend.** Utilizza la firma
  lato backend, portafogli hardware, portafogli multifirma o servizi di gestione
  delle chiavi.
- **Separa i portafogli hot e cold.** Il portafoglio hot per le operazioni,
  quello cold per la tesoreria.
- **Esegui il backup di tutte le chiavi di produzione.** Archivia backup cifrati
  in più posizioni sicure. Perdere una chiave significa perdere l'accesso in
  modo permanente.
- **Usa chiavi diverse per devnet e mainnet.** Le chiavi devnet non devono
  coincidere con quelle mainnet. Utilizza una configurazione basata
  sull'ambiente per assicurarti che vengano caricate le chiavi corrette per
  ciascuna rete.

### Infrastruttura di firma

Per la firma backend in produzione, utilizza
[Keychain](/docs/tools/keychain)—una libreria di firma unificata che astrae
molteplici backend di gestione delle chiavi tramite un'unica interfaccia:
Memory, Vault, Privy, Turnkey, AWS KMS, Fireblocks, GCP KMS, CDP, Para, Dfns,
Crossmint, Openfort e Utila. Questo ti consente di sviluppare in locale con
