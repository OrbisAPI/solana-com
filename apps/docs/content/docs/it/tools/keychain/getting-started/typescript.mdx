...
`identity` così da saltare la configurazione manuale di `pipe` mostrata sopra:
...
`createKeychainSigner`, e viene incluso nel bundle solo il backend a cui viene
inoltrata la richiesta (i pacchetti backend vengono caricati tramite `import()`
dinamico).
...
invece (`pnpm add @solana/kit-plugin-signer`):
