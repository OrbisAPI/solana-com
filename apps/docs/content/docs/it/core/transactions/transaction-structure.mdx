...
Ogni firma viene prodotta da una chiave privata. Il luogo in cui risiede quella
chiave — un keypair locale, un HSM o KMS cloud, oppure un servizio di wallet
gestito — è una decisione di progettazione per la produzione. Vedere
...
denominato **firma della transazione**.
...
- Deve possedere lamport sufficienti a coprire
...
<Callout>
  ...
  permessi di ciascun account senza memorizzare flag di metadati per ogni
...
Il campo `recent_blockhash` è un hash di 32 byte che ha due scopi:
...
[transazione nonce durevole](/docs/core/transactions/durable-nonces).
...
formato utilizza 1 byte per i valori 0-127 e 2-3 byte per i valori più grandi.
...
account. Non esiste alcun controllo a livello di protocollo che verifichi che il
destinatario possa spostare SOL all'esterno. I lamport possono essere spostati
fuori solo dal programma proprietario dell'account, quindi inviare SOL a un
...
trasferimenti nativi di **SOL** non dispongono di tale protezione, quindi il
...
L'esempio seguente mostra il codice relativo ai diagrammi sopra riportati.
Consulta la
