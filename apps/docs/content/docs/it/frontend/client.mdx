...
<Callout type="info">
  Esecuzione headless — in una route API, worker o script senza wallet browser?
  Firma con un servizio di gestione delle chiavi backend invece di un keypair
  grezzo. Vedi [Keychain](/docs/tools/keychain) e [Firma in
...
- **Bridge verso i hook React**: Passa un'istanza del client configurata a
