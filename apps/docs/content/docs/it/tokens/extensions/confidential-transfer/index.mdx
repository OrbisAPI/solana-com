...
senza rivelare l'importo del trasferimento. Ciò è utile per transazioni che
...
3. Esegui il mint dei token sull'account del mittente.
4. **Deposita** il saldo pubblico del mittente nel **saldo in sospeso
   riservato**.
5. **Applica** il saldo in sospeso del mittente al **saldo disponibile
   riservato**.
6. **Trasferisci** in modo riservato i token dal token account del mittente al
   token account del destinatario.
7. **Applica** il saldo in sospeso del destinatario al **saldo disponibile
   riservato**.
...
<Cards>
  ...
  >
    Come applicare il saldo in sospeso al saldo disponibile riservato
  </Card>
  <Card
    title="Preleva Token"
  ...
  >
    Come portafogli, explorer e exchange possono supportare i token con
    trasferimento riservato
  </Card>
  <Card
    title="Guida per gli Emittenti"
  ...
  >
    ...
    approvazione, revisori, commissioni, mint e burn)
  </Card>
</Cards>
...
L'esempio Rust seguente mostra i passaggi per i trasferimenti di token riservati
corrispondenti al diagramma precedente. I passaggi sono suddivisi in funzioni
auxiliarie per maggiore chiarezza.

Per eseguire l'esempio, avvia un validator locale con il Token Extensions
Program clonato dalla mainnet usando il comando seguente. È necessario avere la
Solana CLI installata per utilizzare il validator locale.
...
| _rs`UpdateMint`_                    | Aggiorna le impostazioni di trasferimento riservato per un mint.                                                                                                                |
...
| _rs`Withdraw`_                      | Converte il saldo riservato disponibile di nuovo in saldo pubblico.                                                                                                             |
...
| _rs`DisableNonConfidentialCredits`_ | Blocca i trasferimenti regolari per fare in modo che l'account riceva solo trasferimenti riservati.                                                                             |
