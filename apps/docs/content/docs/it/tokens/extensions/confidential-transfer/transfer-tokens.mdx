...
1. Creare
   ...
   **Ciphertext Validity Proof
   ...
   Verifica che i testi cifrati dell'importo del trasferimento siano
   correttamente generati per tutte e tre le parti (sorgente, destinazione e
   revisore), garantendo che l'importo del trasferimento sia correttamente
   cifrato con la chiave pubblica di ciascuna parte.
   ...
   **Range Proof
   ...
   (suddiviso in bit bassi/alti) siano tutti non negativi e rientranti in un
   intervallo specificato.

2. Per ciascuna prova:
   - Invocare il programma di prova ZK ElGamal per verificare i dati della
     prova.
...
4. Chiudere gli account di stato del contesto della prova per recuperare il SOL
   utilizzato per crearli.

Il diagramma seguente mostra i passaggi necessari per trasferire token dal token
account del mittente al token account del destinatario.
...
- Generare una equality proof, una ciphertext validity proof e una range proof
  lato client
...
L'esempio seguente mostra come trasferire token in modo riservato tra token
account.
...
<Callout type="info">
  ...
  dalla mainnet per eseguire il codice di esempio.
</Callout>
