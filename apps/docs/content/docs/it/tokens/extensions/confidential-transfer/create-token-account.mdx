...
aggiungendo stato extra al token account. Questa sezione spiega come creare un
...
- **approved**: Lo stato di approvazione dell'account per i trasferimenti
  ...
  impostata su `true`, tutti i token account vengono approvati automaticamente
...
- **pending_balance_lo**: I 16 bit inferiori cifrati del saldo in sospeso. Il
  saldo è suddiviso in parte alta e parte bassa per una decrittazione
  efficiente.
...
- **maximum_pending_balance_credit_counter**: Il limite massimo di crediti in
  sospeso prima di richiedere un'istruzione _rs`ApplyPendingBalance`_ per
...
ripetutamente token a un token account, impedendo al proprietario del token
account di trasferire token. Il proprietario del token account non sarebbe in
grado di trasferire token perché il saldo crittografato cambierebbe tra il
momento in cui la transazione viene inviata e quello in cui viene elaborata,
causando il fallimento della transazione.
...
`pending_balance_hi` perché la decrittografia ElGamal richiede più elaborazione
per numeri più grandi. L'implementazione dell'aritmetica del testo cifrato è
disponibile
...
1. Il client recupera i saldi in sospeso e disponibili correnti, cifra la somma
   e fornisce un `decryptable_available_balance` crittografato utilizzando la
   chiave AES del proprietario del token account.

2. I contatori di credito in sospeso attesi ed effettivi tracciano le modifiche
...
Contatori attesi/effettivi corrispondenti indicano che
`decryptable_available_balance` corrisponde a `available_balance`.

Quando si recupera lo stato di un token account per leggere
`decryptable_available_balance`, valori diversi dei contatori attesi/effettivi
...
Quando i contatori del saldo in attesa atteso ed effettivo differiscono, segui
questi passaggi per riconciliare `decryptable_available_balance`:

1. Inizia con `decryptable_available_balance` dal token account
2. Recupera le transazioni più recenti includendo le istruzioni di deposito e
   trasferimento fino alla differenza del contatore (effettivo - atteso):
   - Aggiungi gli importi pubblici dalle istruzioni di deposito
   - Decifra e aggiungi gli importi del testo cifrato di destinazione dalle
...
2. **Ridimensiona lo Spazio dell'Account**: Invoca l'istruzione
...
3. **Configura i Trasferimenti Riservati**: Invoca l'istruzione
...
**Solo il proprietario del token account può configurare un token account per i
trasferimenti riservati**.

L'istruzione _rs`ConfigureAccount`_ richiede la generazione lato client di
chiavi di cifratura e dati di prova che possono essere generati solo dal
proprietario del token account.

L'istruzione _rs`PubkeyValidityProofData`_ crea una prova che verifica la
validità di una chiave ElGamal. Per i dettagli sull'implementazione, vedi:
...
Il codice seguente mostra come creare un associated token account con
l'estensione Confidential Transfer,

Per eseguire l'esempio, avvia un validator locale con il Token Extensions
Program clonato dalla mainnet utilizzando il comando seguente. È necessario
avere la Solana CLI installata per avviare il validator locale.
...
<Callout type="info">
  Al momento della stesura, i Trasferimenti Riservati non sono abilitati sul
