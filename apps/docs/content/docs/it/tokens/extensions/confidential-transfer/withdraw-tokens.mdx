...
4. Chiudere gli account di prova per recuperare il SOL utilizzato per crearli.

Il seguente diagramma illustra i passaggi necessari per prelevare token dal
...
- Generare una equality proof e una range proof lato client
...
Il seguente esempio dimostra come prelevare token dal saldo disponibile
riservato al saldo pubblico.
...
avere la CLI di Solana installata per avviare il validator locale.
...
<Callout type="info">
  Al momento della stesura, i Confidential Transfer non sono abilitati sul
