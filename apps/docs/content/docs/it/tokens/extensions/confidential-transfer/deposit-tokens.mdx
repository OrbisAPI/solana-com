...
Prima che i token possano essere trasferiti in modo riservato, il saldo pubblico
dei token deve essere convertito in un saldo riservato. Questa conversione
avviene in due fasi:

1. **Saldo in sospeso riservato**: Inizialmente, i token vengono "depositati"
   dal saldo pubblico a un saldo riservato "in sospeso".
2. **Saldo disponibile riservato**: Il saldo in sospeso viene quindi "applicato"
...
nel saldo in sospeso riservato.

Il diagramma seguente mostra i passaggi necessari per depositare token dal saldo
pubblico al saldo in sospeso riservato:
...
Per convertire un saldo pubblico in un saldo in sospeso riservato, richiamare l'
...
istruzione. L'
[importo massimo](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
...
L'esempio seguente mostra come depositare il saldo pubblico dei token nel saldo
in sospeso riservato.

Per eseguire l'esempio, avviare un validator locale con il Token Extensions
