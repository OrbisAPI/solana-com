...
## Come Creare un Mint e Bruciare Token con Permesso

Per creare un mint con burn con autorizzazione e bruciare token:
...
3. Crea un token account e minta i token.
4. Brucia con _rs`PermissionedBurnInstruction::BurnChecked`_ firmato sia dal
   proprietario del token account che dall'autorità di burn.
5. Facoltativamente, disabilita il burning con autorizzazione tramite
   _rs`SetAuthority`_ usando _rs`AuthorityType::PermissionedBurn`_ e un'autorità
   `None`, il che riabilita le istruzioni di burn standard. Gli esempi di codice
   completi qui sotto mostrano questo passaggio.
...
## !!steps Brucia con l'autorità di combustione

Brucia token con _rs`PermissionedBurnInstruction::BurnChecked`_ firmato sia dal
proprietario del token account che dall'autorità di combustione.
...
| Item                                           | Descrizione                                                                                                                               | Fonte                                                                                                                                         |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Estensione del mint che memorizza l'autorità necessaria per co-firmare ogni combustione per il mint.                                      | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Istruzione che inizializza la configurazione di combustione con permesso prima di _rs`InitializeMint`_.                                   | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Istruzione di combustione che richiede le firme dell'autorità di combustione e del proprietario o delegato del token account.             | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Istruzione di combustione con verifica dei decimali che utilizza gli stessi account di _rs`PermissionedBurnInstruction::Burn`_.           | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Discriminatore di autorità utilizzato con _rs`SetAuthority`_ per ruotare o disabilitare l'autorità di combustione su un mint.             | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Logica del processore che inizializza _rs`PermissionedBurnConfig`_ su un mint non inizializzato e memorizza l'autorità di combustione.    | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Logica del processore che rifiuta le combustioni standard quando l'autorità di combustione è impostata e verifica la firma dell'autorità. | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logica del processore che convalida e ruota l'autorità di combustione quando viene utilizzato _rs`AuthorityType::PermissionedBurn`_.      | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |
...
per il burn con permessi, quindi non è incluso alcun esempio legacy.
