...
<Callout type="info">
  ...
  [Keychain](/docs/tools/keychain) và [Ký giao dịch trong Môi trường
...
- **Máy trạng thái headless**: Chạy client bên trong các API route, script, hoặc
  worker để tái sử dụng cùng logic điều phối ví + RPC vốn đang hỗ trợ giao diện
  người dùng của bạn.
- **Dashboard thời gian thực**: Kết hợp các watcher (số dư, tài khoản, chữ ký)
  với thư viện UI ưa thích của bạn; client xử lý việc phân phối websocket và làm
  mới cache.
- **Store tùy chỉnh**: Inject store Zustand của riêng bạn để nạp dữ liệu từ
  IndexedDB/localStorage, đồng bộ trạng thái với phiên server, hoặc điều phối
  giữa các tab trình duyệt.
- **Kết nối với React hooks**: Truyền một client instance đã được cấu hình vào
  `@solana/react-hooks` khi bạn muốn sử dụng các hook tiện lợi trên cùng một
  runtime.
- **Khả năng kiểm thử**: Giao diện client đơn nhất có thể được mock trong unit
  test, giúp dễ dàng mô phỏng các phản hồi RPC hoặc phiên ví mà không cần ví
  trình duyệt.
