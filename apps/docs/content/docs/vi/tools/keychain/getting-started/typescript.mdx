...
`identity` để bỏ qua bước thiết lập thủ công `pipe` như trên:
...
Plugin chấp nhận cùng cấu hình có tag backend như `createKeychainSigner`, và chỉ
backend được điều phối đến mới được đóng gói (các gói backend tải thông qua
`import()` động).

Dùng `keychainPayer` hoặc `keychainIdentity` để chỉ đặt một vai trò, và kết hợp
nhiều backend trên một client duy nhất:
...
Đã có sẵn `SolanaSigner`? Mỗi keychain signer đều là một Kit `TransactionSigner`
hợp lệ, vì vậy bạn có thể cài đặt signer hiện có bằng
...
## Xây Dựng Custom Signer

Triển khai giao diện `SolanaSigner` để tạo custom signer:
