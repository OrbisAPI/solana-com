...
riêng tư. Chỉ có số lượng chuyển khoản và số dư token là được bảo mật. Địa chỉ
của token account vẫn hiển thị công khai.
...
- [Hướng dẫn khởi động nhanh](https://www.solana-program.com/docs/confidential-balances#setup) -
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Các đoạn mã hướng dẫn cách sử dụng extension Chuyển khoản bảo mật

### Cơ chế hoạt động?

Extension Chuyển khoản bảo mật bổ sung
...
1. Tạo một mint account với extension chuyển khoản bảo mật.
2. Tạo các token account với extension chuyển khoản bảo mật cho người gửi và
   người nhận.
...
6. **Chuyển** token bảo mật từ token account của người gửi sang token account
   của người nhận.
...
<Cards>
  ...
  >
    Cách tạo một mint account với extension Chuyển khoản bảo mật
  </Card>
  <Card
    title="Tạo Token Account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Cách cấu hình một token account với extension Chuyển khoản bảo mật
  </Card>
  <Card
    title="Nạp Token"
  ...
  >
    Cách ví, trình khám phá và sàn giao dịch có thể hỗ trợ token chuyển khoản
    bảo mật
  </Card>
  <Card
...
Sơ đồ dưới đây mô tả chi tiết trình tự luồng cơ bản cho các giao dịch token bảo
mật:
...
Ví dụ Rust dưới đây trình bày các bước thực hiện giao dịch token bảo mật tương
ứng với sơ đồ trên. Các bước được tách thành các hàm trợ giúp để dễ đọc hơn.
...
<Callout type="info">
  Tại thời điểm viết bài, Confidential Transfers chưa được bật trên validator
  cục bộ mặc định. Bạn phải sao chép Token Extensions Program từ mainnet để chạy
  mã ví dụ.
</Callout>
...
## Các Lệnh Confidential Transfer
...
của extension Confidential Transfer như sau:
...
| _rs`UpdateMint`_                    | Cập nhật cài đặt giao dịch bảo mật cho một mint.                                                                                                       |
...
| _rs`TransferWithFee`_               | Chuyển token giữa các token account một cách bảo mật kèm phí.                                                                                          |
| _rs`ConfigureAccountWithRegistry`_  | Cách thay thế để cấu hình token account cho các giao dịch bảo mật bằng tài khoản _rs`ElGamalRegistry`_ thay vì bằng chứng _rs`VerifyPubkeyValidity`_.  |
