...
## Cách nạp token vào số dư chờ bảo mật
...
1. **Số dư chờ bảo mật**: Ban đầu, token được "nạp" từ số dư công khai vào số dư
   bảo mật "chờ xử lý".
2. **Số dư khả dụng bảo mật**: Số dư chờ xử lý sau đó được "áp dụng" vào số dư
   khả dụng, giúp token sẵn sàng cho các giao dịch chuyển bảo mật.

Phần này giải thích giai đoạn đầu tiên: nạp số dư token công khai vào số dư chờ
bảo mật.
...
Để chuyển đổi số dư công khai thành số dư chờ bảo mật, hãy gọi lệnh
...
cho mỗi lệnh nạp bị giới hạn ở mức 2^48.

Crate `spl_token_client` cung cấp phương thức `confidential_transfer_deposit`
giúp xây dựng và gửi một giao dịch với lệnh `Deposit`, như được minh họa trong
ví dụ bên dưới.

### Mã ví dụ

Ví dụ sau đây minh họa cách nạp số dư token công khai vào số dư chờ bảo mật.
