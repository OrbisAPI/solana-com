...
## Cách áp dụng số dư đang chờ xử lý vào số dư khả dụng
...
1. **Số dư đang chờ xử lý bảo mật**: Ban đầu, các token được "nạp" từ số dư công
   khai sang số dư bảo mật "đang chờ xử lý".
2. **Số dư khả dụng bảo mật**: Số dư đang chờ xử lý sau đó được "áp dụng" vào số
   dư khả dụng, giúp các token sẵn sàng cho việc chuyển khoản bảo mật.

Phần này giải thích giai đoạn thứ hai: áp dụng số dư đang chờ xử lý vào số dư
khả dụng.

Khi các token được "nạp" từ số dư công khai hoặc khi các token được chuyển khoản
bảo mật từ token account này sang token account khác, các token sẽ ban đầu được
thêm vào số dư bảo mật đang chờ xử lý. Trước khi token có thể được sử dụng cho
các giao dịch chuyển khoản bảo mật, số dư đang chờ xử lý phải được "áp dụng" vào
số dư khả dụng.
...
Sơ đồ sau đây mô tả các bước liên quan đến việc áp dụng số dư đang chờ xử lý vào
số dư khả dụng:
...
Để chuyển đổi số dư đang chờ xử lý thành số dư khả dụng, hãy gọi lệnh
