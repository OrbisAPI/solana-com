...
Để chuyển token bí mật từ token account này sang token account khác, cả người
gửi và người nhận đều phải có token account được cấu hình với trạng thái
_rs`ConfidentialTransferAccount`_ và được phê duyệt cho việc chuyển bí mật.
...
1. Tạo
   ...
   **Bằng chứng Đẳng thức
   ...
   Xác minh rằng bản mã số dư khả dụng mới sau khi chuyển khớp với
   ...
   **Bằng chứng Tính hợp lệ của Bản mã
   ...
   Xác minh rằng các bản mã của số tiền chuyển được tạo đúng cách cho cả ba bên
   (nguồn, đích và kiểm toán viên), đảm bảo số tiền chuyển được mã hóa chính xác
   theo khóa công khai của từng bên.
   ...
   **Bằng chứng Phạm vi
   ...
   thấp/cao) đều không âm và nằm trong một phạm vi xác định.
...
Để chuyển token bí mật từ token account này sang token account khác, bạn phải:

- Tạo bằng chứng đẳng thức, bằng chứng tính hợp lệ của bản mã và bằng chứng phạm
  vi phía client
...
- Phương thức `confidential_transfer_create_context_state_account` tạo một proof
  account.
- Phương thức `confidential_transfer_transfer` gọi lệnh `Transfer` instruction.
- Phương thức `confidential_transfer_close_context_state_account` đóng một proof
  account.
