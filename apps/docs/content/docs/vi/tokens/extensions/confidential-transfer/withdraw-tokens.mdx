...
1. Tạo
   ...
   **Bằng chứng Đẳng thức
   ...
   Xác minh rằng bản mã số dư khả dụng còn lại sau khi rút khớp với
...
2. Đối với mỗi bằng chứng:
   ...
   - Lưu trữ siêu dữ liệu dành riêng cho bằng chứng trong tài khoản "context
     state" của bằng chứng để sử dụng trong các lệnh khác.
...
- Tạo bằng chứng đẳng thức và bằng chứng phạm vi phía client
...
- Phương thức `confidential_transfer_create_context_state_account` để tạo một
  tài khoản bằng chứng.
- Phương thức `confidential_transfer_withdraw` gọi lệnh `Withdraw`.
- Phương thức `confidential_transfer_close_context_state_account` để đóng một
  tài khoản bằng chứng.
...
được sao chép từ mainnet bằng lệnh sau. Bạn phải cài đặt Solana CLI trước khi
...
<Callout type="info">
  Tại thời điểm viết bài, Chuyển khoản Bảo mật chưa được bật trên validator cục
  bộ mặc định. Bạn phải sao chép Token Extensions Program từ mainnet để chạy mã
  ví dụ.
</Callout>
