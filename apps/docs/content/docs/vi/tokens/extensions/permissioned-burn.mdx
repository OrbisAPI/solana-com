...
## Cách Tạo Mint và Burn Có Quyền Hạn

Để tạo một mint với tính năng burn có quyền hạn và đốt token:
...
5. Tùy chọn vô hiệu hóa tính năng burn có quyền hạn bằng _rs`SetAuthority`_ sử
   dụng _rs`AuthorityType::PermissionedBurn`_ và một quyền `None`, điều này kích
   hoạt lại các lệnh burn tiêu chuẩn. Các ví dụ mã đầy đủ bên dưới hiển thị bước
   này.
...
_rs`PermissionedBurnConfig`_. Đây là kích thước được dùng trong
_rs`CreateAccount`_.
...
Tính toán rent sử dụng kích thước cần thiết cho mint cộng với phần mở rộng
_rs`PermissionedBurnConfig`_.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Extension của Mint lưu trữ authority cần thiết để đồng ký mỗi lần đốt cho mint.                                     | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Instruction khởi tạo cấu hình permissioned burn trước _rs`InitializeMint`_.                                         | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Instruction đốt yêu cầu chữ ký từ burn authority và chủ sở hữu hoặc đại diện của token account.                     | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Instruction đốt có kiểm tra decimals sử dụng cùng các tài khoản với _rs`PermissionedBurnInstruction::Burn`_.        | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
...
| _rs`process_burn`_                             | Logic xử lý từ chối các lệnh đốt thông thường khi burn authority được thiết lập và xác minh chữ ký của authority.   | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logic xử lý xác thực và xoay vòng burn authority khi _rs`AuthorityType::PermissionedBurn`_ được sử dụng.            | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Ví dụ `Kit` bên dưới sử dụng trực tiếp các hướng dẫn đã được tạo ra. Gói
`@solana/spl-token` legacy đã được xuất bản chưa hỗ trợ đốt có quyền, nên không
có ví dụ legacy nào được đưa vào.
