...
### État du Mint Confidential Transfer
...
- **authority** : Le compte disposant de la permission de modifier les
  paramètres de transfert confidentiel pour le mint et d'approuver les nouveaux
  comptes confidentiels si l'approbation automatique est désactivée.

- **auto_approve_new_accounts** : Lorsque ce champ est défini sur true, les
  utilisateurs peuvent créer des token accounts avec les transferts
  confidentiels activés par défaut. Lorsqu'il est défini sur false, l'autorité
  doit approuver chaque nouveau token account avant qu'il puisse être utilisé
  pour des transferts confidentiels.

- **auditor_elgamal_pubkey** : Un auditeur optionnel capable de déchiffrer les
  montants des transferts dans les transactions confidentielles, offrant un
...
avoir installé le CLI Solana pour démarrer le validator local.
