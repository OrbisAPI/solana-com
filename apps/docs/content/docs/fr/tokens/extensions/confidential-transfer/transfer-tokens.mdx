...
1. Créez
   ...
   **Preuve d'égalité
   ...
   correctement calculé comme `new_balance = current_balance - transfer_amount`.
   ...
   **Preuve de validité du texte chiffré
   ...
   garantissant que le montant du transfert est correctement chiffré avec la clé
   ...
   **Preuve de plage
   ...
   en bits de poids faible/fort) sont tous non négatifs et dans une plage
   spécifiée.

2. Pour chaque preuve :
   - Invoquez le programme de preuve ZK ElGamal pour vérifier les données de la
     preuve.
...
Le diagramme suivant illustre les étapes du transfert de tokens depuis le token
account d'un expéditeur vers le token account d'un destinataire.
...
- La méthode `confidential_transfer_create_context_state_account` crée un compte
  de preuve.
- La méthode `confidential_transfer_transfer` invoque l'instruction `Transfer`.
- La méthode `confidential_transfer_close_context_state_account` ferme un compte
  de preuve.

### Exemple de code

L'exemple suivant montre comment transférer des tokens de manière confidentielle
entre des token accounts.
