...
Le diagramme suivant illustre les étapes de création d'un token account avec
...
- **decryptable_available_balance** : Le solde disponible chiffré avec une clé
  AES (Advanced Encryption Standard) pour un déchiffrement efficace par le
...
- **maximum_pending_balance_credit_counter** : La limite du nombre de crédits en
  attente avant qu'une instruction _rs`ApplyPendingBalance`_ soit requise pour
...
pour prévenir les attaques par déni de service (DoS). Sans cette séparation, un
attaquant pourrait envoyer répétitivement des tokens vers un token account,
bloquant ainsi la capacité du propriétaire du token account à transférer des
tokens. Le propriétaire du token account serait dans l'impossibilité de
transférer des tokens, car le solde chiffré changerait entre le moment où la
transaction est soumise et celui où elle est traitée, entraînant l'échec de la
transaction.
...
`pending_balance_hi` car le déchiffrement ElGamal nécessite davantage de calculs
pour les grands nombres. Vous pouvez trouver l'implémentation de l'arithmétique
des textes chiffrés
...
1. Commencer avec le `decryptable_available_balance` depuis le token account
...
L'instruction _rs`PubkeyValidityProofData`_ crée une preuve vérifiant qu'une clé
ElGamal est valide. Pour les détails d'implémentation, voir :
...
Le code suivant illustre comment créer un associated token account avec
l'extension Confidential Transfer,

Pour exécuter l'exemple, démarrez un validator local avec le Token Extension
Program cloné depuis le mainnet à l'aide de la commande suivante. Vous devez
...
<Callout type="info">
  ...
  Program du réseau principal pour exécuter l'exemple de code.
</Callout>
