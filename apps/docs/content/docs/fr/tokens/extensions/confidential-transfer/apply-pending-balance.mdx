...
2. **Solde confidentiel disponible** : Le solde en attente est ensuite «
   appliqué » au solde disponible, rendant les tokens utilisables pour des
...
Le diagramme suivant illustre les étapes nécessaires à l'application du solde en
...
avoir le Solana CLI installé pour démarrer le validator local.
...
<Callout type="info">
  ...
  sur le validator local par défaut. Vous devez cloner le Token Extension
