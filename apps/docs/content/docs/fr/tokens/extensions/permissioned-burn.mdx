...
Un validator de test local intègre également une ancienne version de Token-2022,
donc pour exécuter les exemples de cette page, chargez le programme v11.0.0
depuis le devnet dans votre validator de test :
...
2. Créez le mint account avec _rs`CreateAccount`_, initialisez
   _rs`PermissionedBurnConfig`_, et initialisez le mint avec
   _rs`InitializeMint`_.
3. Créez un token account et émettez des tokens.
4. Brûlez avec _rs`PermissionedBurnInstruction::BurnChecked`_ signé à la fois
   par le propriétaire du token account et par l'autorité de destruction.
5. Désactivez éventuellement la destruction autorisée avec _rs`SetAuthority`_ en
   utilisant _rs`AuthorityType::PermissionedBurn`_ et une autorité `None`, ce
...
Calculez la taille du mint account pour le mint de base ainsi que l'extension
...
Calculez le rent en utilisant la taille nécessaire pour le mint ainsi que
l'extension _rs`PermissionedBurnConfig`_.
...
Initialisez l'extension _rs`PermissionedBurnConfig`_ sur le mint avec l'autorité
de destruction.
...
## !!steps Créer un token account et minter des tokens

Créez un token account pour le payeur et minter des tokens sur celui-ci.
...
## !!steps Brûler avec l'autorité de brûlage

Brûlez des tokens avec _rs`PermissionedBurnInstruction::BurnChecked`_ signé à la
fois par le propriétaire du token account et l'autorité de brûlage.
...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Extension de mint qui stocke l'autorité requise pour co-signer chaque brûlage pour le mint.                                              | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Instruction qui initialise la configuration de brûlage autorisé avant _rs`InitializeMint`_.                                              | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Instruction de brûlage qui requiert les signatures de l'autorité de brûlage et du propriétaire ou délégué du token account.              | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Instruction de brûlage avec vérification des décimales qui utilise les mêmes comptes que _rs`PermissionedBurnInstruction::Burn`_.        | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Discriminateur d'autorité utilisé avec _rs`SetAuthority`_ pour faire tourner ou désactiver l'autorité de brûlage sur un mint.            | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Logique du processeur qui initialise _rs`PermissionedBurnConfig`_ sur un mint non initialisé et stocke l'autorité de brûlage.            | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Logique du processeur qui rejette les brûlages standard lorsque l'autorité de brûlage est définie et vérifie la signature de l'autorité. | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logique du processeur qui valide et fait tourner l'autorité de brûlage lorsque _rs`AuthorityType::PermissionedBurn`_ est utilisé.        | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

L'exemple `Kit` ci-dessous utilise directement les instructions générées. Le
package `@solana/spl-token` legacy publié ne prend pas encore en charge la
