...
<Callout type="info">
  Vous exécutez en mode headless — dans une route API, un worker ou un script
...
- **Tableaux de bord en temps réel** : Combinez les observateurs (soldes,
...
- **Testabilité** : L'interface client unique peut être simulée dans des tests
  unitaires, ce qui facilite la simulation de réponses RPC ou de sessions de
  portefeuille sans portefeuille navigateur présent.
