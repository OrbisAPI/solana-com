...
Le plugin accepte la même configuration étiquetée par backend que
`createKeychainSigner`, et seul le backend vers lequel il effectue la
distribution est inclus dans le bundle (les packages backend se chargent via
`import()` dynamique).

Utilisez `keychainPayer` ou `keychainIdentity` pour définir un seul rôle, et
combinez plusieurs backends sur un même client :
