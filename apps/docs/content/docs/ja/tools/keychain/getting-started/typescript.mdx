...
### Kitクライアントとの使用（keychainSignerプラグイン）

`@solana/keychain-kit-plugin`
は、[Kitクライアント](https://github.com/anza-xyz/kit)にキーチェーン署名者を直接インストールし、`payer`
および `identity` として接続することで、上記の手動 `pipe`
設定をスキップできます：
...
と同じバックエンドタグ付き設定を受け入れ、ディスパッチ先のバックエンドのみがバンドルされます（バックエンドパッケージは動的
`import()` 経由で読み込まれます）。

`keychainPayer` または `keychainIdentity`
を使用して単一のロールのみを設定し、1つのクライアントで複数のバックエンドを組み合わせることができます：
...
既に `SolanaSigner` をお持ちですか？キーチェーン署名者はすべて有効な Kit
...
を使用して既存のものをインストールできます（`pnpm add @solana/kit-plugin-signer`）：
