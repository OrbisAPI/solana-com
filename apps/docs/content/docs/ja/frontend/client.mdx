...
<Callout type="info">
  ヘッドレスで実行している場合 —
  APIルート、ワーカー、またはブラウザウォレットのないスクリプトの場合は、
  生のkeypairの代わりにバックエンドのキー管理サービスで署名してください。
...
- **ヘッドレスステートマシン**:
  APIルート、スクリプト、またはワーカー内でクライアントを実行し、UIを支える同じウォレット +
  RPCオーケストレーションロジックを再利用する。
- **リアルタイムダッシュボード**: ウォッチャー（残高、アカウント、署名）をお好みのUIライブラリと組み合わせる。クライアントはWebSocketのファンアウトとキャッシュの無効化を処理する。
- **カスタムストア**: 独自のZustandストアを注入してIndexedDB/localStorageからハイドレートしたり、サーバーセッションに状態をミラーリングしたり、ブラウザタブ間で調整したりする。
- **Reactフックへのブリッジ**: 同じランタイム上でエルゴノミックなフックを使いたい場合は、設定済みのクライアントインスタンスを`@solana/react-hooks`に渡す。
- **テスト容易性**: 単一のクライアントインターフェースはユニットテストでモック化でき、ブラウザウォレットなしでRPCレスポンスやウォレットセッションを簡単にシミュレートできる。
