...
Confidential Transfer拡張機能は、Token Extension
...
コンフィデンシャルなトークン転送の基本的な流れは次のとおりです：

1. コンフィデンシャル転送拡張機能を持つmint accountを作成する。
2. 送信者と受信者のために、コンフィデンシャル転送拡張機能を持つtoken
   accountを作成する。
3. 送信者のアカウントにトークンをミントする。
4. 送信者のパブリック残高を**コンフィデンシャル保留残高**に**デポジット**する。
...
8. 受信者のコンフィデンシャル利用可能残高を**パブリック残高**に**引き出す**。
...
<Cards>
  ...
  >
    Confidential Transfer拡張機能を使ってtoken accountを設定する方法
  </Card>
  <Card
    title="トークンのデポジット"
...
以下のRustサンプルは、前述の図に対応する機密トークン転送の手順を示しています。手順は、わかりやすさのためにヘルパー関数に分割されています。

サンプルを実行するには、以下のコマンドを使用して、メインネットからクローンしたToken
Extensions
Programを含むローカルvalidatorを起動してください。ローカルvalidatorを使用するには、Solana
...
<Callout type="info">
  本稿執筆時点では、Confidential
  TransfersはデフォルトのローカルvalidatorでHは有効になっていません。サンプルコードを実行するには、メインネットのToken
...
Confidential Transfer拡張機能の全instructions一覧
...
| instruction                         | 説明                                                                                                                                                      |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 機密転送用にmint accountを設定します。このinstructionは、_rs`TokenInstruction::InitializeMint`_ instructionと同じトランザクションに含める必要があります。 |
| _rs`UpdateMint`_                    | mintの機密転送設定を更新します。                                                                                                                          |
| _rs`ConfigureAccount`_              | 機密転送用にtoken accountを設定します。                                                                                                                   |
| _rs`ApproveAccount`_                | mintが新しいtoken accountの承認を必要とする場合、機密転送用のtoken accountを承認します。                                                                  |
...
| _rs`DisableConfidentialCredits`_    | 公開転送は引き続き許可しながら、受信側の機密転送をブロックします。                                                                                        |
| _rs`EnableNonConfidentialCredits`_  | token accountが公開トークン転送を受け取れるようにします。                                                                                                 |
| _rs`DisableNonConfidentialCredits`_ | 通常の転送をブロックし、アカウントが機密転送のみを受け取れるようにします。                                                                                |
| _rs`TransferWithFee`_               | token account間でトークンを手数料付きで機密転送します。                                                                                                   |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ proofの代わりに*rs`ElGamalRegistry`* accountを使用して、機密転送用のtoken accountを設定する代替方法です。                      |
