...
## トークンをコンフィデンシャル保留残高に入金する方法

トークンを機密転送するには、まず公開トークン残高をコンフィデンシャル残高に変換する必要があります。この変換は2つの段階で行われます：

1. **コンフィデンシャル保留残高**：最初に、トークンは公開残高から「保留中」のコンフィデンシャル残高へ「入金」されます。
2. **コンフィデンシャル利用可能残高**：次に、保留残高が利用可能残高に「適用」され、トークンがコンフィデンシャル転送に使用できる状態になります。

このセクションでは、第1段階である公開トークン残高をコンフィデンシャル保留残高に入金する手順を説明します。

以下の図は、公開残高からコンフィデンシャル保留残高へトークンを入金する際の手順を示しています：
...
instructionを呼び出します。入金instructionあたりの
...
以下の例では、公開トークン残高をコンフィデンシャル保留残高に入金する方法を示します。

この例を実行するには、以下のコマンドを使用して、メインネットからクローンしたToken
Extensions
Programを使用してローカルvalidatorを起動してください。ローカルvalidatorを起動するには、Solana
...
<Callout type="info">
  本稿執筆時点では、コンフィデンシャル転送はデフォルトのローカルvalidatorでは有効になっていません。サンプルコードを実行するには、メインネットのToken
