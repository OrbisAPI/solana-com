...
1. クライアント側で
   ...
   **等価証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**: 転送後の新しい利用可能残高の暗号文が、対応する
   ...
   と一致することを検証し、ソースアカウントの新しい利用可能残高が
   `new_balance = current_balance - transfer_amount`
   として正しく計算されていることを保証します。

   **暗号文有効性証明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**: 転送金額の暗号文が3者（送信元、送信先、監査者）全員に対して適切に生成されていることを検証し、各当事者の公開鍵のもとで転送金額が正しく暗号化されていることを保証します。

   **範囲証明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**: 新しい利用可能残高と転送金額（上位/下位ビットに分割）がすべて非負であり、指定された範囲内にあることを検証します。
...
あるtoken accountから別のtoken
accountへトークンを秘密裏に転送するには、以下を行う必要があります：

- 等価証明、暗号文有効性証明、および範囲証明をクライアント側で生成する
- Zk
...
- `confidential_transfer_create_context_state_account`
  メソッドはプルーフアカウントを作成します。
- `confidential_transfer_transfer` メソッドは `Transfer`
  instructions を呼び出します。
- `confidential_transfer_close_context_state_account`
  メソッドはプルーフアカウントを閉じます。

### サンプルコード

以下の例では、token accounts 間でトークンを秘密裏に転送する方法を示します。

この例を実行するには、以下のコマンドを使用してメインネットからクローンした Token
Extensions
Program を使って、ローカル validator を起動してください。ローカル validator を起動するには、Solana
CLI がインストールされている必要があります。
...
<Callout type="info">
  執筆時点では、Confidential Transfers はデフォルトのローカル validator
  では有効になっていません。サンプルコードを実行するには、メインネットの Token
  Extensions Program をクローンする必要があります。
</Callout>
