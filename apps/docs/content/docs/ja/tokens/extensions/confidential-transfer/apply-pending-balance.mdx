...
トークンを秘密裏に転送する前に、公開トークン残高を秘密残高に変換する必要があります。この変換は2段階で行われます：

1. **秘密保留残高**：まず、トークンが公開残高から「保留中」の秘密残高に「預け入れ」られます。
2. **秘密利用可能残高**：次に、保留残高が利用可能残高に「適用」され、トークンが秘密転送に使用できる状態になります。

このセクションでは、第2段階である保留残高を利用可能残高に適用する手順について説明します。

トークンが公開残高から「預け入れ」られる場合、またはトークンがある token
account から別の token
account へ秘密裏に転送される場合、トークンは最初に秘密保留残高に追加されます。トークンを秘密転送に使用するには、保留残高を利用可能残高に「適用」する必要があります。
...
instructionsを呼び出してください。

`spl_token_client` クレートは、以下の例に示すように、`ApplyPendingBalance`
instructionsを含むトランザクションを構築して送信する
`confidential_transfer_apply_pending_balance` メソッドを提供しています。

### サンプルコード

次の例では、秘密保留残高を秘密利用可能残高に適用する方法を示しています。

例を実行するには、以下のコマンドを使用して、メインネットからクローンした Token
Extension
Program を使用してローカル validator を起動してください。ローカル validator を起動するには、Solana
...
<Callout type="info">
  ...
  Extension Program をクローンする必要があります。
</Callout>
