...
1. クライアント側で
   ...
   **等価証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**: 出金後の残存する利用可能残高の暗号文が、対応する
   [Pedersen コミットメント](https://en.wikipedia.org/wiki/Commitment_scheme)
   と一致することを検証し、アカウントの新しい利用可能残高が
   `remaining_balance = current_balance - withdraw_amount`
   として正しく計算されることを保証します。

   **範囲証明 ([BatchedRangeProofU64Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u64.rs#L37))**: 出金後の残存する利用可能残高が非負であり、指定された範囲内であることを検証します。

2. 各証明について：
   - ZK ElGamal 証明プログラムを呼び出して証明データを検証します。
   - 他の instructions で使用するため、証明固有のメタデータを証明「コンテキスト状態」アカウントに保存します。
...
4. 証明アカウントを閉じて、作成に使用した SOL を回収します。

次の図は、機密利用可能残高からパブリック残高にトークンを出金する際の手順を示しています：
...
機密利用可能残高からパブリック残高にトークンを出金するには、以下を行う必要があります：

- クライアント側で等価証明と範囲証明を生成する
- ZK
  ElGamal 証明プログラムを呼び出して証明を検証し、「コンテキスト状態」アカウントを初期化する
- 2つの証明アカウントを提供して
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  instructionを呼び出す
- 2つの証明アカウントを閉じて rent を回収する

`spl_token_client` クレートは以下のメソッドを提供します：

- `confidential_transfer_create_context_state_account`
  は証明アカウントを作成するメソッドです。
- `confidential_transfer_withdraw` は `Withdraw`
  instructionを呼び出すメソッドです。
- `confidential_transfer_close_context_state_account`
  は証明アカウントを閉じるメソッドです。

### サンプルコード

次の例は、機密利用可能残高からパブリック残高にトークンを出金する方法を示しています。

サンプルを実行するには、以下のコマンドを使用してメインネットからToken Extension
Programをクローンしたローカルvalidatorを起動してください。ローカルvalidatorを起動するには、Solana
...
<Callout type="info">
  執筆時点では、Confidential
  TransfersはデフォルトのローカルvalidatorではValidatorが有効になっていません。サンプルコードを実行するには、メインネットのToken
