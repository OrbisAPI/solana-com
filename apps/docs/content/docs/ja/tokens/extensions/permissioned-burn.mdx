...
1. mint
   accountのサイズと、ミントおよび*rs`PermissionedBurnConfig`*エクステンションに必要なrentを計算します。
2. *rs`CreateAccount`*でmint
   accountを作成し、*rs`PermissionedBurnConfig`*を初期化し、*rs`InitializeMint`*でミントを初期化します。
3. token accountを作成してトークンをミントします。
4. token
   accountのオーナーとバーンオーソリティの両方が署名した*rs`PermissionedBurnInstruction::BurnChecked`*でバーンします。
5. 任意で、*rs`AuthorityType::PermissionedBurn`*と`None`オーソリティを使用して*rs`SetAuthority`*で許可付きバーンを無効化すると、標準のバーンinstructionsが再度有効になります。以下の完全なコード例にこのステップが示されています。

<ScrollyCoding>

## !!steps アカウントサイズの計算

ベースミントと*rs`PermissionedBurnConfig`*エクステンションのmint
...
ミントと*rs`PermissionedBurnConfig`*エクステンションに必要なサイズを使用してrentを計算します。
...
バーンオーソリティを指定してミントの*rs`PermissionedBurnConfig`*エクステンションを初期化します。
...
同一トランザクション内で*rs`InitializeMint`*を使用してミントを初期化します。
...
| アイテム                                       | 説明                                                                                                         | ソース                                                                                                                                         |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | ミントに対するすべてのバーンに共同署名するために必要な権限を保存するミント拡張機能。                         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | _rs`InitializeMint`_ の前にパーミッションドバーン設定を初期化するinstruction。                               | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | バーン権限およびtoken accountのオーナーまたはデリゲートからの署名を必要とするバーンinstruction。             | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
...
| _rs`AuthorityType::PermissionedBurn`_          | ミントのバーン権限を更新または無効化するために _rs`SetAuthority`_ と共に使用される権限ディスクリミネーター。 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | 未初期化のミントに _rs`PermissionedBurnConfig`_ を初期化し、バーン権限を保存するプロセッサーロジック。       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
...
| _rs`process_set_authority`_                    | _rs`AuthorityType::PermissionedBurn`_ が使用された際にバーン権限を検証および更新するプロセッサーロジック。   | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |
