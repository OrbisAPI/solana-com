...
Solana JSON RPC
APIには、トランザクションに必要なコンピュートユニットを見積もるために使用できる
[`simulatetransaction`](/docs/rpc/http/simulatetransaction)メソッドがあります。これにはコンピュートユニットの使用量の見積もりも含まれます。
[`@solana/kit`](https://github.com/anza-xyz/kit)は、トランザクションのリソース制限を見積もり、1ステップでメッセージに設定するヘルパー関数を提供します（内部では`simulatetransaction`メソッドを使用）。バージョン1トランザクションの場合、これらのヘルパーはロードされたアカウントのデータサイズ制限も見積もります。
...
<Callout type="info">
  kitプラグインクライアントを使用してトランザクションをビルドおよび送信する場合、通常このステップは不要です
  —
  クライアントは送信時（`.sendTransaction()`）にコンピュートバジェットのinstructionsを自動的に追加します。上記の手動フローは、`@solana/kit`を使用して直接トランザクションをアセンブルする場合に使用します。
</Callout>

本番環境で同じ種類のトランザクションを繰り返し実行する場合は、毎回コンピュートユニットを見積もるオーバーヘッドを避けるため、そのトランザクションタイプのコンピュート見積もりをキャッシュすることを検討してください。

### Jito Bundles

Jito
bundlesは、複数のトランザクションのアトミックな実行を管理するためのツールです。これは、チップとともに複数のトランザクションをJitoネットワークに送信することで実現されます。チップは、Jitoネットワークがトランザクションをブロックに含めるよう促すインセンティブとして使用できます。

**リソース:**

- [Jitoドキュメント](https://docs.jito.wtf/lowlatencytxnsend/)
- [🎥 動画: Jito Bundles](https://www.youtube.com/watch?v=HZ1pK9i6zx4)
- [Jito bundlesに関するQuickNodeガイド](https://www.quicknode.com/guides/solana-development/transactions/jito-bundles)

### リトライ戦略

トランザクションはさまざまな理由で失敗する可能性があります。成功/失敗を即座に返す従来の決済APIとは異なり、ブロックチェーンのトランザクションは確認のトラッキングが必要です。

**主要な概念:**

- **ブロックハッシュの有効期限**: トランザクションは約150ブロック（約60〜90秒）有効です
- **冪等性**: 同じ署名済みトランザクションは常に同じシグネチャを生成するため、再送信は安全です
- **指数バックオフ**: 急速なリトライでネットワークに過負荷をかけないようにしてください
...
- [Helius: Solanaでトランザクションを通す方法](https://www.helius.dev/blog/how-to-land-transactions-on-solana)
...
| ----------- | ---------------------------------------------- | ---------------------- | -------------------------- |
| `processed` | ブロックに含まれているが、まだ投票されていない | 承認待ち               | リアルタイムUIの更新       |
| `confirmed` | スーパーマジョリティが投票済み                 | 資金決済済み           | **ほとんどの支払い**       |
| `finalized` | ルート化済み、取り消し不可                     | 資金確定済み           | 高額取引・コンプライアンス |

**各レベルの使い分け：**

- **UIの更新**：即時フィードバックのために `processed`
  を表示（「支払いが送信されました」）
- **ユーザーアカウントへの入金**：`confirmed`
  を待つ（ほとんどのトランザクションで安全）
- **物品の発送**：`finalized` を待つ
- **大口の出金**：`finalized` を待つ
- **コンプライアンス・監査**：常に `finalized` のステータスを記録する

トランザクションステータスの確認について詳しくは、
[Solanaとのインタラクション](/docs/payments/interacting-with-solana)を参照してください。

## エラー処理

Solana Kit は `isSolanaError()`
を通じて型付きエラーを提供します。文字列マッチングではなく、特定のエラーコードを使用してください：
...
| `INSUFFICIENT_FUNDS`         | トークン残高不足               | ユーザーの残高補充が必要                                                                                          |
| `ACCOUNT_NOT_FOUND`          | token account が存在しない     | トランザクション内でATAを作成                                                                                     |
...
- **フロントエンドコードに秘密鍵を公開しないでください。**
  バックエンド署名、ハードウェアウォレット、マルチシグウォレット、または鍵管理サービスを使用してください。
- **ホットウォレットとコールドウォレットを分離してください。**
  ホットウォレットは運用用、コールドウォレットは資金管理用に使用します。
- **すべての本番環境の鍵をバックアップしてください。**
  暗号化されたバックアップを複数の安全な場所に保存します。鍵を紛失すると、アクセスが永久に失われます。
- **devnetとmainnetで異なる鍵を使用してください。**
  devnet用の鍵をmainnet用の鍵として使用しないでください。環境ベースの設定を使用して、各ネットワークに適切な鍵が読み込まれるようにしてください。

### 署名インフラストラクチャ

本番環境のバックエンド署名には、[Keychain](/docs/tools/keychain)をご利用ください。これは、単一のインターフェースを通じて複数の鍵管理バックエンドを抽象化する統合署名ライブラリです。対応バックエンドは次のとおりです：Memory、Vault、Privy、Turnkey、AWS
KMS、Fireblocks、GCP
KMS、CDP、Para、Dfns、Crossmint、Openfort、Utila。これにより、インメモリ鍵を使用してローカルで開発し、アプリケーションコードを変更することなく本番グレードのバックエンドに切り替えることができます。

どのバックエンドが適しているかわからない場合は、
[バックエンドの選択](/docs/tools/keychain/choosing-a-backend)をご参照ください。Keychainは[Rust](https://crates.io/crates/solana-keychain)と
