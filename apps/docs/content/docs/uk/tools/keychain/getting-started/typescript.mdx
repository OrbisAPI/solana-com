...
`@solana/keychain-kit-plugin` встановлює підписувач ключового ланцюга
безпосередньо на [Kit client](https://github.com/anza-xyz/kit), підключаючи його
як `payer` та `identity`, щоб пропустити ручне налаштування `pipe`, показане
вище:
...
`createKeychainSigner`, і бундлюється лише той бекенд, до якого він звертається
(пакети бекендів завантажуються через динамічний `import()`).

Використовуйте `keychainPayer` або `keychainIdentity`, щоб встановити лише одну
роль, та поєднуйте бекенди на одному клієнті:
...
Вже маєте `SolanaSigner`? Кожен підписувач ключового ланцюга є дійсним Kit
...
## Створення власних підписувачів

Реалізуйте інтерфейс `SolanaSigner` для створення власних підписувачів:
...
Перегляньте
[посібник із додавання підписувачів](/docs/tools/keychain/adding-signers), щоб
