...
1. Створіть
   ...
   **Доказ діапазону
   ...
   знаходиться у зазначеному діапазоні.

2. Для кожного доказу:
   - Викличте програму доказів ZK ElGamal для перевірки даних доказу.
   - Збережіть специфічні метадані доказу в обліковому записі "контекстного
...
4. Закрийте облікові записи доказів, щоб повернути SOL, витрачений на їх
   створення.

Наступна діаграма показує кроки, пов'язані з виведенням токенів із
...
- Метод `confidential_transfer_create_context_state_account`, що створює
  обліковий запис доказу.
- Метод `confidential_transfer_withdraw`, що викликає інструкцію `Withdraw`.
- Метод `confidential_transfer_close_context_state_account`, що закриває
  обліковий запис доказу.
...
<Callout type="info">
  На момент написання цієї статті Confidential Transfers не було увімкнено у
  стандартному локальному validator. Щоб запустити приклад коду, необхідно
  клонувати mainnet Token Extension Program.
</Callout>
