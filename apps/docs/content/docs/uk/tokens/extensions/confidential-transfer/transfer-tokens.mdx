...
## Як конфіденційно переказати токени з одного token account в інший

Щоб конфіденційно переказати токени з одного token account в інший, як
відправник, так і отримувач повинні мати token account, налаштований зі станом
_rs`ConfidentialTransferAccount`_ та схвалений для конфіденційних переказів.
...
1. Створіть
   ...
   **Доказ рівності
   ...
   гарантуючи, що новий доступний баланс облікового запису джерела правильно
   обчислено як `new_balance = current_balance - transfer_amount`.
   ...
   **Доказ дійсності шифротексту
   ...
   сторін (джерело, отримувач та аудитор), гарантуючи, що сума переказу
   правильно зашифрована під відкритим ключем кожної сторони.
...
2. Для кожного доказу:
   - Викликати програму ZK ElGamal proof для перевірки даних доказу.
   - Зберегти специфічні метадані доказу в обліковому записі "context state" для
     використання в інших інструкціях.

3. Викликати інструкцію
...
4. Закрити облікові записи контексту стану доказів, щоб повернути SOL,
   використаний для їх створення.

Наступна діаграма показує кроки, задіяні у переказі токенів з token account
відправника до token account отримувача.
...
Щоб конфіденційно переказати токени з одного token account в інший, необхідно:
...
- Закрити три облікові записи доказів, щоб повернути rent.
...
клонованою з мережі mainnet, за допомогою наведеної нижче команди. Для запуску
...
<Callout type="info">
  ...
  локальному validator. Щоб запустити приклад коду, необхідно клонувати Token
  Extension Program з мережі mainnet.
</Callout>
