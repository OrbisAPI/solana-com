...
## Як перемістити токени на конфіденційний очікуваний баланс

Перш ніж токени можна буде передати конфіденційно, публічний баланс токенів
необхідно конвертувати в конфіденційний баланс. Ця конвертація відбувається у
два етапи:
...
У цьому розділі пояснюється перший етап: депонування публічного балансу токенів
на конфіденційний очікуваний баланс.

Наступна діаграма показує кроки, задіяні під час депонування токенів з
публічного балансу на конфіденційний очікуваний баланс:
...
на одну інструкцію депозиту обмежена значенням 2^48.

Крейт `spl_token_client` надає метод `confidential_transfer_deposit`, який
формує та надсилає транзакцію з інструкцією `Deposit`, як показано в прикладі
нижче.
...
локального validator необхідно мати встановлений Solana CLI.
...
<Callout type="info">
  На момент написання цього матеріалу Confidential Transfers не було увімкнено у
  стандартному локальному validator. Щоб запустити приклад коду, необхідно
  клонувати Token Extensions Program з mainnet.
</Callout>
