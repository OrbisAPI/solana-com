...
Конфіденційні перекази дозволяють переказувати токени між token accounts, не
розкриваючи суму переказу. Це корисно для транзакцій із захистом приватності.
Лише суми переказів і баланси токенів є приватними. Адреси token account
...
- [Cookbook конфіденційних балансів](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Фрагменти коду для використання розширення Confidential Transfer
...
до Token Extensions Program, що дозволяє переказувати токени між акаунтами, не
розкриваючи суму переказу.
...
1. Створіть mint account із розширенням конфіденційного переказу.
2. Створіть token accounts із розширенням конфіденційного переказу для
   відправника та одержувача.
3. Випустіть токени на акаунт відправника.
...
7. **Застосуйте** очікуваний баланс одержувача до **конфіденційного доступного
   балансу**.
8. **Виведіть** конфіденційний доступний баланс одержувача на **публічний
   баланс**.
...
<Cards>
  ...
  >
    Як створити mint account із розширенням Confidential Transfer
  </Card>
  <Card
    title="Створити Token Account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Як налаштувати token account із розширенням Confidential Transfer
  </Card>
  <Card
    title="Внести токени"
  ...
  >
    Як гаманці, браузери та біржі можуть підтримувати токени конфіденційних
    переказів
  </Card>
  <Card
  ...
  >
    ...
    підтвердження, аудитори, комісії, випуск і спалення)
  </Card>
</Cards>
...
Наведений нижче приклад на Rust демонструє кроки для конфіденційних переказів
токенів, що відповідають попередній діаграмі. Для зручності кроки розділені на
допоміжні функції.

Щоб запустити приклад, запустіть локальний validator із Token Extensions
Program, клонованою з mainnet, використовуючи наведену нижче команду. Для
використання локального validator необхідно встановити Solana CLI.
...
<Callout type="info">
  На момент написання цього матеріалу Confidential Transfers не було увімкнено в
  стандартному локальному validator. Для запуску прикладу коду необхідно
  клонувати Token Extension Program з mainnet.
</Callout>
...
| _rs`ApproveAccount`_                | Схвалює token account для конфіденційних переказів, якщо mint вимагає підтвердження для нових token account.                                                                     |
| _rs`EmptyAccount`_                  | Очищає очікуваний та доступний конфіденційні баланси для можливості закриття token account.                                                                                      |
...
| _rs`DisableConfidentialCredits`_    | Блокує вхідні конфіденційні перекази, дозволяючи при цьому публічні перекази.                                                                                                    |
...
| _rs`ConfigureAccountWithRegistry`_  | Альтернативний спосіб налаштування token account для конфіденційних переказів з використанням облікового запису _rs`ElGamalRegistry`_ замість доказу _rs`VerifyPubkeyValidity`_. |
