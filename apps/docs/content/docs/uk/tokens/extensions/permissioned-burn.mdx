...
запуску прикладів на цій сторінці завантажте програму v11.0.0 з devnet у ваш
тестовий validator:
...
2. Створіть mint account за допомогою _rs`CreateAccount`_, ініціалізуйте
   _rs`PermissionedBurnConfig`_ і ініціалізуйте mint за допомогою
   _rs`InitializeMint`_.
3. Створіть token account і відчеканіть токени.
4. Виконайте спалювання за допомогою
   _rs`PermissionedBurnInstruction::BurnChecked`_, підписаного як власником
   token account, так і адміністратором спалювання.
5. За бажанням вимкніть дозволене спалювання за допомогою _rs`SetAuthority`_,
   використовуючи _rs`AuthorityType::PermissionedBurn`_ і повноваження `None`,
   що повторно вмикає стандартні інструкції спалювання. Повні приклади коду
   нижче демонструють цей крок.
...
Створіть mint account із розрахованим простором і lamport.
...
Ініціалізуйте розширення _rs`PermissionedBurnConfig`_ на mint з адміністратором
спалювання.
...
## !!steps Створити token account та карбувати токени

Створіть token account для платника та відкарбуйте на нього токени.
...
## !!steps Спалення з повноваженнями на спалення

Спаліть токени за допомогою _rs`PermissionedBurnInstruction::BurnChecked`_,
підписаного як власником token account, так і уповноваженим на спалення.
...
| _rs`PermissionedBurnInstruction::Burn`_        | Інструкція спалення, що вимагає підписів від уповноваженого на спалення та власника або делегата token account.                    | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
...
| _rs`AuthorityType::PermissionedBurn`_          | Дискримінатор повноважень, що використовується з _rs`SetAuthority`_ для ротації або вимкнення повноважень на спалення для mint.    | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Логіка процесора, що ініціалізує _rs`PermissionedBurnConfig`_ на неініціалізованому mint та зберігає повноваження на спалення.     | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Логіка процесора, що відхиляє стандартні спалення, коли встановлено повноваження на спалення, та перевіряє підпис уповноваженого.  | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Логіка процесора, що перевіряє та виконує ротацію повноважень на спалення при використанні _rs`AuthorityType::PermissionedBurn`_.  | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |
