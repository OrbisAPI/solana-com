...
<Callout type="info">
  ...
  гаманця? Підписуйте за допомогою серверного сервісу керування ключами замість
  використання raw keypair. Дивіться [Keychain](/docs/tools/keychain) та
  [Підписання в продакшені](/docs/core/transactions/signing-in-production).
</Callout>

## Поширені патерни для розробників Solana

- **Headless-автомати станів**: Запускайте клієнт всередині API-маршрутів,
  скриптів або воркерів, щоб повторно використовувати ту саму логіку оркестрації
  гаманця + RPC, яка керує вашим UI.
- **Дашборди реального часу**: Поєднуйте спостерігачів (баланси, акаунти,
  підписи) з вашою бажаною UI-бібліотекою; клієнт керує розподілом websocket і
  інвалідацією кешу.
- **Кастомні сховища**: Підключіть власне сховище Zustand для гідрації з
  IndexedDB/localStorage, дзеркалювання стану в серверні сесії або координації
  між вкладками браузера.
- **Інтеграція з React-хуками**: Передайте налаштований екземпляр клієнта до
  `@solana/react-hooks`, якщо потрібні зручні хуки поверх того самого рантайму.
- **Тестованість**: Єдиний інтерфейс клієнта можна замокати в юніт-тестах, що
  дозволяє легко симулювати RPC-відповіді або сесії гаманця без браузерного
  гаманця.
