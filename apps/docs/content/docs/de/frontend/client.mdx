...
- **Headless-Zustandsmaschinen**: Führen Sie den Client in API-Routen, Skripten
  oder Workern aus, um dieselbe Wallet- und RPC-Orchestrierungslogik
  wiederzuverwenden, die Ihre Benutzeroberfläche antreibt.
- **Echtzeit-Dashboards**: Kombinieren Sie Beobachter (Salden, Konten,
...
- **Benutzerdefinierte Stores**: Injizieren Sie Ihren eigenen Zustand-Store, um
  aus IndexedDB/localStorage zu hydratisieren, den Zustand mit Server-Sitzungen
  zu spiegeln oder zwischen Browser-Tabs zu koordinieren.
- **Bridge zu React-Hooks**: Übergeben Sie eine konfigurierte Client-Instanz an
  `@solana/react-hooks`, wenn Sie ergonomische Hooks auf demselben Runtime
  aufbauen möchten.
