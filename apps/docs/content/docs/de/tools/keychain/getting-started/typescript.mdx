...
`identity` ein, sodass Sie das manuelle `pipe`-Setup wie oben beschrieben
überspringen können:
...
`createKeychainSigner`, und es wird nur das Backend gebündelt, an das es
weiterleitet (Backend-Pakete werden über dynamisches `import()` geladen).

Verwenden Sie `keychainPayer` oder `keychainIdentity`, um nur eine Rolle
festzulegen, und kombinieren Sie Backends auf einem einzelnen Client:
...
Haben Sie bereits einen `SolanaSigner`? Jeder Keychain-Signer ist ein gültiger
Kit- `TransactionSigner`, daher können Sie einen vorhandenen mit
...
Das `SolanaSigner` Interface erweitert `@solana/signers` Typen für vollständige
Kompatibilität:
...
Implementieren Sie das `SolanaSigner` Interface, um benutzerdefinierte Signer zu
erstellen:
...
[Leitfaden zum Hinzufügen von Signern](/docs/tools/keychain/adding-signers), um
