...
außerdem ein verfügbares vertrauliches Guthaben für die Übertragung aufweisen.
...
1. Erstellen Sie
   ...
   **Gleichheitsnachweis
   ...
   Überprüft, ob der neue verfügbare Guthaben-Geheimtext nach der Übertragung
   mit seiner entsprechenden
   ...
   **Geheimtext-Gültigkeitsnachweis
   ...
   Überprüft, ob die Übertragungsbetrag-Geheimtexte für alle drei Parteien
   (Quelle, Ziel und Prüfer) ordnungsgemäß generiert wurden, und stellt sicher,
   dass der Übertragungsbetrag unter dem öffentlichen Schlüssel jeder Partei
   ...
   **Bereichsnachweis
   ...
   Überprüft, ob das neue verfügbare Guthaben und der Übertragungsbetrag
   (aufgeteilt in Nieder-/Hochbits) alle nicht-negativ und innerhalb eines
   festgelegten Bereichs liegen.

2. Für jeden Nachweis:
   - Rufen Sie das ZK-ElGamal-Nachweisprogramm auf, um die Nachweisdaten zu
     verifizieren.
   - Speichern Sie die nachwesspezifischen Metadaten in einem
     Nachweis-"Kontextzustands"-Konten zur Verwendung in anderen Anweisungen.
...
3. Rufen Sie die
   ...
   Anweisungen auf und geben Sie die Nachweiskontextzustands-Konten an.

4. Schließen Sie die Nachweiskontextzustands-Konten, um das zur Erstellung
   verwendete SOL zurückzuerhalten.
...
- Einen Gleichheitsnachweis, einen Geheimtext-Gültigkeitsnachweis und einen
  Bereichsnachweis clientseitig generieren
- Das ZK-ElGamal-Nachweisprogramm aufrufen, um die Nachweise zu verifizieren und
  die "Kontextzustands"-Konten zu initialisieren
- Die
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  Anweisungen aufrufen und dabei die drei Nachweiskonten angeben.
- Die drei Nachweiskonten schließen, um rent zurückzuerhalten.

Das `spl_token_client` Crate bietet folgende Methoden:

- `confidential_transfer_create_context_state_account` Methode, die ein
  Proof-Konten erstellt.
- `confidential_transfer_transfer` Methode, die die `Transfer` Anweisungen
  aufruft.
- `confidential_transfer_close_context_state_account` Methode, die ein Proof-
  Konten schließt.

### Beispielcode

Das folgende Beispiel zeigt, wie Token vertraulich zwischen token account
übertragen werden.
...
Befehls. Sie müssen die Solana CLI installiert haben, um den lokalen validator
zu starten.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung ist Confidential Transfers beim standardmäßigen
  lokalen validator nicht aktiviert. Sie müssen das Mainnet Token Extensions
