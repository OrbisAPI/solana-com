...
übertragen, ohne den Transferbetrag preiszugeben. Dies ist nützlich für
...
Die Confidential Transfer-Erweiterung fügt dem Token Extensions Program
Anweisungen hinzu
...
die es ermöglichen, Token zwischen Konten zu übertragen, ohne den Transferbetrag
preiszugeben.
...
1. Erstelle ein mint account mit der Confidential Transfer-Erweiterung.
2. Erstelle token accounts mit der Confidential Transfer-Erweiterung für den
   Sender und den Empfänger.
3. Präge Token auf das Sender-Konten.
...
5. **Anwenden** des ausstehenden Guthabens des Senders auf das **vertrauliche
   verfügbare Guthaben**.
6. Vertrauliches **Übertragen** von Token vom Sender-token account zum
...
die dem vorangegangenen Diagramm entsprechen. Die Schritte sind zur besseren
...
Extensions Program, das von Mainnet geklont wurde, mithilfe des folgenden
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung dieser Dokumentation sind Confidential Transfers
  im standardmäßigen lokalen Validatoren nicht aktiviert. Sie müssen das Mainnet
...
| _rs`Withdraw`_                      | Konvertiert den verfügbaren vertraulichen Saldo zurück in einen öffentlichen Saldo.                                                                                                          |
| _rs`Transfer`_                      | Überträgt Token vertraulich zwischen token accounts.                                                                                                                                         |
| _rs`ApplyPendingBalance`_           | Konvertiert ausstehende Salden nach Einzahlungen oder Transfers in verfügbare Salden.                                                                                                        |
| _rs`EnableConfidentialCredits`_     | Ermöglicht einem token account, vertrauliche Token-Transfers zu empfangen.                                                                                                                   |
| _rs`DisableConfidentialCredits`_    | Blockiert eingehende vertrauliche Transfers und erlaubt weiterhin öffentliche Transfers.                                                                                                     |
| _rs`EnableNonConfidentialCredits`_  | Ermöglicht einem token account, öffentliche Token-Transfers zu empfangen.                                                                                                                    |
| _rs`DisableNonConfidentialCredits`_ | Blockiert reguläre Transfers, damit Konten ausschließlich vertrauliche Transfers empfangen.                                                                                                  |
| _rs`TransferWithFee`_               | Überträgt Token vertraulich mit einer Gebühr zwischen token accounts.                                                                                                                        |
| _rs`ConfigureAccountWithRegistry`_  | Alternativer Weg zur Konfiguration von token accounts für vertrauliche Transfers, wobei ein _rs`ElGamalRegistry`_-Konto anstelle eines _rs`VerifyPubkeyValidity`_-Nachweises verwendet wird. |
