...
So heben Sie Token aus dem vertraulichen verfügbaren Guthaben auf das
öffentliche Guthaben ab:
...
1. Erstellen Sie
   ...
   **Gleichheitsnachweis
   ...
   Überprüft, ob der verbleibende verfügbare Guthaben-Chiffretext nach der
   Abhebung mit dem entsprechenden
   [Pedersen-Commitment](https://en.wikipedia.org/wiki/Commitment_scheme)
   übereinstimmt und stellt sicher, dass das neue verfügbare Guthaben der Konten
...
2. Für jeden Nachweis:
   - Rufen Sie das ZK-ElGamal-Nachweisprogramm auf, um die Nachweisdaten zu
     verifizieren.
   - Speichern Sie die nachweissspezifischen Metadaten in einem
...
3. Rufen Sie die
   ...
   Anweisungen auf und übergeben Sie die beiden Nachweiskonten.

4. Schließen Sie die Nachweiskonten, um das zum Erstellen verwendete SOL
   zurückzuerhalten.
...
- Einen Gleichheits- und Bereichsnachweis auf der Client-Seite generieren
...
- Die
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  Anweisungen aufrufen und die beiden Nachweiskonten übergeben.
- Die beiden Nachweiskonten schließen, um rent zurückzuerhalten.

Das `spl_token_client`-Crate stellt folgende Methoden bereit:

- `confidential_transfer_create_context_state_account`-Methode, die ein
  Nachweiskonto erstellt.
- `confidential_transfer_withdraw`-Methode, die die `Withdraw` Anweisungen
  aufruft.
...
Das folgende Beispiel zeigt, wie Token aus dem vertraulichen verfügbaren
Guthaben auf das öffentliche Guthaben abgehoben werden.

Um das Beispiel auszuführen, starten Sie einen lokalen Validatoren mit dem Token
Extensions Program, der von Mainnet geklont wurde, mit dem folgenden Befehl. Sie
müssen die Solana CLI installiert haben, um den lokalen Validatoren zu starten.
