...
χρησιμοποιηθεί για την εκτίμηση των υπολογιστικών μονάδων που απαιτούνται για
μια συναλλαγή, η οποία περιλαμβάνει μια εκτίμηση των υπολογιστικών μονάδων που
θα χρησιμοποιηθούν. Το [`@solana/kit`](https://github.com/anza-xyz/kit) παρέχει
...
το όριο μεγέθους δεδομένων των φορτωμένων λογαριασμών.
...
<Callout type="info">
  Εάν δημιουργείτε και αποστέλλετε συναλλαγές με ένα πρόσθετο πελάτη kit,
  συνήθως δεν χρειάζεστε αυτό το βήμα — ο πελάτης προσθέτει οδηγίες
  υπολογιστικού προϋπολογισμού για εσάς κατά την αποστολή
  (`.sendTransaction()`). Η παραπάνω χειροκίνητη ροή αφορά όταν συναρμολογείτε
  συναλλαγές απευθείας με `@solana/kit`.
</Callout>

Στην παραγωγή, εάν επαναλαμβάνετε τον ίδιο τύπο συναλλαγής πολλές φορές, θα
πρέπει να εξετάσετε την αποθήκευση στην κρυφή μνήμη της εκτίμησης υπολογιστικών
μονάδων για τον τύπο συναλλαγής, ώστε να αποφύγετε την επιβάρυνση από την
εκτίμηση των υπολογιστικών μονάδων κάθε φορά.
...
για να δώσουν κίνητρο στο δίκτυο Jito να συμπεριλάβει τις συναλλαγές σας σε ένα
block.
...
### Στρατηγικές Επαναποστολής

Οι συναλλαγές μπορεί να αποτύχουν για πολλούς λόγους. Σε αντίθεση με τα
παραδοσιακά API πληρωμών που επιστρέφουν αμέσως επιτυχία/αποτυχία, οι συναλλαγές
blockchain απαιτούν παρακολούθηση επιβεβαίωσης.

**Βασικές έννοιες:**

- **Λήξη blockhash**: Οι συναλλαγές ισχύουν για ~150 blocks (~60-90
  δευτερόλεπτα)
- **Idempotency**: Η ίδια υπογεγραμμένη συναλλαγή πάντα παράγει την ίδια
  υπογραφή — η επαναποστολή είναι ασφαλής
- **Εκθετική αναμονή**: Αποφύγετε την υπερφόρτωση του δικτύου με γρήγορες
  επαναποστολές
...
την παρακολούθηση επιβεβαίωσης και την παρακολούθηση ύψους μπλοκ. Εκπέμπει
`SOLANA_ERROR__BLOCK_HEIGHT_EXCEEDED` όταν λήγει το blockhash της συναλλαγής,
σηματοδοτώντας ότι πρέπει να αναδομήσετε τη συναλλαγή με ένα νέο blockhash.

### Πρόσθετοι Πόροι

- [Οδηγός: Επιβεβαίωση & Λήξη Συναλλαγής](https://solana.com/developers/guides/advanced/confirmation)
...
| ----------- | ------------------------------- | ----------------------- | ---------------------------------- |
| `processed` | Σε μπλοκ, χωρίς ψηφοφορία ακόμα | Εκκρεμής εξουσιοδότηση  | Ενημερώσεις UI σε πραγματικό χρόνο |
| `confirmed` | Ψηφοφορία υπερπλειοψηφίας       | Εκκαθαρισμένα κεφάλαια  | **Οι περισσότερες πληρωμές**       |
...
- **Ενημερώσεις UI**: Εμφανίστε `processed` για άμεση ανατροφοδότηση ("Η πληρωμή
  υποβλήθηκε")
- **Πίστωση λογαριασμού χρήστη**: Αναμείνετε `confirmed` (ασφαλές για τις
  περισσότερες συναλλαγές)
- **Αποστολή φυσικών αγαθών**: Αναμείνετε `finalized`
- **Μεγάλες αναλήψεις**: Αναμείνετε `finalized`
- **Συμμόρφωση/έλεγχος**: Καταγράφετε πάντα την κατάσταση `finalized`

Για περισσότερες πληροφορίες σχετικά με τον έλεγχο κατάστασης συναλλαγής, δείτε
...
| ---------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| `BLOCK_HEIGHT_EXCEEDED`      | Λήξη blockhash           | Αναδόμηση με νέο blockhash                                                                                                          |
...
| `INSUFFICIENT_FUNDS`         | Ανεπαρκή token           | Ο χρήστης χρειάζεται περισσότερο υπόλοιπο                                                                                           |
| `ACCOUNT_NOT_FOUND`          | Λείπει token account     | Δημιουργία ATA στη συναλλαγή                                                                                                        |

## Συναλλαγές Χωρίς Χρέωση Gas

Οι χρήστες αναμένουν να πληρώνουν σε stablecoins, χωρίς να χρειάζεται να
αποκτούν SOL για τα έξοδα δικτύου. Οι συναλλαγές χωρίς χρέωση gas λύνουν αυτό το
πρόβλημα—παρόμοια με τον τρόπο που οι χρήστες του Venmo δεν σκέφτονται τα έξοδα
ACH. Δείτε
[Αφαίρεση Χρέωσης](/docs/payments/send-payments/payment-processing/fee-abstraction)
για πλήρη υλοποίηση.
...
- **Μην εκθέτετε ποτέ ιδιωτικά κλειδιά στον κώδικα frontend.** Χρησιμοποιείτε
  υπογραφή backend, hardware wallets, multisignature wallets ή υπηρεσίες
  διαχείρισης κλειδιών.
- **Διαχωρίστε τα hot και cold wallets.** Hot wallet για λειτουργίες, cold για
  το ταμείο.
- **Δημιουργείτε αντίγραφα ασφαλείας όλων των κλειδιών παραγωγής.** Αποθηκεύστε
  κρυπτογραφημένα αντίγραφα σε πολλαπλές ασφαλείς τοποθεσίες. Η απώλεια ενός
  κλειδιού σημαίνει μόνιμη απώλεια πρόσβασης.
- **Χρησιμοποιείτε διαφορετικά κλειδιά για devnet και mainnet.** Τα κλειδιά του
  devnet δεν πρέπει να είναι τα κλειδιά του mainnet. Χρησιμοποιείτε διαμόρφωση
  βάσει περιβάλλοντος για να διασφαλίσετε ότι φορτώνονται τα σωστά κλειδιά για
  κάθε δίκτυο.

### Υποδομή Υπογραφής

Για υπογραφή backend σε περιβάλλον παραγωγής, χρησιμοποιείτε το
...
αναπτύσσετε τοπικά με κλειδιά in-memory και στη συνέχεια να μεταβαίνετε σε
backends παραγωγικού επιπέδου χωρίς να αλλάζετε τον κώδικα της εφαρμογής.

Δεν είστε σίγουροι ποιο backend ταιριάζει; Δείτε
