...
Το `@solana/keychain-kit-plugin` εγκαθιστά απευθείας έναν υπογράφοντα keychain
σε έναν [Kit client](https://github.com/anza-xyz/kit), συνδέοντάς τον ως `payer`
και `identity`, ώστε να παραλείπετε τη χειροκίνητη ρύθμιση `pipe` που φαίνεται
παραπάνω:
...
`createKeychainSigner`, και μόνο το backend στο οποίο αποστέλλει φορτώνεται (τα
πακέτα backend φορτώνουν μέσω δυναμικού `import()`).

Χρησιμοποιήστε το `keychainPayer` ή το `keychainIdentity` για να ορίσετε μόνο
έναν ρόλο και να συνδυάσετε backends σε έναν μοναδικό client:
...
Έχετε ήδη έναν `SolanaSigner`; Κάθε υπογράφων keychain είναι έγκυρο Kit
`TransactionSigner`, οπότε μπορείτε να εγκαταστήσετε έναν υπάρχοντα με
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
αντί για αυτό (`pnpm add @solana/kit-plugin-signer`):
...
Δείτε τον [οδηγό Προσθήκης Signers](/docs/tools/keychain/adding-signers) για την
ενσωμάτωση πρόσθετων υπηρεσιών διαχείρισης κλειδιών.
