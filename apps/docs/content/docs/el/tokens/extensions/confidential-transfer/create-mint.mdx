...
### Κατάσταση Mint του Confidential Transfer
...
- **authority**: Ο λογαριασμός που έχει άδεια να αλλάζει τις ρυθμίσεις του
  confidential transfer για το mint και να εγκρίνει νέα confidential accounts
  εάν η αυτόματη έγκριση είναι απενεργοποιημένη.

- **auto_approve_new_accounts**: Όταν οριστεί σε true, οι χρήστες μπορούν να
  δημιουργούν token accounts με confidential transfers ενεργοποιημένα από
  προεπιλογή. Όταν είναι false, η authority πρέπει να εγκρίνει κάθε νέο token
  account πριν μπορεί να χρησιμοποιηθεί για confidential transfers.

- **auditor_elgamal_pubkey**: Ένας προαιρετικός ελεγκτής που μπορεί να
  αποκρυπτογραφεί τα ποσά μεταφοράς σε εμπιστευτικές συναλλαγές, παρέχοντας έναν
...
2. **Αρχικοποίηση της Επέκτασης Confidential Transfer**: Καλέστε την οδηγία
   ...
   του Token Extensions Program για τη διαμόρφωση της κατάστασης
   _rs`ConfidentialTransferMint`_ για το mint.

3. **Αρχικοποίηση Mint**: Καλέστε την οδηγία _rs`Instruction::InitializeMint`_
   του Token Extensions Program για την αρχικοποίηση της τυπικής κατάστασης
   mint.
...
όπως αποδεικνύεται στο παρακάτω παράδειγμα.
