...
<Callout type="info">
  ...
  请使用后端密钥管理服务进行签名，而非原始 keypair。请参阅
...
- **无头状态机**：在 API 路由、脚本或 Worker 中运行客户端，以复用驱动 UI 的相同钱包 +
  RPC 编排逻辑。
- **实时仪表盘**：将监听器（余额、账户、签名）与您偏好的 UI 库结合使用；客户端负责处理 WebSocket 扇出和缓存失效。
- **自定义存储**：注入您自己的 Zustand 存储，以从 IndexedDB/localStorage 中注水、将状态镜像到服务器会话，或在浏览器标签页之间进行协调。
- **与 React
  Hooks 桥接**：当您希望在相同运行时之上使用符合人体工程学的 Hooks 时，将已配置的客户端实例传递给
  `@solana/react-hooks`。
- **可测试性**：单一客户端接口可在单元测试中被模拟，从而无需浏览器钱包即可轻松模拟 RPC 响应或钱包会话。
