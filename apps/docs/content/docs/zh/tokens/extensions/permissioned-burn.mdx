...
本地测试 validator 还捆绑了一个较旧的 Token-2022 版本，因此要运行本页上的示例，请将 devnet 中的 v11.0.0 程序加载到您的测试 validator 中：
...
1. 计算 mint account 的大小以及 mint 和 _rs`PermissionedBurnConfig`_
   扩展所需的 rent。
2. 使用 _rs`CreateAccount`_ 创建 mint account，初始化
   _rs`PermissionedBurnConfig`_，并使用 _rs`InitializeMint`_ 初始化 mint。
3. 创建 token account 并铸造代币。
4. 使用 _rs`PermissionedBurnInstruction::BurnChecked`_ 进行销毁，需由 token
   account 所有者和销毁授权方共同签名。
5. 可选择使用 _rs`AuthorityType::PermissionedBurn`_ 和 `None` 授权方，通过
   _rs`SetAuthority`_
...
计算基础 mint 加上 _rs`PermissionedBurnConfig`_ 扩展的 mint account 大小。这是在
...
使用 mint 加上 _rs`PermissionedBurnConfig`_ 扩展所需的大小来计算 rent。
...
使用计算所得的空间和 lamport 创建 mint account。
...
在 mint 上使用销毁授权方初始化 _rs`PermissionedBurnConfig`_ 扩展。
...
使用由 token account 所有者和销毁权限共同签名的
_rs`PermissionedBurnInstruction::BurnChecked`_ 销毁代币。
...
| 条目                                           | 描述                                                                                  | 源码                                                                                                                                         |
| ---------------------------------------------- | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Mint 扩展，存储对该 mint 的每次销毁操作进行联署所需的权限。                           | [源码](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | 在 _rs`InitializeMint`_ 之前初始化受权限保护的销毁配置的指令。                        | [源码](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | 需要销毁权限以及 token account 所有者或委托人签名的销毁指令。                         | [源码](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | 带有小数位检查的销毁指令，使用与 _rs`PermissionedBurnInstruction::Burn`_ 相同的账户。 | [源码](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | 与 _rs`SetAuthority`_ 配合使用的权限标识符，用于轮换或禁用 mint 上的销毁权限。        | [源码](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
...
| _rs`process_burn`_                             | 当销毁权限已设置时拒绝标准销毁操作，并验证权限签名的处理器逻辑。                      | [源码](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | 在使用 _rs`AuthorityType::PermissionedBurn`_ 时验证并轮换销毁权限的处理器逻辑。       | [源码](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |
