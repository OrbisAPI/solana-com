...
1. 在客户端创建
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证提款后剩余可用余额密文与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)
   相匹配，确保账户新的可用余额被正确计算为
   `remaining_balance = current_balance - withdraw_amount`。

   **范围证明 ([BatchedRangeProofU64Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u64.rs#L37))**：验证提款后剩余可用余额为非负数且在指定范围内。
...
4. 关闭证明账户以回收创建时使用的 SOL。

下图展示了将代币从机密可用余额提取到公开余额的步骤：
...
以下示例演示如何将代币从机密可用余额提取到公开余额。

要运行示例，请使用以下命令启动一个从主网克隆了 Token Extensions
...
<Callout type="info">
  在撰写本文时，默认的本地 validator 尚未启用机密转账功能。您必须克隆主网的
