...
隐私转账使您能够在 token
account 之间转移代币，而无需披露转账金额。这对于注重隐私保护的交易非常有用。只有转账金额和代币余额是私密的，token
account 地址仍然是公开的。

- [协议概述](https://www.solana-program.com/docs/confidential-balances/overview) - 底层密码学协议的详细说明
- [快速入门指南](https://www.solana-program.com/docs/confidential-balances#setup) - 配置步骤与基础 CLI 命令
- [隐私余额示例手册](https://github.com/solana-developers/Confidential-Balances-Sample) - 如何使用隐私转账扩展的代码片段

### 它是如何工作的？

隐私转账扩展为 Token Extensions Program 添加了
...
1. 创建带有隐私转账扩展的 mint account。
2. 为发送方和接收方创建带有隐私转账扩展的 token account。
3. 将代币铸造到发送方账户。
4. 将发送方的公开余额**存入\*\***隐私待处理余额\*\*。
5. 将发送方的待处理余额**应用**至**隐私可用余额**。
6. 将代币从发送方 token account 隐私**转账**至接收方 token account。
7. 将接收方的待处理余额**应用**至**隐私可用余额**。
8. 将接收方的隐私可用余额**提取**至**公开余额**。

有关隐私转账流程中各步骤的更多详情，请参阅对应页面：
...
以下 Rust 示例展示了与前述流程图对应的保密代币转账步骤。为便于理解，各步骤被拆分为独立的辅助函数。

要运行此示例，请使用以下命令启动一个从主网克隆了 Token Extensions
Program 的本地 validator。您需要安装 Solana CLI 才能使用本地 validator。
...
<Callout type="info">
  在撰写本文时，默认本地 validator 上尚未启用保密转账功能。您必须克隆主网 Token
...
保密转账扩展的完整[指令](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)列表如下：

| 指令                                | 描述                                                                                                           |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 为 mint account 设置保密转账配置。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令包含在同一笔交易中。 |
| _rs`UpdateMint`_                    | 更新 mint 的保密转账设置。                                                                                     |
| _rs`ConfigureAccount`_              | 为 token account 设置保密转账配置。                                                                            |
| _rs`ApproveAccount`_                | 若 mint 要求对新 token account 进行审批，则批准该 token account 进行保密转账。                                 |
| _rs`EmptyAccount`_                  | 清空待处理和可用的保密余额，以允许关闭 token account。                                                         |
| _rs`Deposit`_                       | 将公开代币余额转换为待处理的保密余额。                                                                         |
| _rs`Withdraw`_                      | 将可用的保密余额转换回公开余额。                                                                               |
| _rs`Transfer`_                      | 在 token account 之间以保密方式转移代币。                                                                      |
| _rs`ApplyPendingBalance`_           | 在存款或转账后，将待处理余额转换为可用余额。                                                                   |
| _rs`EnableConfidentialCredits`_     | 允许 token account 接收保密代币转账。                                                                          |
| _rs`DisableConfidentialCredits`_    | 阻止传入的保密转账，同时仍允许公开转账。                                                                       |
| _rs`EnableNonConfidentialCredits`_  | 允许 token account 接收公开代币转账。                                                                          |
| _rs`DisableNonConfidentialCredits`_ | 阻止常规转账，使账户仅接收保密转账。                                                                           |
| _rs`TransferWithFee`_               | 在 token account 之间以保密方式转移代币，并附带手续费。                                                        |
| _rs`ConfigureAccountWithRegistry`_  | 使用 _rs`ElGamalRegistry`_ 账户代替 _rs`VerifyPubkeyValidity`_ 证明，为 token account 配置保密转账的替代方式。 |
