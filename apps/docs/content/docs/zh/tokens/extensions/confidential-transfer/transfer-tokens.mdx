...
要从一个 token account 向另一个 token
account 进行机密代币转账，发送方和接收方都必须拥有配置了
_rs`ConfidentialTransferAccount`_ 状态并已获批准用于机密转账的 token
account。发送方的 token account 还必须具有可用的机密余额以供转账。
...
1. 在客户端创建
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证转账后新的可用余额密文与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)
   相匹配，确保来源账户的新可用余额被正确计算为
   `new_balance = current_balance - transfer_amount`。

   **密文有效性证明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**：验证转账金额密文已为三方（来源方、目标方和审计方）正确生成，确保转账金额在各方公钥下均被正确加密。

   **范围证明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**：验证新的可用余额与转账金额（按高/低位拆分）均为非负数且在指定范围内。
...
要从一个 token account 向另一个 token account 进行机密代币转账，您必须：
...
以下示例演示如何在 token account 之间进行机密代币转账。

要运行此示例，请使用以下命令启动一个本地 validator，并从主网克隆 Token
Extensions Program。您必须安装 Solana CLI 才能启动本地 validator。
