...
在代币能够以机密方式转账之前，公开代币余额必须转换为机密余额。此转换分两个阶段进行：

1. **机密待处理余额**：首先，代币从公开余额"存入" 到"待处理"机密余额。
2. **机密可用余额**：待处理余额随后被"应用"到可用余额，使代币可用于机密转账。

本节说明第一阶段：将公开代币余额存入机密待处理余额。

下图展示了将代币从公开余额存入机密待处理余额所涉及的步骤：
...
方法，该方法构建并发送包含 `Deposit` 指令的交易，如下方示例所示。

### 示例代码

以下示例演示如何将公开代币余额存入机密待处理余额。

要运行该示例，请使用以下命令启动一个从主网克隆了 Token Extensions
Program 的本地 validator。您必须已安装 Solana CLI 才能启动本地 validator。
...
<Callout type="info">
  在撰写本文时，默认本地 validator 尚未启用机密转账功能。您必须克隆主网 Token
