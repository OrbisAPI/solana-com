...
- **authority**：有权更改 mint 的机密转账设置的账户，以及在禁用自动审批时批准新机密账户的账户。
...
- **auditor_elgamal_pubkey**：一个可选的审计员，可以解密机密交易中的转账金额，在对公众保持隐私的同时提供合规机制。

### 所需指令

创建启用机密转账的 mint 需要在单笔交易中包含三条指令：
...
以下代码演示了如何使用 Confidential Transfer 扩展创建铸币账户。
...
<Callout type="info">
  在撰写本文时，默认本地 validator 尚未启用 Confidential
  Transfers。您必须克隆主网的 Token Extensions Program 才能运行示例代码。
</Callout>
