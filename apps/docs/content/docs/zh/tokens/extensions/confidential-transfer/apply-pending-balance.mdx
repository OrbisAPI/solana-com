...
在代币可以进行保密转账之前，公开代币余额必须转换为保密余额。此转换分两个阶段进行：

1. **保密待处理余额**：首先，代币从公开余额"存入"到"待处理"的保密余额。
2. **保密可用余额**：待处理余额随后被"应用"到可用余额，使代币可用于保密转账。

本节介绍第二阶段：将待处理余额应用于可用余额。

当代币从公开余额"存入"，或代币从一个token account保密转账到另一个token
account时，代币最初会被添加到保密待处理余额中。在代币可用于保密转账之前，待处理余额必须被"应用"到可用余额。
...
方法，用于构建并发送包含 `ApplyPendingBalance` 指令的交易，如下方示例所示。

### 示例代码

以下示例演示如何将保密待处理余额应用于保密可用余额。

要运行此示例，请使用以下命令启动一个本地 validator，并从主网克隆 Token
Extensions Program。启动本地 validator 需要安装 Solana CLI。
...
<Callout type="info">
  在撰写本文时，默认本地 validator 尚未启用保密转账功能。您必须克隆主网 Token
