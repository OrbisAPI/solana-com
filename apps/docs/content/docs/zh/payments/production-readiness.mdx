...
提供了辅助函数，可一步估算交易的资源限制并将其设置在消息中（底层使用
`simulatetransaction`
...
在生产环境中，如果您多次重复执行相同类型的交易，应考虑缓存该交易类型的计算估算结果，以避免每次都需要估算计算单元所带来的额外开销。

### Jito 捆绑包

Jito 捆绑包是一种用于管理多笔交易原子性执行的工具。其实现方式是向 Jito 网络发送多笔附带小费的交易。小费可用于激励 Jito 网络将您的交易打包进区块。

**相关资源：**

- [Jito 文档](https://docs.jito.wtf/lowlatencytxnsend/)
- [🎥 视频：Jito 捆绑包](https://www.youtube.com/watch?v=HZ1pK9i6zx4)
- [QuickNode 关于 Jito 捆绑包的指南](https://www.quicknode.com/guides/solana-development/transactions/jito-bundles)

### 重试策略

交易可能因多种原因而失败。与传统支付 API 会立即返回成功或失败不同，区块链交易需要追踪确认状态。

**核心概念：**

- **区块哈希过期**：交易有效期约为 150 个区块（约 60–90 秒）
- **幂等性**：同一笔已签名的交易始终生成相同的签名——重复提交是安全的
- **指数退避**：避免因频繁重试而使网络不堪重负
...
`@solana/kit`，自动处理确认轮询和区块高度跟踪。当交易的 blockhash 过期时，它会抛出
...
- [Helius：如何在 Solana 上完成交易](https://www.helius.dev/blog/how-to-land-transactions-on-solana)
- [QuickNode：优化 Solana 交易的策略](https://www.quicknode.com/docs/solana/transactions)

## 理解确认级别

Solana 提供三种确认级别。以传统金融术语来说：

| 级别        | Solana 定义            | 传统对应概念 | 使用场景             |
| ----------- | ---------------------- | ------------ | -------------------- |
| `processed` | 已打包进区块，尚未投票 | 待授权       | 实时 UI 更新         |
| `confirmed` | 超级多数已投票         | 资金已清算   | **大多数支付场景**   |
| `finalized` | 已生根，不可逆         | 资金已结算   | 高价值交易、合规场景 |

**各级别适用场景：**

- **UI 更新**：显示 `processed` 以获得即时反馈（"支付已提交"）
- **入账用户账户**：等待 `confirmed`（适用于大多数交易）
- **发货实物商品**：等待 `finalized`
- **大额提款**：等待 `finalized`
...
| `INSUFFICIENT_FUNDS_FOR_FEE` | SOL 余额不足         | 为付费账户充值或使用[手续费抽象](/docs/payments/send-payments/payment-processing/fee-abstraction) |
| `INSUFFICIENT_FUNDS`         | 代币余额不足         | 用户需要增加余额                                                                                  |
| `ACCOUNT_NOT_FOUND`          | token account 不存在 | 在交易中创建 ATA                                                                                  |

## 无 Gas 交易

用户希望使用稳定币付款，而不是为支付网络费用而获取 SOL。无 Gas 交易解决了这一问题——类似于 Venmo 用户无需考虑 ACH 费用。完整实现方案请参阅
[费用抽象](/docs/payments/send-payments/payment-processing/fee-abstraction)。
...
- **隔离热钱包与冷钱包。** 热钱包用于日常操作，冷钱包用于资金储备。
- **备份所有生产环境密钥。**
  将加密备份存储于多个安全位置。一旦丢失密钥，将永久失去访问权限。
- **为开发网和主网使用不同的密钥。**
  开发网密钥不应与主网密钥相同。使用基于环境的配置，确保每个网络加载正确的密钥。
...
KMS、Fireblocks、GCP
KMS、CDP、Para、Dfns、Crossmint、Openfort 和 Utila。这让您可以在本地使用内存密钥进行开发，之后无需修改应用代码即可切换到生产级后端。

不确定哪种后端最适合您？请参阅
[选择后端](/docs/tools/keychain/choosing-a-backend)。Keychain 同时支持
