...
### 使用 Kit 客户端（keychainSigner 插件）

`@solana/keychain-kit-plugin` 可直接在
[Kit 客户端](https://github.com/anza-xyz/kit)上安装密钥链签名器，并将其自动配置为
...
该插件接受与 `createKeychainSigner`
相同的后端标记配置，且仅打包其实际调度的后端（后端包通过动态 `import()` 加载）。

使用 `keychainPayer` 或 `keychainIdentity`
仅设置单一角色，并在同一客户端上混合使用多个后端：
...
请参阅[添加签名器指南](/docs/tools/keychain/adding-signers)，了解如何集成其他密钥管理服务。
