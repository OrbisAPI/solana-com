...
每个签名由私钥生成。该密钥的存储位置——本地 keypair、云端 HSM 或 KMS，还是托管钱包服务——属于生产环境的设计决策。请参阅
[生产环境中的签名](/docs/core/transactions/signing-in-production)。

数组中的第一个签名属于**手续费支付方**，即支付交易
[基础费用和优先级费用](/docs/core/fees/fee-structure#base-fee)的账户。该第一个签名同时作为**交易 ID**，用于在网络上查询该交易。交易 ID 通常也称为
**交易签名**。

手续费支付方的要求：

- 必须是消息中的第一个账户（索引 0）且为签名者。
- 必须是归属于系统程序的账户或 nonce 账户（由
...
字段是一个紧凑编码的公钥数组。每个条目标识交易指令中至少使用过一次的账户。该数组必须包含所有账户，并遵循以下严格的排列顺序：
...
<Callout>
  这种严格的排序允许将 `account_keys` 数组与消息 [`header`](#header)
  中的三个计数相结合，从而在无需存储每个账户元数据标志的情况下确定每个账户的权限。头部计数将数组划分为上述四个权限组。
</Callout>
...
区块哈希在 150 个 slot 后过期。如果交易到达时区块哈希已不再有效，则会以
_rs`BlockhashNotFound`_
错误被拒绝，除非它是有效的[持久化 nonce 交易](/docs/core/transactions/durable-nonces)。
...
交易使用紧凑编码方案进行序列化。所有可变长度数组（签名、账户密钥、指令）均以紧凑 u16 长度编码为前缀。此格式对 0-127 的值使用 1 个字节，对较大的值使用 2-3 个字节。
...
Program 扣除 lamport。发送方和接收方账户都必须是可写的，以便其 lamport 余额能够发生变化。为执行此指令，发送方的钱包会发送包含其签名和含有 SOL 转账指令的消息的交易。
...
[token mint](/docs/tokens/basics/create-mint)、程序或您无法控制的
[PDA](/docs/core/pda) 发送 SOL **存在永久丢失资金的风险**
...
Program 钱包（或未充值的[链上地址](/docs/payments/send-payments/verify-address#on-curve-addresses)）发送转账；拒绝向您不控制的铸币账户、token
account、程序或 PDA 发送。
...
<Callout>
  ...
  代币发送，包括 token account、ATA 及
  Token-2022），请参阅[验证地址](/docs/payments/send-payments/verify-address)。
</Callout>
...
<Callout>
  您也可以通过 [Solana Explorer](https://explorer.solana.com) 查询该交易。
</Callout>
