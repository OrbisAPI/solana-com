...
Lokalny test validator zawiera również starszą kompilację Token-2022, dlatego
aby uruchomić przykłady z tej strony, załaduj program v11.0.0 z devnet do
swojego test validatora:
...
## Jak tworzyć mint account i spalać tokeny z uprawnieniami

Aby utworzyć mint account z uprawnionym spalaniem i spalić tokeny:

1. Oblicz rozmiar mint account oraz rent potrzebny dla mint account i
   rozszerzenia _rs`PermissionedBurnConfig`_.
2. Utwórz mint account za pomocą _rs`CreateAccount`_, zainicjuj
   _rs`PermissionedBurnConfig`_, a następnie zainicjuj mint account za pomocą
   _rs`InitializeMint`_.
3. Utwórz token account i mintuj tokeny.
4. Spalaj za pomocą _rs`PermissionedBurnInstruction::BurnChecked`_ podpisanego
   zarówno przez właściciela token account, jak i przez upoważnienie do
   spalania.
5. Opcjonalnie wyłącz uprawnione spalanie za pomocą _rs`SetAuthority`_, używając
   _rs`AuthorityType::PermissionedBurn`_ i uprawnień `None`, co ponownie włącza
...
Oblicz rozmiar mint account dla podstawowego mint account plus rozszerzenie
...
Oblicz rent używając rozmiaru potrzebnego dla mint account plus rozszerzenie
...
## !!steps Zainicjuj PermissionedBurn

Zainicjuj rozszerzenie _rs`PermissionedBurnConfig`_ na mint account z
upoważnieniem do spalania.
...
## !!steps Zainicjuj mint account

Zainicjuj mint account za pomocą _rs`InitializeMint`_ w tej samej transakcji.
...
## !!steps Spalaj z uprawnieniem spalania

Spalaj tokeny za pomocą _rs`PermissionedBurnInstruction::BurnChecked`_
podpisanego zarówno przez właściciela token account, jak i przez organ
uprawniony do spalania.
...
| Element                                        | Opis                                                                                                                                    | Źródło                                                                                                                                         |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Rozszerzenie mint przechowujące uprawnienie wymagane do współpodpisywania każdego spalenia dla danego mint.                             | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
...
| _rs`PermissionedBurnInstruction::Burn`_        | Instrukcja spalania wymagająca podpisów od organu uprawnionego do spalania oraz właściciela lub delegata token account.                 | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
...
| _rs`AuthorityType::PermissionedBurn`_          | Dyskryminator uprawnień używany razem z _rs`SetAuthority`_ do rotacji lub wyłączenia organu uprawnionego do spalania w mint.            | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Logika procesora inicjalizująca _rs`PermissionedBurnConfig`_ na niezainicjalizowanym mint i przechowująca organ uprawniony do spalania. | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Logika procesora odrzucająca standardowe spalenia, gdy organ uprawniony do spalania jest ustawiony, i weryfikująca podpis tego organu.  | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logika procesora walidująca i rotująca organ uprawniony do spalania, gdy używany jest _rs`AuthorityType::PermissionedBurn`_.            | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### TypeScript

Poniższy przykład `Kit` używa wygenerowanych instrukcji bezpośrednio.
Opublikowany starszy pakiet `@solana/spl-token` nie zawiera jeszcze obsługi
permissioned burn, dlatego nie dołączono przykładu dla starszej wersji.
