...
2. Dla każdego dowodu:
   - Wywołaj program dowodów ZK ElGamal w celu weryfikacji danych dowodu.
   - Zapisz metadane specyficzne dla dowodu na koncie "stanu kontekstu" dowodu,
     aby wykorzystać je w innych instrukcjach.
...
3. Wywołaj instrukcję
   ...
   podając dwa konta dowodów.

4. Zamknij konta dowodów, aby odzyskać SOL użyty do ich utworzenia.
...
- Wywołać instrukcję
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  podając dwa konta dowodów.
- Zamknąć dwa konta dowodów, aby odzyskać rent.
...
- Metoda `confidential_transfer_create_context_state_account` tworzy konto
  dowodu.
- Metoda `confidential_transfer_withdraw` wywołuje instrukcję `Withdraw`.
...
sklonowanym z sieci mainnet, używając poniższego polecenia. Musisz mieć
...
<Callout type="info">
  ...
  lokalnym validatorze. Musisz sklonować mainnetowy Token Extensions Program,
