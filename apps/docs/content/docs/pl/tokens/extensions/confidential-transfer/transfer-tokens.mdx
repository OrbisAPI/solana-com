...
jak i odbiorca muszą posiadać token account skonfigurowane ze stanem
_rs`ConfidentialTransferAccount`_ i zatwierdzone do poufnych transferów. Token
account nadawcy musi również posiadać dostępne poufne saldo do przekazania.
...
1. Utwórz
   ...
   **Dowód równości
   ...
   Weryfikuje, że nowy zaszyfrowany tekst dostępnego salda po transferze
   odpowiada swojemu powiązanemu
   [zobowiązaniu Pedersena](https://en.wikipedia.org/wiki/Commitment_scheme),
   zapewniając, że nowe dostępne saldo konta źródłowego jest poprawnie obliczane
   ...
   **Dowód ważności szyfrogramu
   ...
   transferu jest poprawnie zaszyfrowana przy użyciu klucza publicznego każdej
   ze stron.
...
2. Dla każdego dowodu:
   - Wywołaj program dowodów ZK ElGamal w celu zweryfikowania danych dowodowych.
   - Zapisz metadane specyficzne dla danego dowodu w koncie "stanu kontekstu"
...
Poniższy diagram przedstawia kroki związane z transferem tokenów z token account
nadawcy do token account odbiorcy.
...
Aby poufnie przenieść tokeny z jednego token account na inny, musisz:

- Wygenerować dowód równości, dowód ważności szyfrogramu oraz dowód zakresu po
  stronie klienta
...
- Metoda `confidential_transfer_create_context_state_account` tworzy konto
  dowodowe.
- Metoda `confidential_transfer_transfer` wywołuje instrukcję `Transfer`.
- Metoda `confidential_transfer_close_context_state_account` zamyka konto
  dowodowe.

### Przykładowy kod

Poniższy przykład ilustruje, jak poufnie transferować tokeny między token
account.

Aby uruchomić przykład, uruchom lokalny validator z Token Extensions Program
sklonowanym z mainnetu, używając poniższego polecenia. Aby uruchomić lokalny
...
<Callout type="info">
  ...
  sklonować mainnetowy Token Extensions Program.
</Callout>
