...
musi zostać przekonwertowane na saldo poufne. Konwersja odbywa się w dwóch
etapach:
...
W tej sekcji opisano pierwszy etap: wpłacanie publicznego salda tokenów na
poufne saldo oczekujące.
...
Aby przekonwertować saldo publiczne na poufne saldo oczekujące, należy wywołać
instrukcję
...
która buduje i wysyła transakcję z instrukcją `Deposit`, jak przedstawiono w
...
sklonowanym z sieci głównej za pomocą poniższego polecenia. Aby uruchomić
lokalny validator, musi być zainstalowany Solana CLI.
...
<Callout type="info">
  ...
  lokalnym validatorze. Aby uruchomić przykładowy kod, musisz sklonować Token
  Extensions Program z sieci głównej.
</Callout>
