...
zachowujących prywatność. Prywatne są wyłącznie kwoty transferów i salda
tokenów. Adresy token accounts pozostają publiczne.
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Fragmenty kodu pokazujące, jak korzystać z rozszerzenia Confidential Transfer
...
Aby uzyskać więcej informacji na temat kroków w przepływie poufnego transferu,
...
<Cards>
  ...
  <Card
    title="Przewodnik emitenta"
    href="/docs/tokens/extensions/confidential-transfer/issuer-guide"
  >
    Jak emitować tokeny z poufnym transferem i zarządzać nimi (polityka
...
Poniższy przykład w języku Rust przedstawia kroki dotyczące poufnych przelewów
tokenów odpowiadające poprzedniemu diagramowi. Kroki są rozdzielone na funkcje
pomocnicze dla zachowania przejrzystości.

Aby uruchomić przykład, uruchom lokalny validator z Token Extensions Program
sklonowanym z sieci głównej za pomocą poniższego polecenia. Aby korzystać z
lokalnego validatora, musisz mieć zainstalowane Solana CLI.
...
<Callout type="info">
  W chwili pisania tego tekstu, Poufne Transfery nie są włączone w domyślnym
  lokalnym validatorze. Aby uruchomić przykładowy kod, musisz sklonować
  mainnetowy Token Extensions Program.
</Callout>
...
| _rs`DisableConfidentialCredits`_    | Blokuje przychodzące poufne transfery, nadal zezwalając na publiczne transfery.                                                                               |
| _rs`EnableNonConfidentialCredits`_  | Umożliwia token account odbieranie publicznych transferów tokenów.                                                                                            |
| _rs`DisableNonConfidentialCredits`_ | Blokuje zwykłe transfery, aby konto odbierało wyłącznie poufne transfery.                                                                                     |
