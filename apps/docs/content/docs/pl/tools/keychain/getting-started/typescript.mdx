...
### Użycie z klientem Kit (wtyczka keychainSigner)

`@solana/keychain-kit-plugin` instaluje podpisującego z pęku kluczy bezpośrednio
na [kliencie Kit](https://github.com/anza-xyz/kit), podłączając go jako `payer`
i `identity`, dzięki czemu pomijasz ręczną konfigurację `pipe` przedstawioną
powyżej:
...
Wtyczka przyjmuje tę samą konfigurację z tagiem backendu co
`createKeychainSigner`, a bundlowany jest tylko ten backend, do którego
przekierowuje (pakiety backendów ładowane są przez dynamiczny `import()`).

Użyj `keychainPayer` lub `keychainIdentity`, aby przypisać tylko jedną rolę i
łączyć backendy w ramach jednego klienta:
...
Masz już `SolanaSigner`? Każdy podpisujący z pęku kluczy jest prawidłowym
`TransactionSigner` dla Kita, więc możesz zainstalować istniejący za pomocą
...
Interfejs `SolanaSigner` rozszerza typy `@solana/signers` dla pełnej
kompatybilności:
...
Zaimplementuj interfejs `SolanaSigner`, aby tworzyć niestandardowe signery:
...
Zapoznaj się z
[przewodnikiem dodawania signerów](/docs/tools/keychain/adding-signers), aby
