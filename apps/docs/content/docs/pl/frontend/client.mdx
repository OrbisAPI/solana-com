...
<Callout type="info">
  Uruchamiasz w trybie bezgłowym — w trasie API, workerze lub skrypcie bez
  portfela przeglądarki? Zamiast surowego keypair użyj backendowej usługi
  zarządzania kluczami. Zobacz [Keychain](/docs/tools/keychain) oraz
  [Podpisywanie w Środowisku
...
- **Dashboardy czasu rzeczywistego**: Połącz obserwatory (salda, konta,
  sygnatury) z preferowaną biblioteką UI; klient obsługuje rozgałęzianie
  websocketów i unieważnianie pamięci podręcznej.
- **Własne store'y**: Wstrzyknij własny store Zustand, aby hydratować dane z
  IndexedDB/localStorage, synchronizować stan z sesjami serwera lub koordynować
  pracę między zakładkami przeglądarki.
- **Most do hooków React**: Przekaż skonfigurowaną instancję klienta do
  `@solana/react-hooks`, gdy chcesz ergonomicznych hooków na szczycie tego
  samego środowiska uruchomieniowego.
- **Testowalność**: Jednolity interfejs klienta można mockować w testach
  jednostkowych, co ułatwia symulowanie odpowiedzi RPC lub sesji portfela bez
  konieczności posiadania portfela przeglądarki.
