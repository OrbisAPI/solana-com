...
로컬 테스트 validator에는 이전 버전의 Token-2022 빌드도 포함되어 있습니다.
따라서 이 페이지의 예제를 실행하려면 devnet에서 v11.0.0 프로그램을 테스트
validator에 로드하세요:
...
권한 있는 소각 기능이 있는 mint를 생성하고 토큰을 소각하려면:
...
5. 선택적으로 *rs`AuthorityType::PermissionedBurn`*와 `None` 권한자를 사용하여
   *rs`SetAuthority`*로 권한 있는 소각을 비활성화할 수 있으며, 이렇게 하면 표준
   소각 명령어가 다시 활성화됩니다. 아래의 전체 코드 예제에서 이 단계를 확인할
   수 있습니다.
...
기본 mint와 _rs`PermissionedBurnConfig`_ 확장을 포함한 mint account 크기를
...
소각 권한자를 지정하여 mint에서 _rs`PermissionedBurnConfig`_ 확장을
초기화합니다.
...
지급자를 위한 token account를 생성하고 토큰을 발행합니다.
...
token account 소유자와 소각 권한자 모두의 서명으로
*rs`PermissionedBurnInstruction::BurnChecked`*를 사용하여 토큰을 소각합니다.
...
<Callout type="info" title="명령어 순서">
  _rs`PermissionedBurnInstruction::Initialize`_는 _rs`InitializeMint`_ 앞에 와야
  합니다. _rs`CreateAccount`_, _rs`PermissionedBurnInstruction::Initialize`_,
  _rs`InitializeMint`_는 반드시 동일한 트랜잭션에 포함되어야 합니다.
</Callout>
...
| 항목                                           | 설명                                                                                                      | 소스                                                                                                                                         |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | 해당 mint의 모든 소각에 공동 서명하는 데 필요한 권한을 저장하는 mint 확장.                                | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | _rs`InitializeMint`_ 이전에 허가된 소각 설정을 초기화하는 명령어.                                         | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | 소각 권한자와 token account 소유자 또는 위임자의 서명을 요구하는 소각 명령어.                             | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | *rs`PermissionedBurnInstruction::Burn`*와 동일한 계정을 사용하는 소수점 자릿수 확인이 포함된 소각 명령어. | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | mint의 소각 권한을 교체하거나 비활성화하기 위해 *rs`SetAuthority`*와 함께 사용되는 권한 식별자.           | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | 초기화되지 않은 mint에서 *rs`PermissionedBurnConfig`*를 초기화하고 소각 권한을 저장하는 프로세서 로직.    | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | 소각 권한이 설정된 경우 표준 소각을 거부하고 권한자의 서명을 검증하는 프로세서 로직.                      | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | _rs`AuthorityType::PermissionedBurn`_ 사용 시 소각 권한을 검증하고 교체하는 프로세서 로직.                | [소스](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### TypeScript

아래 `Kit` 예시는 생성된 명령어를 직접 사용합니다. 게시된 레거시
`@solana/spl-token` 패키지는 아직 권한 있는 소각(permissioned burn) 지원을
포함하지 않으므로, 레거시 예시는 포함되지 않습니다.
