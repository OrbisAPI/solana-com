...
한 token account에서 다른 token account로 토큰을 기밀 전송하려면, 발신자와
수신자 모두 _rs`ConfidentialTransferAccount`_ 상태로 구성되고 기밀 전송이 승인된
token account를 보유해야 합니다. 또한 발신자의 token account에는 전송 가능한
기밀 잔액이 있어야 합니다.
...
1. 클라이언트 측에서
   ...
   **암호문 유효성 증명
   ...
   전송 금액 암호문이 세 당사자(소스, 대상, 감사자) 모두를 위해 적절히
   ...
   **범위 증명
   ...
   새로운 가용 잔액과 전송 금액(하위/상위 비트로 분할)이 모두 음수가 아니며
   지정된 범위 내에 있음을 검증합니다.

2. 각 증명에 대해:
   - ZK ElGamal 증명 프로그램을 호출하여 증명 데이터를 검증합니다.
   - 다른 명령어에서 사용하기 위해 증명별 메타데이터를 증명 "컨텍스트 상태"
     계정에 저장합니다.
...
- 클라이언트 측에서 동등성 증명, 암호문 유효성 증명, 범위 증명을 생성합니다
...
- [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  명령어를 호출하여 세 가지 증명 계정을 제공합니다.
- 세 가지 증명 계정을 닫아 rent를 회수합니다.
...
다음 예제는 token account 간에 토큰을 기밀로 전송하는 방법을 보여줍니다.

예제를 실행하려면 다음 명령어를 사용하여 메인넷에서 복제한 Token Extensions
Program이 포함된 로컬 validator를 시작하세요. 로컬 validator를 시작하려면 Solana
CLI가 설치되어 있어야 합니다.
