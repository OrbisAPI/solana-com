...
Confidential Transfer 확장은 mint account에 추가 상태를 추가하여 프라이빗 토큰
전송을 가능하게 합니다. 이 섹션에서는 이 확장이 활성화된 상태로 토큰 mint를
생성하는 방법을 설명합니다.

다음 다이어그램은 Confidential Transfer 확장으로 mint를 생성하는 데 관련된
단계를 보여줍니다:
...
- **authority**: mint의 Confidential Transfer 설정을 변경하고, 자동 승인이
  비활성화된 경우 새로운 기밀 계정을 승인할 권한을 가진 계정입니다.

- **auto_approve_new_accounts**: true로 설정하면 사용자가 기본적으로
  Confidential Transfer가 활성화된 token account를 생성할 수 있습니다. false인
  경우, authority가 각 새로운 token account를 Confidential Transfer에 사용하기
  전에 승인해야 합니다.

- **auditor_elgamal_pubkey**: Confidential 트랜잭션에서 전송 금액을 복호화할 수
  있는 선택적 감사자로, 일반 대중으로부터의 프라이버시를 유지하면서 규정 준수
  메커니즘을 제공합니다.
...
예시에서 보여주듯이 단일 함수 호출로 세 가지 명령어 모두를 포함한 트랜잭션을
...
Program과 함께 로컬 validator를 시작하세요. 로컬 validator를 시작하려면 Solana
CLI가 설치되어 있어야 합니다.
...
<Callout type="info">
  작성 시점 기준으로, Confidential Transfers는 기본 로컬 validator에서
