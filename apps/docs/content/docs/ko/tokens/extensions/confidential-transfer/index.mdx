...
1. 기밀 전송 확장 기능이 적용된 mint account를 생성합니다.
2. 발신자와 수신자를 위한 기밀 전송 확장 기능이 적용된 token account를
   생성합니다.
3. 발신자 계정에 토큰을 발행합니다.
...
<Cards>
  ...
  >
    Confidential Transfer 확장 기능이 적용된 mint account를 생성하는 방법
  </Card>
  <Card
  ...
  >
    지갑, 익스플로러 및 거래소에서 기밀 전송 토큰을 지원하는 방법
  </Card>
  <Card
    title="발행자 가이드"
...
아래 Rust 예제는 앞의 다이어그램에 해당하는 기밀 토큰 전송 단계를 보여줍니다. 각
단계는 명확성을 위해 헬퍼 함수로 분리되어 있습니다.

예제를 실행하려면 다음 명령어를 사용하여 메인넷에서 클론한 Token Extensions
Program과 함께 로컬 validator를 시작하세요. 로컬 validator를 사용하려면 Solana
CLI가 설치되어 있어야 합니다.
...
<Callout type="info">
  작성 시점 기준으로, 기밀 전송(Confidential Transfers)은 기본 로컬
  validator에서 활성화되어 있지 않습니다. 예제 코드를 실행하려면 메인넷 Token
  Extensions Program을 클론해야 합니다.
</Callout>
...
기밀 전송 확장의 전체 명령어 목록
[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)은
다음과 같습니다:
...
| _rs`ApproveAccount`_                | mint가 새로운 token account에 대한 승인을 요구하는 경우, 기밀 전송을 위한 token account를 승인합니다.                                        |
...
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ 증명 대신 _rs`ElGamalRegistry`_ 계정을 사용하여 기밀 전송을 위한 token account를 구성하는 대안적인 방법입니다.    |
