...
## Confidential Transfer 확장으로 token account를 생성하는 방법

Confidential Transfer 확장은 token account에 추가 상태를 더하여 비공개 토큰
전송을 가능하게 합니다. 이 섹션에서는 이 확장이 활성화된 token account를
생성하는 방법을 설명합니다.

다음 다이어그램은 Confidential Transfer 확장으로 token account를 생성하는 데
관련된 단계를 보여줍니다:
...
- **allow_confidential_credits**: true인 경우, 수신 비공개 전송을 허용합니다.

- **allow_non_confidential_credits**: true인 경우, 수신 비공개 전송이 아닌 일반
  전송을 허용합니다.

- **pending_balance_credit_counter**: 입금 및 전송 명령어로부터 수신된 대기 중인
  잔액 크레딧을 카운트합니다.

- **maximum_pending_balance_credit_counter**: 대기 중인 잔액을 사용 가능한
  잔액으로 전환하기 위해 _rs`ApplyPendingBalance`_ 명령어가 필요하기 전, 대기
  중인 크레딧의 최대 허용 횟수입니다.

- **expected_pending_balance_credit_counter**: _rs`ApplyPendingBalance`_
  명령어가 마지막으로 처리되었을 때 클라이언트가 instruction data를 통해 제공한
...
이 구분이 없으면 공격자가 token account에 토큰을 반복적으로 전송하여 token
...
ElGamal 복호화가 더 큰 숫자에 대해 더 많은 연산을 필요로 하기 때문입니다. 암호문
산술 구현은
...
1. 클라이언트는 현재 대기 잔액과 사용 가능 잔액을 조회하고 합계를 암호화한 후,
   token account 소유자의 AES 키를 사용하여 암호화된
   `decryptable_available_balance`를 제공합니다.

2. 예상 및 실제 대기 크레딧 카운터는 _rs`ApplyPendingBalance`_ 명령어가 생성된
   시점과 처리된 시점 사이의 카운터 값 변경을 추적합니다:
   ...
   - `actual_pending_balance_credit_counter`: _rs`ApplyPendingBalance`_ 명령어가
     처리되는 시점의 token account에 있는 `pending_balance_credit_counter` 값
...
예상/실제 카운터 값이 다른 경우 클라이언트는 카운터 차이에 해당하는 최근
입금/전송 명령어를 조회하여 정확한 잔액을 계산해야 합니다.

**잔액 조정 프로세스**

예상 및 실제 보류 잔액 카운터가 다를 경우, 다음 단계에 따라
`decryptable_available_balance`를 조정하세요:

1. token account에서 `decryptable_available_balance`로 시작합니다
2. 카운터 차이(실제 - 예상)만큼 입금 및 전송 명령어를 포함한 가장 최근
   트랜잭션을 가져옵니다:
   - 입금 명령어에서 공개 금액을 더합니다
   - 전송 명령어에서 목적지 암호문 금액을 복호화하여 더합니다

</Callout>

### 필수 명령어

Confidential Transfer 확장 기능이 있는 token account를 생성하려면 세 가지
명령어가 필요합니다:

1. **Token Account 생성**: Associated Token Program의
   _rs`AssociatedTokenAccountInstruction:Create`_ 명령어를 호출하여 token
   account를 생성합니다.
...
**token account 소유자만 기밀 전송을 위한 token account를 구성할 수 있습니다**.

_rs`ConfigureAccount`_ 명령어는 token account 소유자만 생성할 수 있는 암호화
키와 증명 데이터를 클라이언트 측에서 생성해야 합니다.

*rs`PubkeyValidityProofData`*는 ElGamal 키가 유효한지 검증하는 증명을
...
다음 코드는 Confidential Transfer 확장 기능이 있는 associated token account를
생성하는 방법을 보여줍니다,

예제를 실행하려면 다음 명령어를 사용하여 메인넷에서 클론된 Token Extension
...
<Callout type="info">
  이 문서 작성 시점에서 기본 로컬 validator에는 Confidential Transfers가
