...
1. 클라이언트 측에서
   ...
   **범위 증명
   ...
   출금 후 남은 가용 잔액이 음수가 아니며 지정된 범위 내에 있는지 검증합니다.
...
4. 증명 계정을 닫아 생성에 사용된 SOL을 회수합니다.

다음 다이어그램은 기밀 가용 잔액에서 공개 잔액으로 토큰을 출금하는 단계를
보여줍니다:
...
기밀 가용 잔액에서 공개 잔액으로 토큰을 출금하려면 다음을 수행해야 합니다:
...
<Callout type="info">
  작성 시점 기준으로, 기본 로컬 validator에서는 Confidential Transfers가
