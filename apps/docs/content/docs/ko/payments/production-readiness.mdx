...
있으며, 여기에는 사용될 컴퓨팅 유닛의 추정치가 포함됩니다.
[`@solana/kit`](https://github.com/anza-xyz/kit)는 트랜잭션의 리소스 한도를
추정하고 한 단계에서 메시지에 설정하는 헬퍼 함수를 제공합니다 (내부적으로
`simulatetransaction` 메서드를 사용). 버전 1 트랜잭션의 경우 이 헬퍼는 로드된
계정 데이터 크기 한도도 추정합니다.
...
<Callout type="info">
  kit 플러그인 클라이언트를 사용하여 트랜잭션을 빌드하고 전송하는 경우,
  일반적으로 이 단계는 필요하지 않습니다 — 클라이언트가 전송
  시(`.sendTransaction()`) 컴퓨팅 예산 명령어를 자동으로 추가합니다. 위의 수동
  흐름은 `@solana/kit`를 사용하여 트랜잭션을 직접 조합할 때를 위한 것입니다.
</Callout>

프로덕션 환경에서 동일한 유형의 트랜잭션을 여러 번 반복하는 경우, 매번 컴퓨팅
유닛을 추정하는 오버헤드를 줄이기 위해 해당 트랜잭션 유형의 컴퓨팅 추정치를
캐싱하는 것을 고려해야 합니다.

### Jito 번들

Jito 번들은 여러 트랜잭션의 원자적 실행을 관리하기 위한 도구입니다. 이는 팁과
함께 여러 트랜잭션을 Jito 네트워크에 전송함으로써 실현됩니다. 팁은 Jito
네트워크가 귀하의 트랜잭션을 블록에 포함하도록 인센티브를 제공하는 데 사용할 수
있습니다.
...
- [Jito 번들에 관한 QuickNode 가이드](https://www.quicknode.com/guides/solana-development/transactions/jito-bundles)

### 재시도 전략

트랜잭션은 다양한 이유로 실패할 수 있습니다. 성공/실패를 즉시 반환하는 전통적인
결제 API와 달리, 블록체인 트랜잭션은 확인 추적이 필요합니다.

**핵심 개념:**

- **블록해시 만료**: 트랜잭션은 약 150블록(약 60~90초) 동안 유효합니다
...
- [Helius: Solana에서 트랜잭션 처리하는 방법](https://www.helius.dev/blog/how-to-land-transactions-on-solana)
...
| 수준        | Solana 정의                     | 전통 금융 대응 | 사용 사례               |
| ----------- | ------------------------------- | -------------- | ----------------------- |
| `processed` | 블록에 포함, 아직 투표되지 않음 | 승인 대기 중   | 실시간 UI 업데이트      |
| `confirmed` | 슈퍼 과반수 투표 완료           | 자금 처리 완료 | **대부분의 결제**       |
| `finalized` | 루트에 고정, 되돌릴 수 없음     | 자금 정산 완료 | 고액 거래, 컴플라이언스 |

**각 수준의 사용 시점:**

- **UI 업데이트**: 즉각적인 피드백을 위해 `processed` 표시 ("결제 제출됨")
- **사용자 계정 크레딧**: `confirmed` 대기 (대부분의 트랜잭션에 안전)
...
| `INSUFFICIENT_FUNDS`         | 토큰 잔액 부족          | 사용자가 더 많은 잔액 필요                                                                                                |
| `ACCOUNT_NOT_FOUND`          | token account 없음      | 트랜잭션에서 ATA 생성                                                                                                     |

## 가스리스 트랜잭션

사용자들은 네트워크 수수료를 위해 SOL을 따로 확보하는 것이 아니라
스테이블코인으로 결제하길 원합니다. 가스리스 트랜잭션은 이 문제를
해결합니다—Venmo 사용자들이 ACH 수수료를 신경 쓰지 않는 것과 비슷한 방식입니다.
전체 구현 방법은
...
- **핫 월렛과 콜드 월렛을 분리하세요.** 핫 월렛은 운영용, 콜드 월렛은 자산
  보관용으로 사용합니다.
- **모든 프로덕션 키를 백업하세요.** 여러 안전한 위치에 암호화된 백업을
  보관하세요. 키를 분실하면 영구적으로 접근이 불가능해집니다.
- **개발 네트워크와 메인넷에 서로 다른 키를 사용하세요.** 개발 네트워크 키를
  메인넷 키로 사용해서는 안 됩니다. 환경 기반 설정을 사용하여 각 네트워크에 맞는
  키가 로드되도록 하세요.

### 서명 인프라

프로덕션 백엔드 서명에는 [Keychain](/docs/tools/keychain)을 사용하세요—단일
인터페이스를 통해 여러 키 관리 백엔드를 추상화하는 통합 서명 라이브러리입니다:
