...
각 서명은 개인 키에 의해 생성됩니다. 해당 키가 로컬 keypair에 있는지, 클라우드
HSM 또는 KMS에 있는지, 아니면 관리형 지갑 서비스에 있는지는 프로덕션 설계 결정
사항입니다.
[프로덕션에서의 서명](/docs/core/transactions/signing-in-production)을
참조하세요.

배열의 첫 번째 서명은 **수수료 지불자**에 속하며, 이는 트랜잭션의
...
- System Program 소유 계정 또는 논스 계정이어야
...
`header` 필드는 `account_keys` 배열을 권한 그룹으로 분류하는 세 개의 `u8` 필드를
가진
...
블록해시는 150 slot 이후 만료됩니다. 트랜잭션이 도착했을 때 블록해시가 더 이상
유효하지 않으면,
[내구성 있는 논스 트랜잭션](/docs/core/transactions/durable-nonces)이 아닌 한
...
대신 `account_keys` 배열의 인덱스를 통해 계정을 참조하며, 다음을 포함합니다:
...
계정 키, 명령어)은 컴팩트-u16 길이 인코딩으로 접두사가 붙습니다. 이 형식은 0~127
...
_rs`PACKET_DATA_SIZE`_ = 1,232바이트를 기준으로, 사용 가능한 공간을 계산할 수
있습니다：
...
상호작용할 수 있도록 하는 방식을 보여줍니다. 이 예시에서는 SOL이 한 계정에서
다른 계정으로 전송됩니다.

발신자 계정의
[메타데이터](/docs/core/instructions/instruction-structure#account-metadata)는
해당 계정이 트랜잭션에 서명해야 함을 나타냅니다. 이를 통해 System Program이
lamport를 차감할 수 있습니다. 발신자와 수신자 계정 모두 lamport 잔액이 변경될 수
있도록 쓰기 가능 상태여야 합니다. 이 명령어를 실행하려면 발신자의 지갑이 서명과
SOL 전송 명령어가 포함된 메시지를 담은 트랜잭션을 전송합니다.

![SOL 전송 다이어그램](/assets/docs/core/transactions/sol-transfer.svg)

트랜잭션이 전송되면 System Program이 전송 명령어를 처리하고 두 계정의 lamport
잔액을 업데이트합니다.
...
출금할 수 있는지에 대한 프로토콜 수준의 검사는 없습니다. lamport는 해당 계정을
소유한 프로그램만 이동시킬 수 있으므로,
[토큰 민트](/docs/tokens/basics/create-mint)나 프로그램, 또는 본인이 제어하지
않는 [PDA](/docs/core/pda)로 SOL을 전송하면 **자금이 영구적으로 손실될 위험**이
있습니다 — 소유 프로그램이 지정한 권한만이 이를 반환할 수 있습니다.
...
예상되는 민트와 일치하지 않는 전송을 거부합니다. 네이티브 **SOL** 전송에는
이러한 보호 장치가 없으므로, 발신자는 서명 전에 수신자를 반드시 확인해야 합니다.
