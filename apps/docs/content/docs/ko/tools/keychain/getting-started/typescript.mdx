...
`@solana/keychain-kit-plugin`는 키체인 서명자를 직접
[Kit 클라이언트](https://github.com/anza-xyz/kit)에 설치하여, `payer` 및
`identity`로 연결하므로 위에서 보여준 수동 `pipe` 설정을 건너뛸 수 있습니다:
...
이 플러그인은 `createKeychainSigner`와 동일한 백엔드 태그 설정을 허용하며,
실제로 디스패치되는 백엔드만 번들에 포함됩니다(백엔드 패키지는 동적 `import()`를
통해 로드됩니다).

`keychainPayer` 또는 `keychainIdentity`를 사용하여 하나의 역할만 설정하고, 단일
