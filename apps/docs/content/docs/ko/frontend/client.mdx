...
<Callout type="info">
  헤드리스 환경 — API 라우트, 워커, 또는 브라우저 지갑이 없는 스크립트에서 실행
  중이신가요? 원시 keypair 대신 백엔드 키 관리 서비스를 사용하여 서명하세요.
...
- **커스텀 스토어**: 자체 Zustand 스토어를 주입하여 IndexedDB/localStorage에서
  하이드레이션하거나, 서버 세션에 상태를 미러링하거나, 브라우저 탭 간 조정을
  수행합니다.
- **React 훅과의 연결**: 동일한 런타임 위에서 편리한 훅을 사용하려면 설정된
