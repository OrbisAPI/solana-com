...
Плагин принимает ту же конфигурацию с тегом бэкенда, что и
`createKeychainSigner`, при этом в сборку включается только тот бэкенд, которому
он делегирует запросы (пакеты бэкендов загружаются через динамический
`import()`).

Используйте `keychainPayer` или `keychainIdentity`, чтобы назначить только одну
роль и комбинировать бэкенды на одном клиенте:
...
Уже есть `SolanaSigner`? Каждый подписант keychain является валидным Kit
...
вместо него (`pnpm add @solana/kit-plugin-signer`):
