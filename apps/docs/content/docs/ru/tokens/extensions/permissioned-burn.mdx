...
## Как создать Mint и сжигание токенов с разрешением
...
4. Выполните сжигание с помощью _rs`PermissionedBurnInstruction::BurnChecked`_,
   подписанного как владельцем token account, так и уполномоченным на сжигание.
5. При необходимости отключите разрешённое сжигание с помощью
   ...
   полномочия `None`, что повторно активирует стандартные инструкции сжигания.
...
_rs`PermissionedBurnConfig`_. Именно этот размер используется в
_rs`CreateAccount`_.
...
Инициализируйте расширение _rs`PermissionedBurnConfig`_ на mint с указанием
уполномоченного на сжигание.
...
## !!steps Создание token account и выпуск токенов
...
## !!steps Сжигание с использованием burn authority
...
<Callout type="info" title="Instruction Order">
  _rs`PermissionedBurnInstruction::Initialize`_ должна идти перед
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Расширение минта, хранящее authority, необходимую для совместной подписи каждого сжигания для данного минта.                   | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Инструкция, инициализирующая конфигурацию permissioned burn до _rs`InitializeMint`_.                                           | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Инструкция сжигания, требующая подписей от burn authority и владельца или делегата token account.                              | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Инструкция сжигания с проверкой десятичных знаков, использующая те же аккаунты, что и _rs`PermissionedBurnInstruction::Burn`_. | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Дискриминатор authority, используемый совместно с _rs`SetAuthority`_ для ротации или отключения burn authority на минте.       | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Логика процессора, инициализирующая _rs`PermissionedBurnConfig`_ на неинициализированном минте и сохраняющая burn authority.   | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Логика процессора, отклоняющая стандартные операции сжигания при установленной burn authority и проверяющая подпись authority. | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Логика процессора, проверяющая и выполняющая ротацию burn authority при использовании _rs`AuthorityType::PermissionedBurn`_.   | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Пример `Kit` ниже использует сгенерированные инструкции напрямую. Опубликованный
устаревший пакет `@solana/spl-token` пока не поддерживает разрешённое сжигание,
поэтому пример для устаревшего варианта не включён.
