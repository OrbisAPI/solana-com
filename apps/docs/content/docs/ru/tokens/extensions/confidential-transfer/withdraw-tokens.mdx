...
1. Создайте
   ...
   **Доказательство равенства
   ...
   Проверяет, что оставшийся зашифрованный текст доступного баланса после вывода
...
4. Закройте аккаунты доказательств, чтобы вернуть использованные SOL.

Следующая диаграмма показывает шаги вывода токенов из конфиденциального
...
- Вызвать инструкцию
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
  передав два аккаунта доказательств.
- Закрыть два аккаунта доказательств для возврата rent.
...
В следующем примере показано, как вывести токены из конфиденциального доступного
баланса в публичный баланс.

Чтобы запустить пример, запустите локальный validator с Token Extensions
Program, клонированной из mainnet, используя следующую команду. Для запуска
...
<Callout type="info">
  На момент написания Confidential Transfers не включён в локальный validator по
  умолчанию. Для запуска примера кода необходимо клонировать Token Extensions
  Program из mainnet.
</Callout>
