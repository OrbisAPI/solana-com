...
В этом разделе описывается второй этап: применение отложенного баланса к
доступному балансу.

Когда токены «вносятся» из публичного баланса или конфиденциально переводятся с
одного token account на другой, они изначально добавляются в конфиденциальный
отложенный баланс. Прежде чем токены можно будет использовать для
конфиденциальных переводов, отложенный баланс необходимо «применить» к
доступному балансу.
...
Следующая диаграмма показывает шаги, выполняемые при применении отложенного
...
Чтобы преобразовать отложенный баланс в доступный, вызовите инструкцию
...
`confidential_transfer_apply_pending_balance`, который создаёт и отправляет
...
Program, клонированной из мейннета, используя следующую команду. Для запуска
...
<Callout type="info">
  На момент написания этой документации Конфиденциальные переводы не включены в
  локальном validator по умолчанию. Для запуска примера кода необходимо
  клонировать Token Extensions Program из мейннета.
</Callout>
