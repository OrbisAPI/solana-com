...
Конфиденциальные переводы позволяют переводить токены между token accounts, не
...
- [Книга рецептов Confidential Balances](https://github.com/solana-developers/Confidential-Balances-Sample)
  — Фрагменты кода для работы с расширением Confidential Transfer
...
в Token Extensions Program, которые позволяют переводить токены между
...
7. **Примените** ожидающий баланс получателя к **конфиденциальному доступному
   балансу**.
8. **Снимите** конфиденциальный доступный баланс получателя на **публичный
   баланс**.

Подробнее о шагах процесса конфиденциального перевода см. на соответствующих
страницах:
...
На диаграмме ниже показана подробная последовательность основного процесса
...
Приведённый ниже пример на Rust демонстрирует шаги для выполнения
конфиденциальных переводов токенов, соответствующие предыдущей диаграмме. Для
наглядности шаги разделены на вспомогательные функции.

Чтобы запустить пример, запустите локальный validator с Token Extensions
Program, клонированной из основной сети, используя следующую команду. Для
использования локального validator необходимо установить Solana CLI.
...
<Callout type="info">
  ...
  локальном validator. Для запуска примера кода необходимо клонировать Token
  Extension Program из основной сети.
</Callout>
...
| _rs`ApproveAccount`_                | Подтверждает token account для конфиденциальных переводов, если mint требует одобрения для новых token accounts.                                                                |
| _rs`EmptyAccount`_                  | Очищает ожидающий и доступный конфиденциальные балансы, чтобы можно было закрыть token account.                                                                                 |
...
| _rs`Transfer`_                      | Конфиденциально переводит токены между token accounts.                                                                                                                          |
...
| _rs`DisableConfidentialCredits`_    | Блокирует входящие конфиденциальные переводы, сохраняя возможность публичных переводов.                                                                                         |
...
| _rs`TransferWithFee`_               | Конфиденциально переводит токены между token accounts с комиссией.                                                                                                              |
| _rs`ConfigureAccountWithRegistry`_  | Альтернативный способ настройки token accounts для конфиденциальных переводов с использованием аккаунта _rs`ElGamalRegistry`_ вместо доказательства _rs`VerifyPubkeyValidity`_. |
