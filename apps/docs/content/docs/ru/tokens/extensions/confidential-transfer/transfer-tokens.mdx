...
отправитель, и получатель должны иметь token account, настроенные с состоянием
...
1. Создайте
   ...
   **Доказательство корректности шифртекста
   ...
   Проверяет, что шифртексты суммы перевода корректно сгенерированы для всех
   трёх сторон (источник, получатель и аудитор), гарантируя, что сумма перевода
...
2. Для каждого доказательства:
   - Вызовите программу доказательств ZK ElGamal для верификации данных
     доказательства.
   - Сохраните специфические метаданные доказательства в аккаунте "context
     state" доказательства для использования в других инструкциях.
...
На следующей диаграмме показаны шаги, выполняемые при переводе токенов с token
account отправителя на token account получателя.
...
Для конфиденциального перевода токенов с одного token account на другой
необходимо:
...
Program, клонированной из mainnet, с помощью следующей команды. Для запуска
...
<Callout type="info">
  ...
  validator. Для запуска примера кода необходимо клонировать Token Extensions
  Program из mainnet.
</Callout>
